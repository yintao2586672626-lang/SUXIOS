(function () {
    ...
    const REVENUE_COCKPIT_STATIC_EXPORT_KEYS = Object.freeze([
    ...
    const REVENUE_COCKPIT_STATIC_ASYNC_KEYS = new Set([
        'loadRevenueCockpitSnapshot',
        'saveRevenueDecisionSnapshotWithReadback',
        'restoreRevenueDecisionSnapshotWithReadback',
        'createRevenueOpportunityPendingApprovalWithReadback',
    ...
    const createRevenueCockpitStaticHelpers = () => {
        if (revenueCockpitStaticHelpers) return revenueCockpitStaticHelpers;
        const factory = window.SUXI_REVENUE_COCKPIT_STATIC;
        if (!factory || typeof factory.create !== 'function') return null;
        const helpers = factory.create({
            revenueAiReasonText,
            resolveRevenueOverviewAsOfDate,
            revenueAiIsoDate,
            revenueOverviewAsOfDateContractVersion: REVENUE_OVERVIEW_AS_OF_DATE_CONTRACT_VERSION,
        });
        const missingKeys = REVENUE_COCKPIT_STATIC_EXPORT_KEYS.filter(
            key => typeof helpers?.[key] !== 'function',
        );
        if (missingKeys.length) {
            throw new Error(`经营驾驶舱静态工具缺少接口：${missingKeys.join('、')}`);
        }
        revenueCockpitStaticHelpers = helpers;
        return revenueCockpitStaticHelpers;
    };

    const loadRevenueCockpitStatic = () => {
        try {
            const loaded = createRevenueCockpitStaticHelpers();
            if (loaded) return Promise.resolve(loaded);
        } catch (error) {
            return Promise.reject(error);
        }
        if (revenueCockpitStaticLoadPromise) return revenueCockpitStaticLoadPromise;
        if (typeof document === 'undefined' || !document.head) {
            return Promise.reject(new Error('当前运行环境无法加载经营驾驶舱静态工具'));
        }

        document.querySelector('script[data-suxi-revenue-cockpit-static="1"]')?.remove();
        revenueCockpitStaticLoadPromise = new Promise((resolve, reject) => {
            const script = document.createElement('script');
            script.src = `${revenueCockpitStaticScript}?v=${revenueCockpitStaticVersion}`;
            script.async = true;
            script.dataset.suxiRevenueCockpitStatic = '1';
            script.onload = () => {
                script.dataset.loaded = '1';
                try {
                    const helpers = createRevenueCockpitStaticHelpers();
                    if (!helpers) throw new Error('经营驾驶舱静态工具加载完成但未注册');
                    resolve(helpers);
                } catch (error) {
                    reject(error);
                }
            };
            script.onerror = () => reject(new Error('经营驾驶舱静态工具加载失败'));
            document.head.appendChild(script);
        }).catch((error) => {
            revenueCockpitStaticLoadPromise = null;
            document.querySelector(
                'script[data-suxi-revenue-cockpit-static="1"]:not([data-loaded="1"])',
            )?.remove();
            throw error;
        });
        return revenueCockpitStaticLoadPromise;
    };

    const revenueCockpitStaticFallback = (key) => {
        void loadRevenueCockpitStatic().catch(() => {});
        if (key === 'resolveRevenueCockpitScope') {
            return {
                status: 'not_loaded',
                selectedPlatform: '',
                selectedDate: '',
                ...
                platformOptions: [],
                dateOptions: [],
                notice: '经营驾驶舱工具正在按需加载。',
            };
        }
        if (key === 'buildRevenueCockpitModel') {
            return {
                status: 'loading',
                statusLabel: '正在加载',
                statusClass: 'border-slate-200 bg-slate-100 text-slate-600',
                headline: '经营驾驶舱工具正在加载',
                summary: '正在加载经营驾驶舱模型、范围与下载工具。',
                dateNotice: '',
                ...
                actionDisabledReason: '经营驾驶舱工具尚未加载完成',
            };
        }
        throw new Error(`经营驾驶舱静态工具尚未加载：${key}`);
    };

    const revenueCockpitStaticBridge = Object.freeze(Object.fromEntries(
        REVENUE_COCKPIT_STATIC_EXPORT_KEYS.map((key) => [
            key,
            REVENUE_COCKPIT_STATIC_ASYNC_KEYS.has(key)
                ? async (...args) => (await loadRevenueCockpitStatic())[key](...args)
                : (...args) => {
                    const helpers = createRevenueCockpitStaticHelpers();
                    return helpers ? helpers[key](...args) : revenueCockpitStaticFallback(key);
                },
        ]),
    ));
