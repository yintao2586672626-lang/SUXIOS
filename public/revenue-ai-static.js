(function() {
    ...
    const revenueAiMetricDefinitions = Object.freeze([ {
        ...
        key: 'whole_hotel_room_revenue',
        label: '全酒店住宿房费',
        scope: 'whole_hotel_accommodation',
        dateBasis: 'pms_business_date'
    }, {
        key: 'whole_hotel_sellable_room_nights',
        label: '全酒店可售间夜',
        scope: 'whole_hotel_accommodation',
        dateBasis: 'pms_business_date'
    }, {
        ...
        key: 'ota_contribution_revpar',
        label: 'OTA收入/全酒店可售间夜',
        scope: 'cross_source_comparison',
        dateBasis: 'same_date_key_distinct_source_semantics'
    }, {
    ...
    const resolveRevenueAiOverviewRequest = ({hasToken = false, currentPage = '', businessDate = '', hotelId = '', platform = 'ctrip'} = {}) => {
        ...
        if (![ 'compass', 'agent-center' ].includes(String(currentPage || ''))) return {
            shouldLoad: false,
            endpoint: '',
            reason: 'not_revenue_ai_surface'
        };
        return {
            shouldLoad: true,
            endpoint: buildRevenueAiOverviewEndpoint({
                businessDate,
                hotelId,
                platform
            }),
    ...
    const resolveRevenueAiOverviewResponse = ({response = null, error = null} = {}) => {
        ...
        if (response && Number(response.code) === 200) return {
    ...
    const metricDisplayText = metric => {
        if (metric && metric.display !== void 0 && metric.display !== null && metric.display !== '') return String(metric.display);
        if (metric && metric.value !== void 0 && metric.value !== null && metric.value !== '') return String(metric.value);
        return '--';
    };
    ...
    const revenueAiMetricTruthLines = (truth = {}) => {
        ...
        const storedText = recordCount > 0 ? `入库 ${storedCount}/${recordCount}；回读 ${readbackCount}/${recordCount}` : '无可核验入库记录';
        ...
        return [ {
            ...
            key: 'date',
            label: '日期',
            value: revenueAiTruthRangeText(truth?.date_range)
        }, {
            key: 'source',
            label: '来源',
            value: sourceText
            ...
            key: 'collected',
            label: '采集时间',
            value: revenueAiTruthRangeText(truth?.collected_at_range)
        }, {
            key: 'persistence',
            label: '入库',
            value: storedText
            ...
            key: 'scope',
            label: '口径',
            value: String(truth?.scope_label || 'OTA渠道指标，不代表全酒店经营')
        } ];
    };
    const revenueAiMetricTruthSummary = (truth = {}) => revenueAiMetricTruthLines(truth).map(line => `${line.label}：${line.value}`).join('；');
    ...
    const revenueAiCanonicalFactLayerGaps = (overview = {}) => {
        ...
        const diagnostics = factLayer.analysis_diagnostics && typeof factLayer.analysis_diagnostics === 'object' ? factLayer.analysis_diagnostics : {};
        const diagnosticIssues = Array.isArray(diagnostics.issues) ? diagnostics.issues : null;
        const hasGapContract = Array.isArray(factLayer.analysis_gaps) || Array.isArray(factLayer.ai_review_gaps) || diagnosticIssues !== null || Object.prototype.hasOwnProperty.call(factLayer, 'unique_remaining_gap');
        ...
        const uniqueGap = factLayer.unique_remaining_gap && typeof factLayer.unique_remaining_gap === 'object' ? [ factLayer.unique_remaining_gap ] : [];
        const seen = new Set;
        const sourceGaps = diagnosticIssues !== null ? diagnosticIssues : [ ...analysisGaps, ...reviewGaps, ...uniqueGap ];
        return sourceGaps.filter(gap => gap && typeof gap === 'object').map(gap => {
            ...
            const platform = source === 'ctrip_ota' ? 'ctrip' : source === 'meituan_ota' ? 'meituan' : source;
            return {
                ...gap,
                ...
                affected_metrics: Array.isArray(gap.affected_metrics) ? gap.affected_metrics : gap.category ? [ String(gap.category) ] : [],
                next_action: gap.next_action || (isPricingGuard ? '为启用房型配置最低保护价，保存回显后重新审核。' : '补齐对应来源并完成同店同日保存回读。'),
                ...
                target_platform: gap.target_platform || ([ 'ctrip', 'meituan' ].includes(platform) ? platform : '')
            };
        }).filter(Boolean);
    };
    ...
    const revenueAiClosureGroupStatus = (chips = []) => {
        ...
        if (statuses.some(status => [ 'warning', 'partial', 'stale', 'not_calculable', 'missing', 'unverified', 'unknown' ].includes(status))) return statuses.includes('ok') ? 'partial' : 'warning';
    ...
    const buildRevenueAiBusinessClosure = ({overview = null, overviewError = '', overviewLoading = false} = {}) => {
        ...
        const factLayer = overview.three_source_fact_layer && typeof overview.three_source_fact_layer === 'object' ? overview.three_source_fact_layer : {};
        const diagnostics = factLayer.analysis_diagnostics && typeof factLayer.analysis_diagnostics === 'object' ? factLayer.analysis_diagnostics : {};
        const diagnosticAnalysisAllowed = typeof diagnostics?.decision_use?.revenue_analysis?.allowed === 'boolean' ? diagnostics.decision_use.revenue_analysis.allowed : null;
        const hasFactLayerContract = Object.keys(factLayer).length > 0 && (Object.prototype.hasOwnProperty.call(factLayer, 'source_completeness') || Object.prototype.hasOwnProperty.call(factLayer, 'analysis_gaps') || Object.prototype.hasOwnProperty.call(factLayer, 'facts'));
        const factLayerReady = diagnosticAnalysisAllowed !== null ? diagnosticAnalysisAllowed : overview.revenue_analysis_status === 'ready' && factLayer.all_three_sources_readback_verified === true;
        const factLayerOtaReady = factLayer.all_three_sources_readback_verified === true || factLayer?.sources?.ctrip_ota?.data_status === 'readback_verified' && factLayer?.sources?.meituan_ota?.data_status === 'readback_verified';
        const useCanonicalOtaMetrics = factLayerReady || factLayerOtaReady;
        const canonicalOta = factLayer?.facts?.ota_channel?.combined || {};
        const revenueMetric = useCanonicalOtaMetrics ? overview.metrics?.ota_room_revenue || {} : revenueAiClosureMetric(closure, [ 'sections', 'revenue' ]);
        const orderMetric = useCanonicalOtaMetrics ? {
            key: 'ota_orders',
            value: canonicalOta.orders ?? null,
            unit: 'orders',
            status: canonicalOta.orders === null || canonicalOta.orders === void 0 ? 'not_calculable' : 'ok',
            reason: canonicalOta.orders === null || canonicalOta.orders === void 0 ? 'source_fact_missing' : '',
            truth: overview.metrics?.ota_room_revenue?.truth || {}
        } : revenueAiClosureMetric(closure, [ 'sections', 'orders' ]);
        const roomNightMetric = useCanonicalOtaMetrics ? overview.metrics?.ota_room_nights || {} : revenueAiClosureMetric(closure, [ 'sections', 'room_nights' ]);
        const adrMetric = useCanonicalOtaMetrics ? overview.metrics?.ota_adr || {} : revenueAiClosureMetric(closure, [ 'sections', 'adr_conversion', 'metrics', 'adr' ]);
        ...
        const flowMetric = useCanonicalOtaMetrics ? canonicalConversionPlaceholder('flow_rate') : revenueAiClosureMetric(closure, [ 'sections', 'adr_conversion', 'metrics', 'flow_rate' ]);
        const submitMetric = useCanonicalOtaMetrics ? canonicalConversionPlaceholder('submit_rate') : revenueAiClosureMetric(closure, [ 'sections', 'adr_conversion', 'metrics', 'submit_rate' ]);
        ...
        const missingRows = canonicalFactLayerGaps === null ? legacyMissingRows : revenueAiClosureIssueRows(canonicalFactLayerGaps, 'missing');
        const legacyAnomalyRows = revenueAiClosureIssueRows(closure.anomaly_judgment?.items, 'anomaly');
        const anomalyRows = hasFactLayerContract && factLayerOtaReady ? legacyAnomalyRows.filter(row => {
            const code = String(row?.code || '');
            return code !== 'ota_collection_quality:unverified' && code !== 'p0_ota_gate_not_ready' && !code.startsWith('critical_metric_untrusted:') && !code.startsWith('p0_ota_gate_missing:');
        }) : legacyAnomalyRows;
        ...
        const calculationAllowed = hasFactLayerContract ? factLayerReady : closure.calculation_allowed === true;
        const canonicalBlockingGap = Array.isArray(canonicalFactLayerGaps) ? canonicalFactLayerGaps[0] || null : null;
        const calculationDetail = hasFactLayerContract ? String(diagnostics.summary || '') || (factLayerReady ? '三源事实已完成同店同日回读。' : String(canonicalBlockingGap?.display_reason || canonicalBlockingGap?.message || '三源事实层仍有必需来源缺口。')) : revenueAiReasonText(revenueUse.status || (calculationAllowed ? '' : 'blocked_by_data_credibility'));
        const canonicalNextAction = hasFactLayerContract ? String(diagnostics.next_action || (!factLayerReady ? canonicalBlockingGap?.next_action || '补齐唯一三源事实缺口后重新审核。' : '')) : '';
        const aiDecisionAllowed = hasFactLayerContract ? factLayerReady : aiDecision.allowed === true;
        const metricChips = [ revenueAiClosureMetricChip(revenueMetric, '收入', 'revenue'), revenueAiClosureMetricChip(orderMetric, '订单', 'orders'), revenueAiClosureMetricChip(roomNightMetric, '间夜', 'room_nights'), revenueAiClosureMetricChip(adrMetric, 'ADR', 'adr'), revenueAiClosureMetricChip(flowMetric, '流量转化', 'flow_rate'), revenueAiClosureMetricChip(submitMetric, '提交转化', 'submit_rate') ];
        const revenueAnalysisStatus = factLayerReady ? 'ready' : factLayerOtaReady ? 'partial' : revenueAiClosureGroupStatus(metricChips);
        const summaryChips = [ revenueAiClosureSummaryChip('calculation', '收益计算', calculationAllowed ? '允许' : '阻断', calculationAllowed ? 'ok' : 'blocked', calculationDetail), revenueAiClosureSummaryChip('missing', '缺失项', `${missingRows.length}项`, missingRows.length > 0 ? 'warning' : 'ok', missingRows.length > 0 ? '继续补齐缺失项' : '未发现关键缺失项'), revenueAiClosureSummaryChip('anomaly', '异常判断', `${anomalyRows.length}项`, anomalyRows.length > 0 ? 'warning' : 'ok', anomalyRows.length > 0 ? '需人工复核' : '未命中异常') ];
        return {
            ...
            summary: String(diagnostics.summary || '') || (factLayerReady ? 'PMS 仅承载全酒店住宿事实，携程/美团仅承载 OTA 渠道事实；收入不跨口径相加。' : factLayerOtaReady ? '携程、美团 OTA 渠道事实已回读；PMS 全酒店住宿事实仍缺当前来源证据，PMS 与跨源指标保持为空，OTA 不冒充全酒店收入。' : closure.scope_statement || '仅基于已验证 OTA 渠道数据，不代表全酒店经营口径。'),
            calculationAllowed,
            summaryChips,
            ...
            rows: [ {
                ...
                secondary: hasFactLayerContract ? factLayerReady ? 'PMS=全酒店住宿；携程/美团=OTA渠道；缺失值保持为空。' : '携程/美团 OTA 已回读；PMS 与跨源指标继续为空，等待当前来源证据通过。' : closure.scope_statement || '只读取 OTA 渠道事实和 metric_trust。',
                statusLabel: revenueAiStatusLabel(closureStatus),
                className: revenueAiStatusClass(closureStatus)
            }, {
                ...
                secondary: canonicalNextAction || (execution.reason ? revenueAiReasonText(execution.reason) : '建议需人工审核、执行证据和效果复盘。'),
    ...
    const buildRevenueAiGapRows = ({overview = null, overviewError = '', overviewLoading = false} = {}) => {
        ...
        const canonicalFactLayerGaps = revenueAiCanonicalFactLayerGaps(overview);
        const channelMetricGaps = Array.isArray(overview.channel_metric_gaps) ? overview.channel_metric_gaps : [];
        ...
        const sourceRows = canonicalFactLayerGaps === null ? [ ...channelMetricGaps, ...legacyRows ] : [ ...channelMetricGaps, ...canonicalFactLayerGaps ];
        const seen = new Set;
    ...
    const buildRevenueAiStatusRows = ({readiness = {}, overview = null, ctripStatus = overview?.channel_statuses?.ctrip || null, meituanStatus = overview?.channel_statuses?.meituan || null, lastSyncedAt = overview?.last_success_at || '--', completeness = overview?.data_completeness || null, overviewStatus = 'unknown', overviewLoading = false, overviewError = '', hotelName = '全部门店', hasHotelFilter = false, businessDate = ''} = {}) => {
        const normalizedOverviewStatus = overviewError ? 'failed' : overview?.revenue_analysis_status || overview?.data_status || (overviewLoading ? 'not_loaded' : overviewStatus || 'unknown');
        const isThreeSourceLayered = overview?.scope === 'three_source_layered';
        const readinessPercent = readiness?.percent !== null && readiness?.percent !== void 0 && readiness?.percent !== '' && Number.isFinite(Number(readiness.percent)) ? Math.max(0, Math.min(100, Number(readiness.percent))) : null;
        return [ {
            key: 'hotel',
            label: '当前酒店',
            ...
            status: isThreeSourceLayered ? 'PMS全酒店住宿 + OTA渠道' : overview?.scope === 'hotel' ? '全酒店' : '非全酒店',
            detail: isThreeSourceLayered ? '同店同日分层读取：PMS 不与 OTA 收入相加，OTA 不冒充全酒店收入。' : '只把已验证 OTA 口径作为首页判断输入，不包装成全酒店经营结论。',
            className: revenueAiStatusClass(isThreeSourceLayered ? 'ready' : 'warning')
        }, ...isThreeSourceLayered ? [ {
            key: 'pms',
            label: 'PMS状态',
            value: overview?.three_source_fact_layer?.sources?.dingdandao_pms?.data_status === 'readback_verified' ? '已保存并回读' : '--',
            status: revenueAiStatusLabel(overview?.three_source_fact_layer?.sources?.dingdandao_pms?.data_status === 'readback_verified' ? 'ready' : 'unverified'),
            detail: '仅作为全酒店住宿收入、可售间夜与 RevPAR 来源。',
            className: revenueAiStatusClass(overview?.three_source_fact_layer?.sources?.dingdandao_pms?.data_status === 'readback_verified' ? 'ready' : 'unverified')
        } ] : [], {
            key: 'ctrip',
            label: '携程状态',
            ...
            status: completeness?.status ? revenueAiStatusLabel(completeness.status) : readinessPercent === null ? '完整度未返回' : `${readinessPercent}%`,
    ...
    const buildRevenueAiSignalRows = ({overview = null} = {}) => {
        ...
        const definitions = [ {
            ...
            key: 'competitor_price_warning',
            label: '竞对价格倒挂预警'
        }, {
            key: 'booking_window_adr',
            label: '提前期房费结构'
        }, {
            key: 'channel_booking_window_month',
            label: '渠道预售窗口'
        }, {
            key: 'pricing_advice',
            label: '今日调价建议'
    ...
    const buildRevenueAiReviewQueueItems = (reviewQueue = {}) => {
        ...
        return sourceItems.slice(0, 5).map((item, index) => {
            ...
            const allowedEndpoints = actionEntry.allowed_endpoints && typeof actionEntry.allowed_endpoints === 'object' ? actionEntry.allowed_endpoints : {};
            const trustedDecision = item.trusted_decision && typeof item.trusted_decision === 'object' ? item.trusted_decision : {};
            ...
            const trustedFormula = trustedDecision.metric_formula && typeof trustedDecision.metric_formula === 'object' ? trustedDecision.metric_formula : {};
            const trustedQuality = trustedDecision.data_quality && typeof trustedDecision.data_quality === 'object' ? trustedDecision.data_quality : {};
            const trustedConfidence = trustedDecision.confidence && typeof trustedDecision.confidence === 'object' ? trustedDecision.confidence : {};
            const trustedAction = trustedDecision.recommended_action && typeof trustedDecision.recommended_action === 'object' ? trustedDecision.recommended_action : {};
            const trustedExpectedEffect = trustedDecision.expected_effect && typeof trustedDecision.expected_effect === 'object' ? trustedDecision.expected_effect : {};
            const trustedConfirmation = trustedDecision.human_confirmation && typeof trustedDecision.human_confirmation === 'object' ? trustedDecision.human_confirmation : {};
            ...
            const canTransferTrusted = trustedContractValid && trustedConfirmation.can_transfer_to_operation_task === true && item.can_transfer_to_operation_task === true;
            ...
            const canCreateExecutionIntent = canTransferTrusted && manualActions.includes('create_execution_intent') && !!allowedEndpoints.execution_intent;
            const actionButtons = [ canApprove ? {
                key: 'approve',
                label: '批准'
            } : null, canApproveWithChanges ? {
                key: 'approve_with_changes',
                label: '改价批准'
            } : null, canReject ? {
                key: 'reject',
                label: '拒绝'
            } : null, canCreateExecutionIntent ? {
            ...
            const priceParts = [ `当前 ${item.current_price_display || '--'}`, `建议 ${item.suggested_price_display || '--'}`, `最低保护 ${item.min_price_display || '--'}` ];
            const evidenceParts = [];
            ...
            const impactLine = revparImpactDisplay !== '--' ? `预计RevPAR影响 ${revparImpactDisplay}` : `预计RevPAR影响 -- ${revenueAiReasonText(revparImpactReason)}`;
            const reason = item.reason && item.reason !== '--' ? item.reason : item.missing_reason ? '关键价格字段不完整，需补齐后再审核。' : '建议原因待补充。';
            ...
            const sourceText = sourceRefs.length ? sourceRefs.join(' / ') : trustedSources.summary || '未提供已验证来源';
            const formulaDisplay = trustedFormula.status === 'calculable' ? trustedFormula.display || '--' : '不可计算';
            const formulaText = trustedFormula.expression ? `${trustedFormula.expression} = ${formulaDisplay}` : formulaDisplay;
            const gapRows = Array.isArray(trustedDecision.gaps) ? trustedDecision.gaps : [];
            const gapText = gapRows.length ? gapRows.map(gap => typeof gap === 'string' ? gap : gap?.message || gap?.code || '').filter(Boolean).join('；') : '无输入缺口';
            const expectedEffectText = trustedExpectedEffect.display || trustedExpectedEffect.summary || '待执行后按同口径回读验证';
            const trustedDecisionRows = [ {
                key: 'store',
                label: '门店',
                value: trustedStore.display || (item.hotel_id ? `门店 #${item.hotel_id}` : '门店未绑定')
            }, {
                key: 'platform',
                label: '平台',
                value: trustedPlatform.label || trustedPlatform.key || '未绑定'
            }, {
                key: 'date',
                label: '日期',
                value: trustedDate.value || item.suggestion_date || '未提供'
            }, {
                key: 'source',
                label: '来源',
                value: sourceText
                ...
                label: '指标公式',
                value: formulaText,
                status: trustedFormula.status || 'not_calculable'
            }, {
                key: 'quality',
                label: '数据质量',
                value: [ trustedQuality.label || trustedQuality.status || '未验证', trustedQuality.note || '' ].filter(Boolean).join(' · ')
            }, {
                key: 'confidence',
                label: '置信度',
                value: trustedConfidence.display || '不可计算'
            }, {
                key: 'gaps',
                label: '缺口',
                value: gapText
                ...
                key: 'action',
                label: '建议动作',
                value: trustedAction.summary || reason
            }, {
                key: 'effect',
                label: '预期效果',
                value: expectedEffectText
            } ];
            return {
                ...
                actionHelpText: canCreateExecutionIntent ? '生成本地待执行运营任务；仍需人工执行、留证和复盘' : canApprove || canApproveWithChanges || canReject ? '仅在可信输入通过后人工审核；不写 OTA' : trustedContractValid ? '当前可信输入仍有阻塞，先处理缺口' : '可信建议契约缺失，禁止批准',
    ...
    const buildRevenueAiResolutionPlanSummary = ({overview = null, action = null} = {}) => {
        const candidates = [ action?.ai_decision_resolution_plan, action?.ai_decision_review_contract?.resolution_plan, overview?.pricing_readiness?.ai_decision_resolution_plan, overview?.pricing_readiness?.ai_decision_review_contract?.resolution_plan ];
        const plan = candidates.find(item => item && typeof item === 'object' && Object.keys(item).length > 0) || {};
        const items = Array.isArray(plan.items) ? plan.items : [];
        if (Object.keys(plan).length === 0 && items.length === 0) return {
            visible: false,
            status: 'not_loaded',
    ...
    const buildRevenueAiPricingGenerationPreflightSummary = ({overview = null, action = null} = {}) => {
        const candidates = [ action?.pricing_generation_preflight, overview?.pricing_generation_preflight, overview?.pricing_readiness?.pricing_generation_preflight ];
        const preflight = candidates.find(item => item && typeof item === 'object' && Object.keys(item).length > 0) || {};
        if (Object.keys(preflight).length === 0) return {
            visible: false
        };
        ...
        const targetFilter = preflight.target_filter && typeof preflight.target_filter === 'object' ? preflight.target_filter : {};
        const rawRequiredInputs = Array.isArray(preflight.required_inputs) ? preflight.required_inputs : [];
        const requiredInputs = rawRequiredInputs.map(item => ({
        ...
        })).filter(item => item.code).slice(0, 4);
        const rawCandidateSkipReasons = Array.isArray(preflight.candidate_skip_reasons) ? preflight.candidate_skip_reasons.map(String).filter(Boolean) : [];
        const rawCandidateDataGaps = Array.isArray(preflight.candidate_data_gaps) ? preflight.candidate_data_gaps.map(String).filter(Boolean) : [];
        const rawHotelChecks = Array.isArray(preflight.hotel_checks) ? preflight.hotel_checks : [];
        const hotelChecks = rawHotelChecks.map((item, index) => {
            const skipReasons = Array.isArray(item?.skip_reasons) ? item.skip_reasons.map(String).filter(Boolean) : [];
            return {
        ...
        }).filter(item => item.hotelId > 0 || item.targetDateRows > 0 || item.roomTypeCount > 0 || item.skipReasons.length > 0).slice(0, 4);
        const targetHotelIds = Array.isArray(preflight.target_hotel_ids) ? preflight.target_hotel_ids.map(item => Number(item || 0)).filter(item => item > 0) : [];
        const detailParts = [ targetHotelIds.length ? `酒店 ${targetHotelIds.join(' / ')}` : '', `OTA行 ${Number(preflight.target_date_rows || 0)}`, `房型 ${Number(preflight.room_type_count || 0)}`, `候选 ${Number(preflight.create_candidate_count || 0)}`, `待审 ${Number(preflight.pending_suggestion_count || 0)}` ].filter(Boolean);
        return {
            ...
            className: revenueAiStatusClass(status),
            reasonText: legacyUnverifiedSkip ? revenueAiPricingGenerationReasonText('missing_pricing_inputs_skipped_by_operator_policy') : String(preflight.detail || '') || revenueAiPricingGenerationReasonText(reason),
    ...
    const buildRevenueAiPriceSuggestionGenerateResult = ({response = null, error = null} = {}) => {
        ...
        const message = isCreated ? `已生成 ${createdCount} 条待审建议并回读 ${readbackVerifiedCount} 条；仍需人工审核，不写 OTA。` : nextAction || reasonText || String(payload.message || '定价建议生成受阻');
        const targetFilter = data.target_filter && typeof data.target_filter === 'object' ? data.target_filter : {};
        const rawSkippedItems = Array.isArray(data.skipped) ? data.skipped : [];
        const skippedItems = rawSkippedItems.map((item, index) => {
            ...
            const priceChangeRate = rawPriceChangeRate === null || rawPriceChangeRate === void 0 || rawPriceChangeRate === '' ? null : Number(rawPriceChangeRate);
            const rawPrimarySignalCount = item?.primary_signal_count;
            const primarySignalCount = rawPrimarySignalCount === null || rawPrimarySignalCount === void 0 || rawPrimarySignalCount === '' ? null : Number(rawPrimarySignalCount);
            return {
        ...
        return {
            ...
            targetHotelIds: Array.isArray(data.target_hotel_ids) ? data.target_hotel_ids.map(item => Number(item || 0)).filter(item => item > 0) : [],
            targetFilter,
            dateRange: data.date_range && typeof data.date_range === 'object' ? data.date_range : {},
            createdCount,
            skippedCount,
            createdRowIds: Array.isArray(data.created_row_ids) ? data.created_row_ids.map(item => Number(item || 0)).filter(item => item > 0) : [],
    ...
    const buildRevenueAiActionRows = ({overview = null, overviewError = ''} = {}) => {
        ...
        return rows.map(action => {
            const reviewQueue = action.review_queue && typeof action.review_queue === 'object' ? action.review_queue : overview?.review_queue || {};
            const reviewQueueTargetFilter = reviewQueue.target_filter && typeof reviewQueue.target_filter === 'object' ? reviewQueue.target_filter : {};
            ...
            const decisionBasis = action.decision_basis_summary && typeof action.decision_basis_summary === 'object' ? action.decision_basis_summary : {};
            const decisionBasisStatus = decisionBasis.status || '';
            const decisionBasisEntries = Array.isArray(decisionBasis.items) ? decisionBasis.items.map((item, index) => ({
                item,
                index
            })).sort((left, right) => {
            ...
            const visibleBlockedDecisionBasisCount = visibleDecisionBasisEntries.filter(({item}) => String(item.status || '').trim() !== 'ok').length;
            ...
            const decisionBasisItems = visibleDecisionBasisEntries.map(({item}) => ({
            ...
            const resolutionPlanSummary = buildRevenueAiResolutionPlanSummary({
                overview,
                action
            });
            const pricingGenerationPreflightSummary = buildRevenueAiPricingGenerationPreflightSummary({
                overview,
                action
            });
            return {
                ...
                nextActions: Array.isArray(action.next_actions) ? action.next_actions.filter(item => String(item || '').trim()).slice(0, 4) : [],
    ...
    const buildRevenueAiEvidenceWorkbenchRows = ({overview = null, overviewError = ''} = {}) => {
        ...
        const reviewQueue = primaryAction.review_queue && typeof primaryAction.review_queue === 'object' ? primaryAction.review_queue : overview?.review_queue || {};
        const aiToOperation = primaryAction.ai_to_operation_handoff && typeof primaryAction.ai_to_operation_handoff === 'object' ? primaryAction.ai_to_operation_handoff : overview?.ai_to_operation_handoff || overview?.pricing_readiness?.ai_to_operation_handoff || {};
        const operationPacket = aiToOperation.operation_intake_packet && typeof aiToOperation.operation_intake_packet === 'object' ? aiToOperation.operation_intake_packet : {};
        const operationPreflight = operationPacket.operation_intake_preflight_contract && typeof operationPacket.operation_intake_preflight_contract === 'object' ? operationPacket.operation_intake_preflight_contract : primaryAction.operation_intake_preflight_contract || {};
        ...
        return [ {
            ...
            statusLabel: revenueAiStatusLabel(operationStatus),
            className: revenueAiStatusClass(operationStatus),
            detailText: [ operationPacket.status ? `packet=${operationPacket.status}` : '', operationPreflight.status ? `preflight=${operationPreflight.status}` : '', operationPacket.candidate_blocked_reason || '' ].filter(Boolean).join(' / ') || revenueAiReasonText('operation_intake_not_approved'),
    ...
    const revenueAiExecutionTargetKind = (actionKey, explicitKind = '') => {
        ...
        if (kind) return kind;
        if ([ 'approve_intent', 'resolve_blocker' ].includes(String(actionKey || ''))) return 'intent';
        if ([ 'record_execution', 'record_evidence', 'review_effect', 'review_failure' ].includes(String(actionKey || ''))) return 'task';
        return '';
    };
    ...
    const buildRevenueAiExecutionRows = ({overview = null, overviewError = ''} = {}) => {
        ...
        return items.slice(0, 5).map((item, index) => {
            ...
            const dateText = item.date_start && item.date_end && item.date_start !== item.date_end ? `${item.date_start}~${item.date_end}` : item.date_start || item.date_end || '--';
    ...
    const revenueAiEffectInputDetail = (item = {}) => {
        const parts = [ item.review_status ? `复盘 ${item.review_status}` : '', item.evidence_count !== void 0 ? `证据 ${Number(item.evidence_count || 0)}` : '', item.evidence_summary ? String(item.evidence_summary) : '', item.roi_display && item.roi_display !== '--' ? `ROI ${item.roi_display}` : '' ].filter(Boolean);
    ...
    const revenueAiExecutionResolvedActionKey = (row = {}) => {
        ...
        if (inputActionKey) return inputActionKey;
    ...
    const revenueAiExecutionTaskActionItem = (row = {}) => {
        ...
        const currentValue = raw.current_value && typeof raw.current_value === 'object' ? raw.current_value : recommendation.current_value && typeof recommendation.current_value === 'object' ? recommendation.current_value : {};
        const targetValue = raw.target_value && typeof raw.target_value === 'object' ? raw.target_value : recommendation.target_value && typeof recommendation.target_value === 'object' ? recommendation.target_value : {};
        return {
            execution: {
                task_id: taskId
            },
    ...
    const resolveRevenueAiExecutionAction = ({row = {}, fallbackHotelId = 0} = {}) => {
        const navigation = resolveRevenueAiExecutionNavigation({
            row,
            fallbackHotelId
        });
        ...
        if (!navigation.targetPage) return {
            ...base,
            action: 'missing_entry',
            message: '该执行记录暂未配置运营执行入口',
            level: 'warning'
        };
        if (navigation.targetPage !== 'ops-track') return {
            ...base,
            action: 'open_page'
        };
        if (taskId > 0 && (nextActionKey === 'record_execution_evidence' || nextActionKey === 'record_evidence')) return {
            ...base,
            action: 'record_execution_evidence',
            confirmText: '确认在 Revenue AI 首页补充执行证据？该动作只记录本地人工执行证据，不写入携程/美团价格。',
            reloadOverview: true
        };
        if (taskId > 0 && nextActionKey === 'record_roi_evidence') return {
            ...base,
            action: 'record_roi_evidence',
            confirmText: '该记录缺少收入/ROI证据。确认先补录执行前后收入或成本证据？该动作只写入本地复盘证据，不写入携程/美团价格。',
            reloadOverview: true
        };
        if (taskId > 0 && nextActionKey === 'record_effect_review') return {
            ...base,
            action: 'record_effect_review',
            reloadOverview: true
        };
        if (taskId > 0 && nextActionKey === 'review_effect') {
            if (revenueAiExecutionNeedsRoiEvidence(row)) return {
                ...base,
                action: 'record_roi_evidence',
                confirmText: '该记录缺少收入/ROI证据。确认先补录执行前后收入或成本证据？该动作只写入本地复盘证据，不写入携程/美团价格。',
                reloadOverview: true
    ...
    const revenueAiReviewEndpoint = (item = {}, action = '') => {
        ...
        return normalizedAction === 'execution_intent' ? normalizeRevenueAiApiPath(endpoints.execution_intent || item.allowedEndpoint) : normalizeRevenueAiApiPath(endpoints.review || item.allowedEndpoint);
    };
    const resolveRevenueAiReviewActionDraft = ({item = {}, action = ''} = {}) => {
        ...
        if (item.autoWriteOta === true) return {
            ok: false,
        ...
        if (!endpoint || !endpoint.startsWith('/revenue-ai/price-suggestions/')) return {
            ok: false,
    ...
    const validateRevenueAiApprovedPrice = (inputValue = '', item = {}) => {
        ...
        if (!Number.isFinite(parsedPrice) || parsedPrice <= 0) return {
            ok: false,
            approvedPrice: null,
            message: '修改后批准价必须是大于 0 的数字'
        };
        const minPrice = Number(item.minPrice || item.min_price || 0);
        if (Number.isFinite(minPrice) && minPrice > 0 && parsedPrice < minPrice) return {
            ok: false,
            approvedPrice: null,
            message: `修改后批准价低于最低保护价 ${minPrice}`
        };
        const maxPrice = Number(item.maxPrice || item.max_price || 0);
        if (Number.isFinite(maxPrice) && maxPrice > 0 && parsedPrice > maxPrice) return {
            ok: false,
            approvedPrice: null,
            message: `修改后批准价高于最高限制价 ${maxPrice}`
        };
        return {
            ok: true,
            approvedPrice: Math.round(parsedPrice * 100) / 100,
            message: ''
        };
    };
    const buildRevenueAiReviewConfirmText = ({action = '', actionText = '', approvedPrice = null} = {}) => {
        const normalizedAction = String(action || '').trim();
        if (normalizedAction === 'execution_intent') return '确认转为本地待执行运营任务？该动作不会写入携程/美团价格，仍需人工执行、留证和复盘。';
        if (normalizedAction === 'approve_with_changes') return `确认以 ${approvedPrice} 元修改后批准？该动作只更新本地审核状态，不写入携程/美团价格。`;
    ...
    const buildRevenueAiReviewRequestBody = ({action = '', item = {}, approvedPrice = null, reviewRemark = ''} = {}) => {
        const normalizedAction = String(action || '').trim();
        if (normalizedAction === 'execution_intent') return {
            source: 'revenue_ai_homepage',
            expected_metric: 'orders',
            approve_to_task: true
        };
        if (normalizedAction === 'approve_with_changes') return {
            action: normalizedAction,
    ...
    const aiDailyReportEvidenceTarget = (item = {}) => {
        ...
        if (/execution|execute|action_item|operation|ops|执行|闭环/.test(text)) return {
            page: 'ops-track',
            tab: '',
            label: '查看执行闭环',
            sourceRef: sourceRef || code || 'execution_flow'
        };
        if (/platform|resource_catalog|collection_status|data_source|sync|profile|authorization|账号|授权|平台/.test(text)) return {
            page: 'online-data',
            tab: 'platform-sources',
            label: '查看平台数据源',
            sourceRef: sourceRef || code || 'platform_sources'
        };
        if (/table_missing|missing_table|init|schema|初始化/.test(text)) return {
            page: 'online-data',
            tab: 'data-health',
            label: '查看初始化状态',
    ...
    const aiDailyReportActionIsInvestigationOnly = action => {
        ...
        return action.can_create_execution_intent === false && String(action.action_type || '') === 'manual_review' && /fallback|investigation-only|investigation item|review daily operating signal|调查项/i.test(text);
    };
    const aiDailyReportActionExecutionReady = action => action?.can_create_execution_intent === true && action?.decision_quality?.contract_version === 'ai_recommendation_quality.v2' && action?.decision_quality?.execution_ready === true;
    const aiDailyReportActionBlockedText = action => {
        if (!action) return '';
        if (aiDailyReportActionIsInvestigationOnly(action)) return '调查项不可转执行单，仅用于查看证据和判断是否需要进一步分析。';
        if (!aiDailyReportActionExecutionReady(action) && !action.execution_intent_id) return action.blocked_reason || action.action_readiness?.notice || action.action_readiness?.next_action || '该建议受数据缺口阻断，不能直接转执行单。';
        ...
        if ([ 'blocked_by_data_gap', 'blocked', 'rejected', 'failed' ].includes(stage)) return readiness.notice || readiness.next_action || '当前阶段不可转执行，需先处理阻断原因。';
        return '';
    };
    const aiDailyReportActionStatusText = action => {
        ...
        if (action?.execution_intent_id) return action.execution_blocked_reason ? '已生成，待补齐' : '已生成执行单';
    ...
    const summarizeAiDailyReportBlockingRows = (rows = []) => {
        ...
        if (readinessCount + actionCount > 0 || dataHealthCount > investigationCount) gateParts.push(`数据健康门禁 ${Math.max(0, dataHealthCount - investigationCount)}`);
    ...
    const buildAiDailyReportEvidenceRows = ({sourceRefs = [], dataGaps = [], actions = []} = {}) => {
        ...
        (Array.isArray(actions) ? actions : []).forEach((action, index) => {
            ...
            rows.push({
                ...
                ref: refs,
                className: isInvestigation ? 'bg-slate-100 text-slate-700' : aiDailyReportActionBlockedText(action) ? 'bg-amber-50 text-amber-700' : 'bg-green-50 text-green-700'
            });
        });
        return rows.slice(0, 12);
    };
    const buildAiDailyFactGate = ({hotelId = '', targetDate = '', collectionStatus = null, profileStatus = null, errors = []} = {}) => {
        ...
        const rawPlatforms = collection?.platforms && typeof collection.platforms === 'object' ? collection.platforms : {};
        ...
        const platformKeys = Array.from(new Set([ 'ctrip', 'meituan', ...collectionRows.map(row => String(row?.platform || '').toLowerCase()), ...profileRows.map(row => String(row?.platform || '').toLowerCase()) ].filter(Boolean)));
        const loginTextMap = {
            logged_in: '登录态已验证',
            waiting_login: '登录待验证',
        ...
        const platformRows = platformKeys.map(platform => {
            ...
            const applicable = configuredCount > 0 || Number(profileDetail.dataSourceId || 0) > 0 || profileDetail.profileExists === true || targetDateRows > 0 || Number(row.storedRowCount || 0) > 0 || ![ '', 'unconfigured', 'unverified' ].includes(loginStatus) || ![ '', 'not_collected', 'not_loaded' ].includes(collectionCode);
            ...
            if (applicable && !loginReady) nextAction = profile.next_action || profileDetail.nextAction || '先验证平台 Profile 登录状态'; else if (applicable && !targetDateReady) nextAction = `补齐 ${targetDate || '目标日'} OTA 入库事实`; else if (applicable && !fieldReady) nextAction = '补齐字段事实、source path、metric key 与 verifier 证据'; else if (ready) nextAction = 'OTA 事实已就绪，下游仍需独立验证';
            return {
                platform,
                ...
                fieldFactsMissing,
                fieldFactStatus,
                fieldText: fieldReady ? `字段事实已闭合 ${fieldFactsReady}` : fieldFactsMissing > 0 ? `字段缺口 ${fieldFactsMissing}` : `字段状态 ${fieldFactStatus || '未核验'}`,
                blockerCodes: blockers,
                nextAction,
                statusText: !applicable ? '未配置，不计入分母' : ready ? '事实就绪' : '事实有缺口',
                statusClass: !applicable ? 'bg-slate-100 text-slate-600 border-slate-200' : ready ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'
            };
        });
        ...
        if (!String(hotelId || '').trim()) status = 'not_selected'; else if (safeErrors.length > 0) status = 'unverified'; else if (!collection) status = 'not_loaded'; else if (configuredCount === 0) status = 'not_configured'; else if (readyCount === configuredCount) status = 'ready'; else if (readyCount > 0) status = 'partial'; else status = 'blocked';
        const statusMeta = {
        ...
        const chain = [ {
            key: 'ota',
            label: 'OTA事实',
            status: otaChainStatus,
            text: statusMeta[0]
        }, {
            key: 'revenue',
            label: '收益分析',
            status: downstreamStatus,
            text: status === 'ready' ? '待独立验证' : '等待OTA事实'
        }, {
            key: 'ai',
            label: 'AI决策',
            status: downstreamStatus,
            ...
            ...item,
            className: chainClass(item.status)
        }));
        return {
    ...
    const buildRevenueAiExecutionIntentOpenRow = ({payload = {}, item = {}} = {}) => {
        ...
        return {
            ...
            actionLabel: taskId > 0 ? '查看待执行运营任务' : data.execution_intent_existing ? '查看执行意图' : '审批执行意图',
    ...
    const resolveRevenueAiReviewNavigation = ({item = {}, isSuperAdmin = false} = {}) => {
        ...
        if (entry.requires_super_admin === true && isSuperAdmin !== true) return {
            action: 'blocked',
            message: '当前账号无权进入超级管理员审核页；Revenue AI 首页不直接批准、拒绝或写 OTA。',
            level: 'warning'
        };
        if (String(entry.target_page || '') !== 'agent-center') return {
            action: 'gap',
            gapTarget: {
                target_tab: 'data-health'
            }
        ...
        const dateText = filter.date ? String(filter.date) : item.suggestionDate && item.suggestionDate !== '--' ? String(item.suggestionDate) : '';
        return {
            action: 'agent-center',
    ...
    const buildRevenueAiReviewNavigationState = (navigation = {}) => {
        if (!navigation || navigation.action !== 'agent-center') return {
            shouldOpen: false,
            hotelId: '',
            date: '',
            status: 0,
    ...
    const competitorMicroscopeName = (row = {}) => {
        ...
        return String(row.competitor_name || row.competitor_hotel?.name || metadata.competitor_name || (Number(row.competitor_hotel_id || 0) > 0 ? `竞对 #${Number(row.competitor_hotel_id)}` : '未知竞对')).trim();
    };
    const competitorMicroscopePlatformKey = (row = {}) => {
        const metadata = competitorMicroscopeObject(row.competitor_data);
        const raw = String(row.ota_platform ?? row.platform ?? metadata.ota_platform ?? metadata.platform_key ?? metadata.platform ?? '').trim().toLowerCase();
        const aliases = {
            1: 'ctrip',
            ctrip: 'ctrip',
            trip: 'ctrip',
    ...
    const competitorMicroscopeSourceStatus = (row = {}) => {
        ...
        const verified = !operatorProvided && sourceMethod !== '' && sourceRef !== '' && capturedAt !== '' && readbackVerified && [ 'verified', 'available', 'normal', 'ok', 'valid' ].includes(validationStatus);
    ...
    const competitorMicroscopeRow = (row = {}, roomTypeName = '') => {
        ...
        const gapPercent = gap === null ? null : explicitGapPercent !== null ? explicitGapPercent : competitorPrice > 0 ? Number((gap / competitorPrice * 100).toFixed(2)) : null;
    ...
    const competitorMicroscopeMeituanRankDefinitions = [ {
        key: 'P_RZ',
        label: '入住榜',
        metrics: [ 'roomNights', 'roomRevenue' ]
    }, {
        key: 'P_XS',
        label: '销售榜',
        metrics: [ 'salesRoomNights', 'sales' ]
    }, {
        key: 'P_LL',
        label: '流量榜',
        metrics: [ 'exposure', 'views' ]
    }, {
        key: 'P_ZH',
        label: '转化榜',
        metrics: [ 'viewConversion', 'payConversion' ]
    } ];
    ...
    const competitorMicroscopeMeituanRankEntry = (row, definition, preferredRange = null, preferredMetric = null) => {
        ...
        const candidates = entries.filter(entry => String(entry?.rankType || '').trim().toUpperCase() === definition.key && (metric === null || String(entry?.metric || '').trim() === metric));
        return (preferred === null ? candidates : candidates.filter(entry => competitorMicroscopeMeituanRankRangeKey(entry?.dateRange) === preferred)).slice().sort((left, right) => {
            ...
            const rangeCompare = competitorMicroscopeMeituanRankRangePriority(leftRange) - competitorMicroscopeMeituanRankRangePriority(rightRange);
    ...
    const competitorMicroscopeMeituanMetricText = entry => {
        ...
        if ([ 'roomRevenue', 'sales' ].includes(metric)) return `¥${value.toLocaleString('zh-CN', {
            maximumFractionDigits: 2
        })}`;
    ...
    const competitorMicroscopeDateOffset = (date, offsetDays) => {
        ...
        return new Date(Date.UTC(year, month - 1, day) + offsetDays * 864e5).toISOString().slice(0, 10);
    };
    const normalizeMeituanCompetitionCircle = (payload = {}, expectedHotelId = '', expectedDate = '', requestedRange = 'yesterday') => {
        ...
        if (missingStatuses.includes(dataStatus)) {
            ...
            if (responseHotelId !== String(expectedHotelId).trim() || responseDate !== String(expectedDate).trim()) {
                circle = {
                    ...
                    message: '美团竞争圈空结果与当前酒店或日期不一致'
                };
                return {
                    payload: circle,
                    accepted: false,
                    status: 'scope_mismatch',
                    errorMessage: circle.message
                };
            }
            return {
                payload: circle,
        ...
        if (usableStatuses.includes(dataStatus)) {
            ...
            if (circle.comparison) {
                ...
                if (comparisonHotelId !== String(expectedHotelId).trim() || String(circle.comparison.data_date || '').trim() !== previousDate) circle = {
                    ...circle,
                    comparison: null,
                    comparison_scope_mismatch: true
                };
            }
            return {
                payload: circle,
    ...
    const competitorMicroscopeMeituanContext = (analysis = {}) => {
        ...
        const scopeMismatch = usable && (expectedHotelId !== '' && responseHotelId !== expectedHotelId || expectedDate !== '' && responseDate !== expectedDate);
        ...
        const selfRow = rows.find(row => row?.isSelf === true) || (targetPoiId !== '' ? rows.find(row => String(row?.poiId ?? row?.poi_id ?? '').trim() === targetPoiId) : null) || null;
        ...
        const options = competitors.map(row => {
            const rankEntries = competitorMicroscopeMeituanRankDefinitions.map(definition => competitorMicroscopeMeituanRankEntry(row, definition, requestedRange)).filter(Boolean);
        ...
        }).sort((left, right) => Number(left.bestRank ?? 999) - Number(right.bestRank ?? 999) || right.sampleCount - left.sampleCount || left.name.localeCompare(right.name, 'zh-CN'));
        return {
            circle,
            status,
            label,
    ...
    const competitorMicroscopeMeituanDetail = (analysis, option, context) => {
        ...
        const comparisonHotelId = String(circle?.comparison?.system_hotel_id ?? '').trim();
        const comparisonScopeMismatch = circle.comparison_scope_mismatch === true || circle.comparison && (comparisonHotelId !== expectedHotelId || comparisonDate !== competitorMicroscopeDateOffset(expectedDate, -1));
        const comparisonRows = !comparisonScopeMismatch && Array.isArray(circle?.comparison?.display_hotels) ? circle.comparison.display_hotels : [];
        ...
        const rows = competitorMicroscopeMeituanRankDefinitions.map(definition => {
            ...
            const rankChange = competitorRank !== null && previousCompetitorRank !== null ? previousCompetitorRank - competitorRank : null;
            const gapText = gap === null ? '差距未知' : gap > 0 ? `竞对领先${gap}名` : gap < 0 ? `本店领先${Math.abs(gap)}名` : '名次持平';
            const trendText = rankChange === null ? '前日未返回' : rankChange > 0 ? `较前日上升${rankChange}名` : rankChange < 0 ? `较前日下降${Math.abs(rankChange)}名` : '较前日持平';
        ...
        if (competitorRow && Object.keys(competitorMicroscopeObject(competitorRow.metricDerived)).length > 0) dataGaps.push('meituan_derived_metrics_present');
    ...
    const buildCompetitorMicroscope = (analysis = {}, requestedKey = '') => {
        ...
        Object.entries(matrix).forEach(([roomTypeName, competitorRows]) => {
            ...
            entries.forEach(row => {
                ...
                if (expectedHotelId && rowHotelId !== expectedHotelId || expectedScopeDate && rowDate !== expectedScopeDate) {
        ...
        const priceOptions = Array.from(grouped.entries()).map(([key, groupRows]) => {
            const gapValues = groupRows.map(row => competitorMicroscopeNumber(row.price_gap_percent)).filter(value => value !== null);
            const sourceStatuses = new Set(groupRows.map(row => row.source_status));
            const sourceStatus = sourceStatuses.size === 1 ? Array.from(sourceStatuses)[0] : 'partial';
            return {
                key,
        ...
        const ctripStatus = sourceErrors.ctrip ? 'error' : priceScopeRejectedCount > 0 ? priceOptions.length ? 'partial' : 'scope_mismatch' : priceOptions.length ? 'ready' : 'missing';
        ...
        const selectedKey = options.some(option => option.key === requestedKey) ? requestedKey : options[0]?.key || '';
        ...
        const selectedRows = (grouped.get(selectedKey) || []).slice().sort((left, right) => (right.price_gap_percent === null ? -1 : Math.abs(Number(right.price_gap_percent))) - (left.price_gap_percent === null ? -1 : Math.abs(Number(left.price_gap_percent))));
        const comparableSelectedRows = selectedRows.filter(row => competitorMicroscopePrice(row?.our_price) !== null && competitorMicroscopePrice(row?.competitor_price) !== null);
        ...
        const priceGap = avgOurPrice !== null && avgCompetitorPrice !== null ? Number((avgOurPrice - avgCompetitorPrice).toFixed(2)) : null;
        const priceGapPercent = priceGap !== null && avgCompetitorPrice > 0 ? Number((priceGap / avgCompetitorPrice * 100).toFixed(2)) : null;
        const trends = analysis?.trends && typeof analysis.trends === 'object' ? analysis.trends : {};
        const rawTrendRows = Array.isArray(trends[String(selectedOption.competitorId)]) ? trends[String(selectedOption.competitorId)] : Array.isArray(trends[selectedOption.competitorId]) ? trends[selectedOption.competitorId] : [];
        ...
        if (/^\d{4}-\d{2}-\d{2}$/.test(responseDate)) {
            ...
            for (let offset = 6; offset >= 0; offset -= 1) expectedTrendDates.push(new Date(endTimestamp - offset * 864e5).toISOString().slice(0, 10));
        } else expectedTrendDates.push(...Array.from(trendGroups.keys()).sort((left, right) => left.localeCompare(right)));
        const populatedTrendKeySets = expectedTrendDates.map(date => trendGroups.get(date) || []).filter(dayRows => dayRows.length > 0).map(dayRows => new Set(dayRows.map(row => row.comparison_key)));
        const commonTrendKeys = populatedTrendKeySets.length ? new Set(Array.from(populatedTrendKeySets[0]).filter(key => populatedTrendKeySets.every(keys => keys.has(key)))) : new Set;
        const trendEvidenceRows = expectedTrendDates.flatMap(date => (trendGroups.get(date) || []).filter(row => commonTrendKeys.has(row.comparison_key)));
        ...
        const trendChange = trendCoverageDays === 7 && firstTrendGap !== null && lastTrendGap !== null ? Number((lastTrendGap - firstTrendGap).toFixed(2)) : null;
        ...
        if (trendCoverageDays === 0) dataGaps.push('seven_day_trend_missing'); else if (trendCoverageDays < 7) dataGaps.push('seven_day_trend_incomplete');
        if (commonTrendKeys.size > 0 && commonTrendKeys.size < selectedComparisonKeys.size) dataGaps.push('trend_room_mix_reduced');
        if (trendEvidenceRows.some(row => competitorMicroscopeSourceStatus(row) !== 'verified')) dataGaps.push('trend_source_unverified');
    ...
    const resolveRevenueCockpitScope = ({scopePayload = null, requestedPlatform = '', requestedDate = '', resetDate = false, today = ''} = {}) => {
        const payload = scopePayload && typeof scopePayload === 'object' ? scopePayload : {};
        const allowedPlatforms = new Set([ 'all_ota', 'ctrip', 'meituan' ]);
        const platformRows = (Array.isArray(payload.platforms) ? payload.platforms : []).filter(row => row && allowedPlatforms.has(String(row.platform || '').toLowerCase())).map(row => {
            const platform = String(row.platform || '').toLowerCase();
            const availableDates = Array.from(new Set((Array.isArray(row.available_dates) ? row.available_dates : []).map(revenueCockpitIsoDate).filter(Boolean))).sort((left, right) => right.localeCompare(left));
        ...
        const selectedRow = platformRows.find(row => row.platform === requestedPlatformKey) || platformRows.find(row => row.platform === recommendedPlatform) || platformRows[0] || null;
        ...
        const selectedDate = !resetDate && selectedRow.availableDates.includes(requestedDateKey) ? requestedDateKey : selectedRow.latestVerifiedDate;
        ...
        const sameWeekdayDate = selectedIndex >= 0 ? selectedRow.availableDates.slice(selectedIndex + 1).find(candidate => {
            const parts = candidate.split('-').map(Number);
            return parts.length === 3 && new Date(Date.UTC(parts[0], parts[1] - 1, parts[2])).getUTCDay() === selectedWeekday;
        }) || '' : '';
        const distance = revenueCockpitDateDistance(selectedDate, today);
        const isToday = distance === 0;
        const distanceText = distance === null ? '与今天的差异待确认' : distance === 0 ? '就是今天' : distance > 0 ? `比今天早 ${distance} 天` : `比今天晚 ${Math.abs(distance)} 天`;
        return {
            status: 'ready',
            strictGate: String(payload?.boundary?.strict_gate || 'history_success+validation_verified+readback_verified'),
    ...
    const revenueCockpitMetricReady = status => [ 'readback_verified', 'derived_verified', 'verified' ].includes(String(status || '').toLowerCase());
    ...
    const revenueCockpitStatusClass = status => {
        ...
        if ([ 'readback_verified', 'derived_verified', 'verified', 'ready', 'ok' ].includes(key)) return 'border-emerald-200 bg-emerald-50 text-emerald-700';
        if ([ 'read_failed', 'failed', 'error', 'blocked' ].includes(key)) return 'border-rose-200 bg-rose-50 text-rose-700';
    ...
    const revenueCockpitSourceEvidence = (source = {}, sourceKey, businessDate, strictEvidence = {}, metricKey = '') => {
        const meta = revenueCockpitSourceMeta(sourceKey);
        const provenance = source?.source && typeof source.source === 'object' ? source.source : {};
        const rowIds = Array.isArray(provenance.row_ids) ? provenance.row_ids.filter(id => Number(id) > 0) : Number(provenance.record_id || 0) > 0 ? [ Number(provenance.record_id) ] : [];
        const traceIds = Array.isArray(provenance.source_trace_ids) ? provenance.source_trace_ids.filter(Boolean) : [];
        const strictMetric = metricKey && strictEvidence?.metrics?.[metricKey] && typeof strictEvidence.metrics[metricKey] === 'object' ? strictEvidence.metrics[metricKey] : null;
        const strictAcceptedIds = Array.isArray(strictMetric?.accepted_row_ids) ? strictMetric.accepted_row_ids.filter(id => Number(id) > 0) : Array.isArray(strictEvidence?.accepted_row_ids) ? strictEvidence.accepted_row_ids.filter(id => Number(id) > 0) : [];
        const strictRejectedIds = Array.isArray(strictMetric?.rejected_row_ids) ? strictMetric.rejected_row_ids.filter(id => Number(id) > 0) : Array.isArray(strictEvidence?.rejected_row_ids) ? strictEvidence.rejected_row_ids.filter(id => Number(id) > 0) : [];
        return [ `来源：${meta.label} · ${String(provenance.table || meta.expectedTable || '来源表待确认')}`, `业务日期：${String(source.business_date || provenance.data_date || businessDate || '待确认')} · 实际日期：${String(source.actual_business_date || provenance.data_date || '待确认')}`, `保存记录：${rowIds.length ? rowIds.map(id => `#${id}`).join('、') : '未返回可追溯记录ID'}`, `严格回读：${revenueCockpitStatusText(provenance.readback_status || source.data_status)}`, `驾驶舱严格事实闸门：${strictAcceptedIds.length ? strictAcceptedIds.map(id => `#${id}`).join('、') : '未命中可用记录'}${strictRejectedIds.length ? `；拒绝 ${strictRejectedIds.map(id => `#${id}`).join('、')}` : ''}`, `来源追踪：${traceIds.length ? `${traceIds.length} 条 trace` : '未返回 trace'}`, `口径：${meta.scopeLabel}` ];
    };
    const revenueCockpitMetricCard = ({source = {}, sourceKey, metricKey, label, unit, businessDate, strictEvidence = {}}) => {
        const meta = revenueCockpitSourceMeta(sourceKey);
        ...
        const factStatus = statuses[metricKey] && typeof statuses[metricKey] === 'object' ? statuses[metricKey] : {};
        ...
        const strictMetric = strictEvidence?.metrics?.[metricKey] && typeof strictEvidence.metrics[metricKey] === 'object' ? strictEvidence.metrics[metricKey] : {};
        ...
        const displayStatus = ready ? String(factStatus.status || 'verified') : strictMismatch ? 'not_verified' : String(hasFactStatus ? factStatus.status : 'missing');
        const reasonCode = ready ? '' : strictMismatch ? 'cockpit_strict_metric_readback_missing' : String(factStatus.reason || `${sourceKey}_${metricKey}_missing`);
        const evidenceLines = revenueCockpitSourceEvidence(source, sourceKey, businessDate, strictEvidence, metricKey);
        ...
        return {
            ...
            reasonCode,
            reasonText: ready ? '该指标命中同酒店、同来源、同业务日的验证状态。' : strictMismatch ? '指标来源行未通过 history_status=success、validation_status=verified、readback_verified=1 的驾驶舱严格事实闸门，数值保持为空。' : revenueAiReasonText(reasonCode),
            evidenceLines
        };
    };
    const revenueCockpitSourceCard = (source = {}, sourceKey, businessDate, strictEvidence = {}) => {
        ...
        return {
            ...
            reasonCode: ready ? '' : status === 'readback_verified' ? 'cockpit_strict_source_readback_missing' : `${sourceKey}_not_readback_verified`,
            reasonText: ready ? '来源身份、业务日期、保存记录和严格回读均已通过。' : status === 'readback_verified' ? '来源总览虽标记已回读，但承载当前指标的保存行未通过驾驶舱严格事实闸门。' : revenueAiReasonText(`${sourceKey}_not_readback_verified`),
            evidenceLines: revenueCockpitSourceEvidence(source, sourceKey, businessDate, strictEvidence)
        };
    };
    const revenueCockpitComparisonCard = (current, previous, previousDate, options = {}) => {
        ...
        const comparable = current?.value !== null && previous?.value !== null && current?.sourceKey === previous?.sourceKey && current?.unit === previous?.unit;
        const delta = comparable ? Number((current.value - previous.value).toFixed(2)) : null;
        const ratio = comparable && previous.value !== 0 ? Number((delta / Math.abs(previous.value) * 100).toFixed(2)) : null;
        const display = !comparable ? '—' : `${delta > 0 ? '+' : ''}${revenueCockpitDisplayValue(delta, current.unit)}${ratio === null ? '（基期为 0）' : `（${ratio > 0 ? '+' : ''}${ratio.toFixed(2)}%）`}`;
        ...
        return {
            ...
            reasonText: comparable ? `只比较 ${current.sourceLabel} 的同酒店、同平台、同一指标与同一单位，没有跨来源合并。` : `当前值或${basisLabel}的同来源、同单位指标缺失，变化保持为空。`,
            comparisonBasis: basisKey,
            baselineDate: previousDate || '',
            formula: comparable ? 'current_value - baseline_value' : '',
            causalityClaimed: false,
            evidenceLines: [ `当前：${current?.businessDate || '日期待确认'} · ${current?.display || '—'} · ${current?.statusLabel || '状态待确认'}`, `基期：${previousDate || '无'} · ${previous?.display || '—'} · ${previous?.statusLabel || '状态待确认'}`, `比较基准：${basisLabel}`, `比较规则：同酒店、同平台、同来源、同指标、同单位；禁止跨来源或跨单位静默合并。`, '证据分类：当前值与基期值为平台事实；差值和百分比为公式计算；不形成因果结论。' ]
        };
    };
    const revenueCockpitTextCard = ({key, kind, label, display, sourceKey = 'cockpit_rule', businessDate = '', status = 'partial', reasonCode = '', reasonText = '', evidenceLines = []}) => {
        const meta = revenueCockpitSourceMeta(sourceKey);
        return {
            key,
            kind,
            ...
            evidenceLines: evidenceLines.length ? evidenceLines : [ `来源：${meta.label}`, `业务日期：${businessDate || '待确认'}`, '边界：只生成解释或待审批入口，不自动执行。' ]
        };
    };
    ...
    const revenueCockpitMetricFromCards = (cards, platform, metricKey) => (Array.isArray(cards) ? cards : []).find(card => String(card?.sourceKey || '') === `${platform}_ota` && String(card?.key || '').split(':').slice(-1)[0] === metricKey) || null;
    const revenueCockpitBaseline = ({currentCards = [], previousCards = [], sameWeekdayCards = [], platform = '', metricKey = '', previousDate = '', sameWeekdayDate = ''} = {}) => {
        ...
        const deltaPercent = comparable && baseline.card.value !== 0 ? Number((delta / Math.abs(baseline.card.value) * 100).toFixed(2)) : null;
        return {
            current,
    ...
    const revenueCockpitSignalForPlatform = ({definition, platform, currentCards, previousCards, sameWeekdayCards, previousDate, sameWeekdayDate, sourceReady, businessDate}) => {
        ...
        let evidenceSupport = sourceReady ? '当前平台来源已通过严格回读，但该机会仍缺必要指标或比较条件。' : '当前平台来源未通过严格回读，不能形成经营机会判断。';
        let missingEvidence = [];
        let reasonCodes = [];
        let formula = '';
        ...
        if (definition.key === 'traffic_entry_shortage') {
            ...
            evidenceSupport = sourceReady ? '平台事实已严格回读；因果识别所需设计证据未提供。' : '平台事实与因果识别证据均未达到门槛。';
        }
        if (!sourceReady) {
            state = 'blocked';
            priorityScore = null;
        ...
        if (!missingEvidence.length && state === 'no_signal') missingEvidence = [ '原因证据未检验；“未命中阈值”不等于经营一定正常' ];
        return {
            platform,
            businessDate,
            state,
            priorityScore,
            priorityBand: priorityScore === null ? state === 'no_signal' ? 'monitor' : 'evidence_first' : priorityScore >= 80 ? 'high' : 'medium',
            evidenceLevel: sourceReady ? state === 'actionable' || state === 'no_signal' ? 'strict_fact_formula' : 'strict_fact_partial' : 'missing_required_fact',
    ...
    const buildRevenueCockpitOpportunities = ({selectedOtaPlatforms = [], currentCards = [], previousCards = [], sameWeekdayCards = [], previousDate = '', sameWeekdayDate = '', sourceReadyByPlatform = {}, businessDate = ''} = {}) => {
        const cards = REVENUE_COCKPIT_OPPORTUNITY_DEFINITIONS.map(definition => {
            ...
            const actionable = platformSignals.filter(signal => signal.state === 'actionable').sort((left, right) => Number(right.priorityScore || 0) - Number(left.priorityScore || 0));
            ...
            const state = actionable.length ? 'actionable' : investigations.length ? 'evidence_investigation' : blocked.length ? 'blocked' : 'no_signal';
            ...
            const platformSummary = platformSignals.map(signal => `${signal.platform === 'ctrip' ? '携程' : '美团'}：${signal.display}`).join('；');
            const evidenceLines = [ ...platformSignals.flatMap(signal => [ `${signal.platform === 'ctrip' ? '携程' : '美团'}事实变化：${signal.factChange}`, `${signal.platform === 'ctrip' ? '携程' : '美团'}证据支持：${signal.evidenceSupport}`, `${signal.platform === 'ctrip' ? '携程' : '美团'}尚缺证据：${signal.missingEvidence.join('、') || '无'}` ]), '证据分类：平台事实 → 公式计算 → 模型解释；相关性未检验；因果结论未声明；缺失保持未知。', `核查动作：${definition.action}` ];
            return {
                ...
                priorityScore: score,
                priorityBand: score === null ? state === 'no_signal' ? 'monitor' : 'evidence_first' : score >= 80 ? 'high' : 'medium',
                ...
                canCreatePendingApproval: [ 'actionable', 'evidence_investigation' ].includes(state) && platformSignals.some(signal => signal.evidenceLevel !== 'missing_required_fact'),
                evidenceLines
            };
        });
        const stateOrder = {
        ...
        cards.sort((left, right) => (stateOrder[left.state] ?? 9) - (stateOrder[right.state] ?? 9) || (left.state === 'actionable' ? Number(right.priorityScore || 0) - Number(left.priorityScore || 0) : 0) || Number(left.businessOrder || 0) - Number(right.businessOrder || 0) || String(left.opportunityKey).localeCompare(String(right.opportunityKey)));
    ...
    const buildRevenueCockpitModel = ({overview = null, comparisonOverview = null, sameWeekdayOverview = null, scope = null, selectedPlatform = '', businessDate = '', today = '', loading = false, error = ''} = {}) => {
        ...
        const selectedOtaPlatforms = platform === 'all_ota' ? [ 'ctrip', 'meituan' ] : [ 'ctrip', 'meituan' ].includes(platform) ? [ platform ] : [];
        ...
        if (!overview || !date || selectedOtaPlatforms.length === 0) return emptyModel('empty', context.notice || '没有找到可供驾驶舱使用的严格回读事实。');
        const factLayer = overview.three_source_fact_layer && typeof overview.three_source_fact_layer === 'object' ? overview.three_source_fact_layer : {};
        const sources = factLayer.sources && typeof factLayer.sources === 'object' ? factLayer.sources : {};
        const strictContract = overview.cockpit_strict_evidence && typeof overview.cockpit_strict_evidence === 'object' ? overview.cockpit_strict_evidence : {};
        const strictPlatforms = strictContract.platforms && typeof strictContract.platforms === 'object' ? strictContract.platforms : {};
        if (!Object.keys(sources).length) return emptyModel('empty', '总览接口未返回三源事实层；数值保持为空。');
        const previousFactLayer = comparisonOverview?.three_source_fact_layer && typeof comparisonOverview.three_source_fact_layer === 'object' ? comparisonOverview.three_source_fact_layer : {};
        const previousSources = previousFactLayer.sources && typeof previousFactLayer.sources === 'object' ? previousFactLayer.sources : {};
        const previousStrictContract = comparisonOverview?.cockpit_strict_evidence && typeof comparisonOverview.cockpit_strict_evidence === 'object' ? comparisonOverview.cockpit_strict_evidence : {};
        const previousStrictPlatforms = previousStrictContract.platforms && typeof previousStrictContract.platforms === 'object' ? previousStrictContract.platforms : {};
        const sameWeekdayFactLayer = sameWeekdayOverview?.three_source_fact_layer && typeof sameWeekdayOverview.three_source_fact_layer === 'object' ? sameWeekdayOverview.three_source_fact_layer : {};
        const sameWeekdaySources = sameWeekdayFactLayer.sources && typeof sameWeekdayFactLayer.sources === 'object' ? sameWeekdayFactLayer.sources : {};
        const sameWeekdayStrictContract = sameWeekdayOverview?.cockpit_strict_evidence && typeof sameWeekdayOverview.cockpit_strict_evidence === 'object' ? sameWeekdayOverview.cockpit_strict_evidence : {};
        const sameWeekdayStrictPlatforms = sameWeekdayStrictContract.platforms && typeof sameWeekdayStrictContract.platforms === 'object' ? sameWeekdayStrictContract.platforms : {};
        ...
        const sourceCards = sourceKeys.map(sourceKey => revenueCockpitSourceCard(sources[sourceKey] || {}, sourceKey, date, strictEvidenceForSource(sourceKey, strictPlatforms)));
        const coreDefinitions = {
            dingdandao_pms: [ [ 'room_revenue', '全酒店住宿房费', 'CNY' ], [ 'sold_room_nights', '全酒店已售间夜', 'room_nights' ], [ 'occupancy_rate_percent', '全酒店入住率', 'percent' ], [ 'adr', '全酒店住宿 ADR', 'CNY' ], [ 'revpar', '全酒店住宿 RevPAR', 'CNY' ] ],
            ctrip_ota: [ [ 'revenue', '携程渠道订单金额', 'CNY' ], [ 'orders', '携程渠道订单', 'orders' ], [ 'room_nights', '携程渠道间夜', 'room_nights' ], [ 'adr', '携程订单金额 / 间夜', 'CNY' ] ],
            meituan_ota: [ [ 'revenue', '美团渠道订单金额', 'CNY' ], [ 'orders', '美团渠道订单', 'orders' ], [ 'room_nights', '美团渠道间夜', 'room_nights' ], [ 'adr', '美团订单金额 / 间夜', 'CNY' ] ]
        };
        const trafficDefinitions = [ [ 'list_exposure', '列表曝光', 'exposures' ], [ 'detail_exposure', '详情曝光', 'exposures' ], [ 'flow_rate_percent', '进店转化率', 'percent' ], [ 'submit_rate_percent', '提交转化率', 'percent' ], [ 'payment_conversion_percent', '支付转化率', 'percent' ], [ 'cancellation_rate_percent', '取消率', 'percent' ] ];
        const coreCards = sourceKeys.flatMap(sourceKey => (coreDefinitions[sourceKey] || []).map(([metricKey, label, unit]) => revenueCockpitMetricCard({
        ...
        const previousCoreCards = sourceKeys.flatMap(sourceKey => (coreDefinitions[sourceKey] || []).map(([metricKey, label, unit]) => revenueCockpitMetricCard({
        ...
        const sameWeekdayCoreCards = sourceKeys.flatMap(sourceKey => (coreDefinitions[sourceKey] || []).map(([metricKey, label, unit]) => revenueCockpitMetricCard({
        ...
        const comparisonCards = currentComparableCards.map(card => revenueCockpitComparisonCard(card, previousByKey.get(card.key), context.previousDate || comparisonOverview?.business_date || ''));
        const sameWeekdayComparisonCards = currentComparableCards.map(card => revenueCockpitComparisonCard(card, sameWeekdayByKey.get(card.key), context.sameWeekdayDate || sameWeekdayOverview?.business_date || '', {
            basisKey: 'same_weekday',
            basisLabel: '同星期'
        }));
        const coverageFor = (cards, sourceKey) => {
            const scoped = cards.filter(card => card.sourceKey === sourceKey);
            const readyKeys = scoped.filter(card => card.value !== null).map(card => card.key.split(':').slice(-1)[0]).sort();
            return {
        ...
        const coverageCards = selectedOtaPlatforms.flatMap(otaPlatform => {
            ...
            return frames.map(frame => {
                const sameCoverage = frame.date && currentCoverage.ready === frame.coverage.ready && currentCoverage.readyKeys.join('|') === frame.coverage.readyKeys.join('|');
                return revenueCockpitTextCard({
                    ...
                    display: frame.date ? `当前 ${currentCoverage.ready}/${currentCoverage.total} 项；基期 ${frame.coverage.ready}/${frame.coverage.total} 项${sameCoverage ? '，覆盖一致' : '，覆盖不一致'}` : '—',
                    sourceKey,
                    businessDate: date,
                    ...
                    reasonText: frame.date ? sameCoverage ? '当前期与基期可用指标集合一致。' : '当前期与基期可用指标集合不同；变化值仍展示，但不得直接解释为经营变化。' : `没有${frame.label}严格回读日期，比较保持为空。`,
                    evidenceLines: [ `当前可用指标：${currentCoverage.readyKeys.join('、') || '无'}`, `基期可用指标：${frame.coverage.readyKeys.join('、') || '无'}`, '覆盖规则：指标集合不一致必须提示，禁止把缺字段造成的变化当作经营变化。' ]
                });
            });
        });
        const campaignStageCard = revenueCockpitTextCard({
            ...
            evidenceLines: [ `当前业务日：${date}`, '尚缺证据：活动 ID、活动阶段、阶段起止日及基期来源记录。', '边界：缺少活动阶段身份时比较为未知，不形成促销增量或因果结论。' ]
        });
        const rawGaps = (Array.isArray(factLayer.analysis_gaps) ? factLayer.analysis_gaps : []).filter(gap => gap && typeof gap === 'object');
        const anomalyCards = [];
        rawGaps.forEach((gap, index) => {
            ...
            anomalyCards.push(revenueCockpitTextCard({
                ...
                evidenceLines: [ `异常代码：${code}`, `来源：${String(gap.source || '三源事实层')}`, `业务日期：${date}`, `处理建议：${String(gap.next_action || '补齐同店同日事实并重新回读。')}` ]
            }));
        });
        selectedOtaPlatforms.forEach(otaPlatform => {
            const prefix = `${otaPlatform}_ota`;
            const cards = Object.fromEntries(coreCards.filter(card => card.sourceKey === prefix).map(card => [ card.key.split(':')[1], card ]));
            if (cards.revenue?.value > 0 && cards.orders?.value === 0) anomalyCards.push(revenueCockpitTextCard({
            ...
            if (cards.revenue?.value > 0 && cards.room_nights?.value === 0) anomalyCards.push(revenueCockpitTextCard({
        ...
        if (!anomalyCards.length) anomalyCards.push(revenueCockpitTextCard({
        ...
        const missingMetricCards = [ ...sourceCards, ...coreCards, ...trafficCards ].filter(card => card.missingState !== '有值' && card.missingState !== '完整');
        const gapCards = [];
        ...
        if (!gapCards.length) gapCards.push(revenueCockpitTextCard({
            key: 'gap:none',
            kind: 'gap',
            label: '数据缺口',
        ...
        const actionCards = [];
        if (dateDistance !== null && dateDistance > 0) actionCards.push(revenueCockpitTextCard({
        ...
        if (!actionCards.length) actionCards.push(revenueCockpitTextCard({
        ...
        const sourceReadyByPlatform = Object.fromEntries(selectedOtaPlatforms.map(otaPlatform => {
            const strictSource = strictPlatforms[otaPlatform] && typeof strictPlatforms[otaPlatform] === 'object' ? strictPlatforms[otaPlatform] : {};
            const acceptedIds = Array.isArray(strictSource.accepted_row_ids) ? strictSource.accepted_row_ids : [];
            return [ otaPlatform, strictSource.source_strict_readback === true && acceptedIds.some(id => Number(id) > 0) ];
        }));
        const requiredOtaSourcesReady = selectedOtaPlatforms.every(otaPlatform => sourceReadyByPlatform[otaPlatform] === true);
        ...
        const sections = [ {
            ...
            cards: [ ...comparisonCards, ...sameWeekdayComparisonCards, ...coverageCards, campaignStageCard ]
        }, {
            key: 'anomaly_reasons',
            title: '5. 异常原因',
        ...
        const status = completeSourceCount === sourceCards.length && missingMetricCards.length === 0 ? 'ready' : 'partial';
        const oldDateNotice = dateDistance === null ? '与今天的差异待确认。' : dateDistance === 0 ? '业务日就是今天。' : dateDistance > 0 ? `业务日比今天早 ${dateDistance} 天，页面展示的是最新严格可用历史事实。` : `业务日晚于今天 ${Math.abs(dateDistance)} 天，请复核日期。`;
        const sourceRecords = selectedOtaPlatforms.map(otaPlatform => ({
            sourceKey: `${otaPlatform}_ota`,
            table: 'online_daily_data',
            platform: otaPlatform,
            businessDate: date,
            rowIds: (Array.isArray(strictPlatforms?.[otaPlatform]?.accepted_row_ids) ? strictPlatforms[otaPlatform].accepted_row_ids : []).map(Number).filter(id => id > 0),
            readbackStatus: sourceReadyByPlatform[otaPlatform] ? 'readback_verified' : 'not_verified',
            factScope: 'ota_channel'
        }));
        const pmsProvenance = sources?.dingdandao_pms?.source && typeof sources.dingdandao_pms.source === 'object' ? sources.dingdandao_pms.source : {};
        if (sourceCards.find(card => card.sourceKey === 'dingdandao_pms')?.status === 'readback_verified' && Number(pmsProvenance.record_id || 0) > 0) sourceRecords.push({
        ...
        const missingItems = sections.flatMap(section => section.cards.filter(card => ![ 'readback_verified', 'derived_verified', 'verified', 'ready', 'ok', 'no_signal' ].includes(String(card.status || ''))).map(card => ({
        ...
        const metricDefinitions = {
            revenue: {
                label: 'OTA渠道订单金额',
                unit: 'CNY',
                sourceMeaning: 'order_amount',
                scope: 'per_ota_platform',
                missingPolicy: 'null'
            },
            orders: {
                label: 'OTA渠道订单',
            ...
            adr: {
                label: 'OTA订单金额 / 间夜',
                unit: 'CNY',
                formula: 'order_amount / room_nights',
                scope: 'per_ota_platform',
                missingPolicy: 'null'
            },
            list_exposure: {
        ...
        return {
            ...
            actionDisabledReason: requiredOtaSourcesReady ? '' : '所选 OTA 范围尚未同时返回可追溯记录ID和严格回读状态，不能生成待审批行动。'
        };
    };
    const buildRevenueCockpitDownloadRows = (model = {}) => {
        const sections = Array.isArray(model.visibleSections) ? model.visibleSections : [];
        let order = 0;
        return sections.flatMap(section => (Array.isArray(section.cards) ? section.cards : []).map(card => {
            order += 1;
            return {
                order,
    ...
    const buildRevenueCockpitCsv = (model = {}) => {
        const headers = [ [ 'order', '顺序' ], [ 'section', '分区' ], [ 'card', '卡片' ], [ 'display', '页面显示' ], [ 'unit', '单位' ], [ 'source', '来源' ], [ 'business_date', '业务日期' ], [ 'verification_status', '验证状态' ], [ 'scope', '口径' ], [ 'missing_state', '缺失状态' ], [ 'opportunity_key', '机会键' ], [ 'rank', '机会顺序' ], [ 'evidence_level', '证据等级' ], [ 'relationship_type', '关系类型' ], [ 'causality_claimed', '是否因果结论' ], [ 'explanation', '说明' ], [ 'evidence', '证据' ] ];
        const rows = buildRevenueCockpitDownloadRows(model);
        return `\ufeff${[ headers.map(([, label]) => revenueCockpitCsvCell(label)).join(','), ...rows.map(row => headers.map(([key]) => revenueCockpitCsvCell(row[key])).join(',')) ].join('\r\n')}`;
    };
    const buildRevenueCockpitDownloadPayload = (model = {}, fallbackHotelName = '') => {
        ...
        if (!rows.length) return {
            ...
            csv: '',
            fileName: '',
            message: '当前没有可见驾驶舱卡片可下载'
        };
        const safeHotel = String(model.hotelName || fallbackHotelName || 'hotel').replace(/[\\/:*?"<>|\s]+/g, '_').slice(0, 40);
        return {
            ok: true,
            rows,
    ...
    const buildRevenueCockpitQuestionDraft = (model = {}, hotelId = 0) => {
        ...
        if (!model.canAskQuestion || !normalizedHotelId || !model.businessDate || !model.selectedPlatform) return {
            ok: false,
            message: '当前酒店、平台或严格业务日期不完整，不能转为经营问题'
        };
    ...
    const buildRevenueCockpitOverviewEndpoint = (hotelId, businessDate, platform) => {
        ...
        } else if (platform) params.set('platform', String(platform));
        return `/revenue-ai/overview?${params.toString()}`;
    };
    const resolveRevenueCockpitOverviewResponse = (response = {}, expected = {}) => {
        if (Number(response.code || 0) !== 200) return {
            ok: false,
            overview: null,
            message: response.message || '经营驾驶舱事实读取失败'
        };
        ...
        const requiredPlatforms = platform === 'all_ota' ? [ 'ctrip', 'meituan' ] : [ 'ctrip', 'meituan' ].includes(platform) ? [ platform ] : [];
        if (!overview || String(overview.business_date || '') !== String(expected.businessDate || '') || Number(overview.hotel_id || factLayer?.hotel?.system_hotel_id || 0) !== Number(expected.hotelId || 0) || String(factLayer.business_date || expected.businessDate || '') !== String(expected.businessDate || '') || String(strict.contract_version || '') !== 'revenue_cockpit_strict_evidence.v1' || Number(strict.hotel_id || 0) !== Number(expected.hotelId || 0) || String(strict.business_date || '') !== String(expected.businessDate || '') || String(strict.platform || '') !== platform || Number(strict.tenant_id || 0) <= 0 || requiredPlatforms.length === 0 || requiredPlatforms.some(item => {
            const source = factLayer?.sources?.[`${item}_ota`] || {};
            const provenance = source?.source || {};
            const strictPlatform = strict?.platforms?.[item] || {};
            const acceptedIds = Array.isArray(strictPlatform.accepted_row_ids) ? strictPlatform.accepted_row_ids.map(Number).filter(id => id > 0) : [];
            const metrics = strictPlatform.metrics && typeof strictPlatform.metrics === 'object' ? strictPlatform.metrics : {};
            const hasAcceptedStrictMetric = Object.values(metrics).some(metric => metric && metric.strict_readback === true && Array.isArray(metric.accepted_row_ids) && metric.accepted_row_ids.some(id => acceptedIds.includes(Number(id))));
            return String(source.business_date || '') !== String(expected.businessDate || '') || String(source.actual_business_date || '') !== String(expected.businessDate || '') || String(provenance.platform || '') !== item || String(provenance.table || '') !== 'online_daily_data' || String(strictPlatform.business_date || '') !== String(expected.businessDate || '') || acceptedIds.length === 0 || !hasAcceptedStrictMetric;
        })) return {
            ok: false,
            overview: null,
            message: '经营驾驶舱回读的酒店、平台、业务日期或严格事实合同与当前筛选不一致'
        };
        return {
            ok: true,
            overview,
    ...
    const resolveRevenueCockpitScopeResponse = (response = {}, hotelId = 0) => {
        if (Number(response.code || 0) !== 200) return {
            ok: false,
            payload: null,
            message: response.message || '严格可用日期读取失败'
        };
        const payload = response.data || {};
        if (String(payload.contract_version || '') !== 'operating_question_scope_options.v1' || Number(payload.hotel_id || 0) !== Number(hotelId || 0) || payload?.boundary?.silent_date_fallback !== false) return {
            ok: false,
            payload: null,
            message: '严格可用日期回读身份或日期回退边界不一致'
        };
        return {
            ok: true,
            payload,
    ...
    const loadRevenueCockpitSnapshot = async ({hotelId = '', scopePayload = null, scopeHotelId = '', selectedPlatform = '', selectedDate = '', reloadScope = false, resetContext = false, resetDate = false, today = '', request, readScope, readOverview, isCurrent = () => true} = {}) => {
        ...
        if (!selection.selectedPlatform || !selection.selectedDate) return {
            status: 'empty',
            scopePayload: nextScope,
            selection,
            overview: null,
        ...
        const [overview, comparisonOverview, sameWeekdayOverview] = await Promise.all([ overviewReader(hotelId, selection.selectedDate, selection.selectedPlatform), selection.previousDate ? overviewReader(hotelId, selection.previousDate, selection.selectedPlatform) : Promise.resolve(null), selection.sameWeekdayDate && selection.sameWeekdayDate !== selection.previousDate ? overviewReader(hotelId, selection.sameWeekdayDate, selection.selectedPlatform) : selection.sameWeekdayDate && selection.sameWeekdayDate === selection.previousDate ? Promise.resolve('__reuse_previous__') : Promise.resolve(null) ]);
        if (!isCurrent()) return {
            status: 'superseded'
        };
        return {
            ...
            overview,
            comparisonOverview,
            sameWeekdayOverview: sameWeekdayOverview === '__reuse_previous__' ? comparisonOverview : sameWeekdayOverview
        };
    };
    ...
    const resolveRevenueDecisionSnapshot = (payload = {}, expected = {}) => {
        ...
        if (wrapper.found === false) return String(wrapper.persistence_status || '') === 'not_saved' ? {
            ok: true,
            status: 'not_saved',
            snapshot: null,
            message: '当前范围尚未保存收益决策快照'
        } : {
            ok: false,
            status: 'invalid',
            snapshot: null,
            message: '快照未保存状态缺少完整回读凭证'
        };
        const snapshot = wrapper.snapshot && typeof wrapper.snapshot === 'object' ? wrapper.snapshot : wrapper;
        const model = snapshot.visible_model && typeof snapshot.visible_model === 'object' ? snapshot.visible_model : {};
        const digestsValid = [ 'visible_model_digest', 'evidence_digest', 'content_digest' ].every(key => /^[a-f0-9]{64}$/.test(String(snapshot[key] || '')));
        const opportunities = Array.isArray(model.opportunities) ? model.opportunities : [];
        if (Number(snapshot.id || 0) <= 0 || String(snapshot.contract_version || '') !== 'revenue_decision_snapshot.v1' || String(snapshot.persistence_status || wrapper.persistence_status || '') !== 'readback_verified' || !digestsValid || String(model.contractVersion || '') !== 'revenue_daily_cockpit.v2' || Number(snapshot.system_hotel_id || 0) !== Number(expected.hotelId || model.hotelId || 0) || Number(model.hotelId || 0) !== Number(snapshot.system_hotel_id || 0) || String(snapshot.platform || '') !== String(expected.platform || model.selectedPlatform || '') || String(model.selectedPlatform || '') !== String(snapshot.platform || '') || String(snapshot.business_date || '') !== String(expected.businessDate || model.businessDate || '') || String(model.businessDate || '') !== String(snapshot.business_date || '') || opportunities.length !== 8 || new Set(opportunities.map(item => String(item?.opportunityKey || ''))).size !== 8) return {
            ok: false,
            status: 'invalid',
            snapshot: null,
            message: '收益决策快照身份、摘要或八类机会合同不一致'
        };
        if (Number(expected.snapshotId || 0) > 0 && Number(snapshot.id) !== Number(expected.snapshotId)) return {
            ok: false,
            status: 'invalid',
            snapshot: null,
            message: '收益决策快照 ID 回读不一致'
        };
        return {
            ok: true,
            status: String(snapshot.evidence_identity_status || 'not_checked'),
            snapshot,
            message: String(snapshot.evidence_identity_status || '') === 'stale_current_evidence' ? `快照 #${snapshot.id} 已精确回读，但当前事实证据身份已变化` : `快照 #${snapshot.id} 已按同一内容与证据身份精确回读`
        };
    };
    const saveRevenueDecisionSnapshotWithReadback = async ({request, model = {}, hotelId = 0} = {}) => {
        ...
        if (typeof request !== 'function' || !model.canSaveSnapshot || !normalizedHotelId || String(model.contractVersion || '') !== 'revenue_daily_cockpit.v2' || !model.businessDate || ![ 'ctrip', 'meituan', 'all_ota' ].includes(String(model.selectedPlatform || ''))) return {
            ok: false,
            snapshot: null,
            message: model.actionDisabledReason || '当前严格事实不足，不能保存收益决策快照'
        };
        ...
        if (!exact.ok || !exact.snapshot || exact.snapshot.content_digest !== savedResult.snapshot.content_digest || exact.snapshot.evidence_digest !== savedResult.snapshot.evidence_digest || exact.snapshot.visible_model_digest !== savedResult.snapshot.visible_model_digest) throw new Error(exact.message || '收益决策快照保存与回读摘要不一致');
        return {
            ok: true,
            status: exact.status,
    ...
    const restoreRevenueDecisionSnapshotWithReadback = async ({request, model = {}, hotelId = 0} = {}) => {
        const normalizedHotelId = Number(hotelId || model.hotelId || 0);
        if (typeof request !== 'function' || !normalizedHotelId || !model.businessDate || ![ 'ctrip', 'meituan', 'all_ota' ].includes(String(model.selectedPlatform || ''))) return {
            ok: false,
            status: 'invalid',
            snapshot: null,
            message: '当前范围不完整，无法恢复收益决策快照'
        };
    ...
    const createRevenueOpportunityPendingApprovalWithReadback = async ({request, snapshot = null, opportunityKey = ''} = {}) => {
        const stored = snapshot && typeof snapshot === 'object' ? snapshot : {};
        const model = stored.visible_model && typeof stored.visible_model === 'object' ? stored.visible_model : {};
        const opportunity = (Array.isArray(model.opportunities) ? model.opportunities : []).find(item => String(item?.opportunityKey || '') === String(opportunityKey || ''));
        if (typeof request !== 'function' || Number(stored.id || 0) <= 0 || !opportunity || opportunity.canCreatePendingApproval !== true || String(stored.evidence_identity_status || '') !== 'matched_current') return {
            ok: false,
            intent: null,
            message: '请先保存并精确回读当前证据身份，再选择一条可送审建议'
        };
        ...
        if (!savedResult.ok || savedResult.status !== 'pending_approval' || savedResult.taskCount !== 0 || Number(payload?.snapshot?.id || 0) !== Number(stored.id) || String(payload?.snapshot?.content_digest || '') !== String(stored.content_digest || '') || String(recommendation.opportunity_key || '') !== String(opportunity.opportunityKey) || !/^[a-f0-9]{64}$/.test(String(recommendation.recommendation_digest || ''))) throw new Error(savedResult.message || '经营机会与收益决策快照绑定不一致');
        ...
        if (!exact.ok || String(actionCard.contract_version || '') !== 'operation_action_card.v1' || !/^[a-f0-9]{64}$/.test(String(actionCard.content_digest || '')) || String(actionCard?.metric_contract?.target_type || '') !== 'observation' || String(actionCard?.action?.title || '') !== String(recommendation.title || '') || String(actionCard?.action?.description || '') !== String(recommendation.action_text || '')) throw new Error(exact.message || '经营机会没有精确回读同一受管行动生命周期');
        return {
            ok: true,
            intent: exact.intent,
            opportunity,
    ...
    const resolveRevenueCockpitPendingApprovalSave = (payload = {}) => {
        ...
        if (!intentId || status !== 'pending_approval' || String(savedIntent.status || '') !== 'pending_approval' || String(payload?.persistence_status || '') !== 'readback_verified' || !Array.isArray(tasks) || !Number.isInteger(taskCount) || taskCount !== 0 || tasks.length !== taskCount || payload?.execution_task_created !== false || payload?.external_action_triggered !== false) return {
            ok: false,
            intentId: 0,
            message: '待审批行动未返回“已回读且未执行”的完整凭证'
        };
        return {
            ok: true,
            intentId,
    ...
    const resolveRevenueCockpitPendingApprovalReadback = (intent = {}, expected = {}) => {
        if (!Array.isArray(intent.tasks)) return {
            ok: false,
            intent: null,
            message: '运营行动回读缺少真实任务数组'
        };
        ...
        if (Number(intent.id || 0) !== Number(expected.intentId || 0) || Number(intent.tenant_id || 0) <= 0 || Number(expected.tenantId || 0) > 0 && Number(intent.tenant_id || 0) !== Number(expected.tenantId || 0) || Number(intent.hotel_id || 0) !== Number(expected.hotelId || 0) || ![ 'ctrip', 'meituan', 'all_ota' ].includes(String(intent.platform || '')) || expected.platform && String(intent.platform || '') !== String(expected.platform) || !allowedSources.includes(String(intent.source_module || '')) || expected.sourceModule && String(intent.source_module || '') !== String(expected.sourceModule) || String(intent.object_type || '') !== String(expected.objectType || 'operation_checklist') || String(intent.action_type || '') !== String(expected.actionType || 'human_reviewed_operating_check') || String(intent.date_start || '') !== String(expected.businessDate || '') || String(intent.date_end || '') !== String(expected.businessDate || '') || !String(intent.status || '') || requirePending && String(intent.status || '') !== 'pending_approval' || requirePending && tasks.length !== 0 || !requirePending && expectedStatus && String(intent.status || '') !== expectedStatus || !requirePending && Number.isInteger(expectedTaskCount) && expectedTaskCount >= 0 && tasks.length !== expectedTaskCount || !revenueCockpitTaskCardinalityIsValid(String(intent.status || ''), tasks) || String(actionCard.contract_version || '') !== 'operation_action_card.v1' || !/^[a-f0-9]{64}$/.test(String(actionCard.content_digest || '')) || decisionContext && (Number(trace.decision_snapshot_id || 0) !== Number(decisionContext.snapshotId || 0) || String(trace.decision_snapshot_digest || '') !== String(decisionContext.snapshotDigest || '') || String(trace.opportunity_key || '') !== String(decisionContext.opportunityKey || '') || String(trace.opportunity_digest || '') !== String(decisionContext.opportunityDigest || '')) || Array.isArray(expected.taskSignatures) && JSON.stringify(tasks.map(task => ({
            id: Number(task?.id || 0),
            status: String(task?.status || ''),
            result_status: String(task?.result_status || '')
        })).sort((left, right) => left.id - right.id)) !== JSON.stringify(expected.taskSignatures)) return {
            ok: false,
            intent: null,
            message: '待审批行动保存与精确回读身份不一致'
        };
        return {
            ok: true,
            intent,
    ...
    const revenueCockpitTaskCardinalityIsValid = (intentStatus = '', tasks = []) => {
        if (!Array.isArray(tasks) || tasks.some(task => !Number.isInteger(Number(task?.id || 0)) || Number(task.id) <= 0)) return false;
    ...
    const restoreRevenueCockpitPendingApprovalWithReadback = async ({request, model = {}, hotelId = 0, snapshot = null} = {}) => {
        const normalizedHotelId = Number(hotelId || 0);
        ...
        if (typeof request !== 'function' || !normalizedHotelId || !tenantId || !businessDate || ![ 'ctrip', 'meituan', 'all_ota' ].includes(platform)) return {
            ok: false,
            intent: null,
            message: '当前经营驾驶舱范围不完整，无法恢复已保存行动'
        };
        const params = new URLSearchParams({
        ...
        if (Number(restored?.code || 0) !== 200) throw new Error(restored?.message || '已保存运营行动恢复失败');
        const payload = restored?.data || {};
        const scope = payload?.cockpit_scope || {};
        if (Number(scope.hotel_id || 0) !== normalizedHotelId || Number(scope.tenant_id || 0) !== tenantId || String(scope.business_date || '') !== businessDate || String(scope.platform || '') !== platform) throw new Error('已保存运营行动恢复范围与当前驾驶舱不一致');
        if (payload?.found === false) {
            if (String(payload?.status || '') !== 'not_saved' || String(payload?.persistence_status || '') !== 'not_saved' || payload?.execution_intent !== null || Array.isArray(payload?.execution_intents) && payload.execution_intents.length !== 0 || Number(payload?.execution_task_count || 0) !== 0) throw new Error('未保存状态缺少完整的只读回读凭证');
            return {
                ok: true,
                status: 'not_saved',
                intent: null,
        ...
        const scopedIntents = Array.isArray(payload?.execution_intents) ? payload.execution_intents : [ payload?.execution_intent || {} ];
        ...
        if (payload?.found !== true || String(payload?.persistence_status || '') !== 'readback_verified' || !intentId || !Array.isArray(scopedIntents) || scopedIntents.length < 1 || !Number.isInteger(intentCount) || intentCount !== scopedIntents.length || scopedIntentIds.some(id => id <= 0) || new Set(scopedIntentIds).size !== scopedIntentIds.length || Number(scopedIntents[0]?.id || 0) !== intentId || !Number.isInteger(taskCount) || taskCount < 0 || !Array.isArray(scopedIntent.tasks) || scopedIntent.tasks.length !== taskCount || String(scopedIntent.status || '') !== String(payload?.status || '') || !revenueCockpitTaskCardinalityIsValid(String(scopedIntent.status || ''), scopedIntent.tasks)) throw new Error('已保存运营行动未返回完整的范围回读凭证');
        const exactIntents = await Promise.all(scopedIntents.map(async (candidate, index) => {
            if (!Array.isArray(candidate?.tasks) || !String(candidate?.status || '')) throw new Error('已保存运营行动列表缺少真实状态或任务数组');
            if (!revenueCockpitTaskCardinalityIsValid(String(candidate.status || ''), candidate.tasks)) throw new Error('已保存运营行动任务基数与生命周期状态不一致');
            const candidateId = Number(candidate.id || 0);
            const readback = await request(`/operation/execution-intents/${candidateId}`);
            if (Number(readback?.code || 0) !== 200) throw new Error(readback?.message || '已保存运营行动精确回读失败');
            ...
            const actionCard = exactResult.intent?.target_value?.action_card || exactResult.intent?.evidence?.action_card || {};
            const trace = actionCard?.trace || {};
            const opportunityKey = String(trace.opportunity_key || '').trim();
            if (opportunityKey) {
                const storedSnapshot = snapshot && typeof snapshot === 'object' ? snapshot : {};
                const storedModel = storedSnapshot.visible_model && typeof storedSnapshot.visible_model === 'object' ? storedSnapshot.visible_model : {};
                const opportunity = (Array.isArray(storedModel.opportunities) ? storedModel.opportunities : []).find(item => String(item?.opportunityKey || '') === opportunityKey);
                if (!opportunity || String(storedSnapshot.evidence_identity_status || '') !== 'matched_current' || Number(trace.decision_snapshot_id || 0) !== Number(storedSnapshot.id || 0) || String(trace.decision_snapshot_digest || '') !== String(storedSnapshot.content_digest || '') || !/^[a-f0-9]{64}$/.test(String(trace.opportunity_digest || '')) || String(actionCard?.action?.title || '') !== String(opportunity.title || '') || String(actionCard?.action?.description || '') !== String(opportunity.recommendedCheckAction || '')) throw new Error('已保存经营机会与当前收益决策快照身份不一致');
            }
            if (index === 0 && candidate.tasks.length !== taskCount) throw new Error('主运营行动任务数与范围回读不一致');
