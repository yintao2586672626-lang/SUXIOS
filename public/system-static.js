window.SUXI_SYSTEM_STATIC = (() => {
    ...
    const aiQuickSetupProviderModelMap = {
        ollama: ['local_second_brain · qwen3:4b'],
    ...
    const createAiGovernanceEvaluationForm = () => ({
    ...
    const createAiGovernanceEvaluationRuntime = ({
        request,
        evaluationForm,
        evaluationCases,
        evaluationRuns,
        ...
        if (typeof request !== 'function') throw new Error('本机评测运行时缺少受控请求函数');
        ...
        const loadCases = async () => {
            const evaluationSet = String(evaluationForm.value.evaluation_set || '').trim();
            const params = new URLSearchParams({ page: '1', page_size: '30' });
            if (evaluationSet) params.set('evaluation_set', evaluationSet);
            const response = await request(`/ai-governance/evaluation-cases?${params.toString()}`);
            if (response.code !== 200) throw new Error(response.message || '评估集加载失败');
            evaluationCases.value = Array.isArray(response.data?.list) ? response.data.list : [];
        ...
        const loadRuns = async () => {
            ...
            if (evaluationSet) params.set('evaluation_set', evaluationSet);
            const response = await request(`/ai-governance/evaluation-runs?${params.toString()}`);
            if (response.code !== 200) throw new Error(response.message || '评测批次加载失败');
            evaluationRuns.value = Array.isArray(response.data?.list) ? response.data.list : [];
            return evaluationRuns.value;
        };
        const saveCase = async () => {
            if (evaluationAction.value) return null;
            ...
            if (!caseKey || !evaluationSet || !String(form.scenario || '').trim() || !String(form.prompt_version || '').trim()) {
                evaluationMessage.value = '请填写用例键、评测集、场景和 Prompt 版本。';
                return null;
            }
            evaluationAction.value = 'save';
            evaluationMessage.value = '';
            try {
                const input = parseJsonObject(form.input_text, '输入');
                const expected = parseJsonObject(form.expected_text, '期望结果');
                const metrics = parseJsonObject(form.metrics_text, '指标策略');
                ...
                const caseId = Number(saved.data?.id || 0);
                if (!caseId) throw new Error('评测用例没有返回保存记录');
                ...
                if (!exact
                    || Number(exact.id || 0) !== caseId
                    || String(exact.scenario || '') !== String(form.scenario || '').trim()
                    || String(exact.prompt_version || '') !== String(form.prompt_version || '').trim()
                    || String(exact.status || '') !== 'active'
                    || !sameJson(exact.input_json, input)
                    || !sameJson(exact.expected_json, expected)
                    || !sameJson(exact.metric_json, metrics)
                ) throw new Error('评测用例保存与精确回读不一致');
                evaluationMessage.value = `用例 #${Number(exact.id || 0)} 已保存并精确回读。`;
                await loadCases();
                return exact;
            } catch (error) {
                evaluationMessage.value = error?.message || '评测用例保存失败';
                return null;
            } finally {
                evaluationAction.value = '';
            }
        };
        const runEvaluation = async (execute = false) => {
            if (evaluationAction.value) return null;
            if (execute && !window.confirm('确认调用本机 Ollama 执行当前评测集？结果只保存为治理记录，不创建经营动作。')) return null;
            const evaluationSet = String(evaluationForm.value.evaluation_set || '').trim();
            if (!evaluationSet) {
                evaluationMessage.value = '请先填写评测集。';
                return null;
            }
            ...
            evaluationAction.value = execute ? 'execute' : 'dry_run';
            evaluationMessage.value = '';
            try {
                ...
                if (Number(exact.id || 0) !== runId
                    ...
                    || exact.dry_run !== !execute
                    || exact?.result?.allow_external_model_call !== false
                    || String(exact.result_digest || '') !== String(savedRun.result_digest || '')
                    || exact.readback_verified !== true
                ) throw new Error('评测批次保存与精确回读不一致');
                evaluationRun.value = exact;
                evaluationMessage.value = execute
                    ? '本机评测已执行、保存并精确回读。'
                    : '评测计划已保存并精确回读，尚未调用模型。';
                await loadRuns();
                return exact;
            } catch (error) {
                evaluationMessage.value = error?.message || '评测批次运行失败';
                return null;
            } finally {
                evaluationAction.value = '';
            }
        };
        return { loadCases, loadRuns, saveCase, runEvaluation };
    };

    const createLocalSecondBrainOperatingQuestionState = () => ({
    ...
    const resetLocalSecondBrainOperatingQuestionState = (state, resetMedia = false) => {
        state.council_error = '';
        state.council_run = null;
        if (!resetMedia) return;
        state.media_file = null;
        state.media_result = null;
        state.media_history = [];
        state.media_error = '';
    };
    const createOperatingQuestionCouncilRuntime = ({ request, stateRef }) => {
        if (typeof request !== 'function') throw new Error('影子复核运行时缺少受控请求函数');
        const loadLatest = async (questionId) => {
            ...
            try {
                ...
                if (response.code !== 200) throw new Error(response.message || '影子复核回读失败');
                const exact = response.data;
                const noSavedRun = exact == null
                    || (Array.isArray(exact) ? exact.length === 0 : typeof exact === 'object' && Object.keys(exact).length === 0);
                if (noSavedRun) {
                    state.council_run = null;
                    return null;
                }
                if (Number(state.result?.id || 0) !== id
                    || Number(exact.question_id || 0) !== id
                    || Number(exact.hotel_id || 0) !== Number(state.result?.hotel_id || 0)
        ...
        const run = async () => {
            ...
            try {
                ...
                if (response.code !== 200 || !response.data) throw new Error(response.message || '影子复核运行失败');
                ...
                ) throw new Error('影子复核没有返回保存回读凭证');
                const exact = await loadLatest(questionId);
                if (!exact
                    || Number(exact.id || 0) !== Number(saved.id || 0)
                    || String(exact.content_digest || '') !== String(saved.content_digest || '')
                ) throw new Error('影子复核保存与 GET 回读不一致');
                return exact;
            } catch (error) {
                state.council_error = error?.message || '影子复核运行失败';
                return null;
            } finally {
                state.council_loading = false;
            }
        };
        return { loadLatest, run };
    };
    ...
    return {
        ...
        createAiGovernanceEvaluationForm,
        createAiGovernanceEvaluationRuntime,
        createLocalSecondBrainOperatingQuestionState,
        resetLocalSecondBrainOperatingQuestionState,
        createOperatingQuestionCouncilRuntime,
