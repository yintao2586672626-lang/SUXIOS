...
- `public/index.html` no longer eagerly loads `ai-analysis-static.js` in the first shell. It lazy-loads the AI analysis static helper on online-data, Ctrip eBooking, and Meituan eBooking pages. Ctrip AI analysis, Meituan AI analysis, and AI hotel-list refreshes explicitly await `ensureAiAnalysisStaticReady()`; load failure remains visible through toast and stops the current action.
