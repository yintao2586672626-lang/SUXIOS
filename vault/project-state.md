...
- Entry and OTA guard scripts were updated: `verify_public_entry_guard.mjs` prevents eager `vue-router.global.prod.js` loading, requires deferred `form-operation-support.js`, and guards the static gzip cache contract; platform batch health, platform data-source, P0 learning, and manual minimum credential UI tests now assert the new scope boundary.
