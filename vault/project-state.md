...
- The same save point preserves the Meituan target-hotel config sync guard: ranking-page hotel select applies the current config directly, the hotelId watcher refreshes the active config, config detail loading aborts stale target updates, and manual credential UI assertions cover the behavior.
...
- Current split-map: `public/index.html` is `38046` lines; frontend function-level blocks are `1408`; `ai` domain span is `1272`; `meituan` domain span is `1249`; `startAiAnalysis` is `144` lines.
- Current self-audit: full directory about `270.59 MB`, without `.git` about `92.24 MB`, without `.git` and dependencies about `63.05 MB`, tracked files about `18.13 MB` / `613` files; code scope `370` files, `189316` total lines, and `173540` nonblank lines; cleanup candidates `0`.
