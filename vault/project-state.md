...
- `public/router.php` now serves static files inside the public root with MIME, ETag, Last-Modified, Cache-Control, and optional gzip handling, while preserving the ThinkPHP fallback for application routes.
- `public/index.html` now requests a 100-row online-analysis sample instead of `page_size=all`, describes full coverage through summary metrics rather than implying complete row detail, lazy-loads Chart.js, and limits Meituan competitor by-hotel summary loading to explicit `includeByHotel: true` call sites. The Meituan ranking refresh still uses the selected hotel id and explicitly passes `includeByHotel: false`.
- Entry and OTA guard scripts were updated: `verify_public_entry_guard.mjs` prevents eager `vue-router.global.prod.js` loading and requires deferred `form-operation-support.js`; platform batch health, platform data-source, P0 learning, and manual minimum credential UI tests now assert the new scope boundary.
- Current self-audit: full directory about `293.54 MB`, without `.git` about `92.60 MB`, without `.git` and dependencies about `63.41 MB`, tracked files about `18.48 MB` / `613` files; code scope `370` files, `193845` total lines, and `177986` nonblank lines; cleanup candidates `0`.
- Current split-map: `public/index.html` has `36772` lines and `1396` frontend function-level blocks; `app/controller/OnlineData.php` has `26725` lines and `871` methods. Both remain real split candidates; `public/tailwind.min.css` remains accepted as a local CSS dependency, not a business-code split target.
