...
final class PromotionIncrementalityServiceTest extends TestCase
{
    public function testPositiveIncrementWithQualifiedEvidenceIsSupported(): void
    {
        ...
        self::assertEqualsWithDelta(0.2, $result['treated_rate_change'], 0.000000001);
        self::assertEqualsWithDelta(0.05, $result['control_rate_change'], 0.000000001);
        self::assertEqualsWithDelta(0.15, $result['incremental_rate'], 0.000000001);
        ...
        self::assertSame('exposure_normalized_difference_in_differences', $result['evidence_boundary']['estimator']);
    ...
    public function testEqualRateGrowthAcrossUnequalGroupsProducesZeroIncrement(): void
    {
        $result = (new PromotionIncrementalityService())->evaluate(self::validInput([
            'treated_before' => 100,
            'treated_after' => 120,
            ...
            'control_before' => 50,
            'control_after' => 60,
            'control_before_exposure' => 100,
            'control_after_exposure' => 100,
            'discount_cost' => 0,
        ]));

        self::assertEqualsWithDelta(0.1, $result['treated_rate_change'], 0.000000001);
        self::assertEqualsWithDelta(0.1, $result['control_rate_change'], 0.000000001);
        self::assertEqualsWithDelta(0.0, $result['incremental_rate'], 0.000000001);
        self::assertEqualsWithDelta(0.0, $result['incremental_room_nights'], 0.000000001);
        self::assertSame('indeterminate', $result['verdict']);
        self::assertSame(['net_incremental_profit_zero'], $result['reason_codes']);
    }

    public function testLegacyInputWithoutExposureIsExplicitlyIndeterminate(): void
    {
        $input = self::validInput();
        unset(
            $input['treated_before_exposure'],
            $input['treated_after_exposure'],
            $input['control_before_exposure'],
            $input['control_after_exposure']
        );

        $result = (new PromotionIncrementalityService())->evaluate($input);

        self::assertSame('indeterminate', $result['verdict']);
        self::assertSame([
            'treated_before_exposure_missing',
            'treated_after_exposure_missing',
            'control_before_exposure_missing',
            'control_after_exposure_missing',
        ], $result['reason_codes']);
        self::assertNull($result['incremental_rate']);
        self::assertNull($result['incremental_room_nights']);
        self::assertNull($result['incremental_contribution']);
        self::assertNull($result['net_incremental_profit']);
        self::assertFalse($result['design_assessment']['exposure_complete']);
        self::assertFalse($result['design_assessment']['evidence_threshold_met']);
    }
    ...
    public static function invalidInputProvider(): iterable
    {
        ...
        yield 'zero exposure' => [
            self::validInput(['treated_after_exposure' => 0]),
            'promotion_incrementality_positive_number_required:treated_after_exposure',
        ];
    }
