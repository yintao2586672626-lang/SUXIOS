...
test('frontend security and hotel-scoped requests reject stale state instead of showing synthetic zeroes', () => {
  ...
  assert.match(appMain, /watch\(currentPage, \(newPage\) => \{[\s\S]{0,4000}?if \(requestSuxiFullRenderForPage\(newPage\)\) return;\s*if \(!guardSuperAdminPageAccess\(newPage\)\) return;/);
