...
test('frontend persists and verifies exact hotel manager formula and digests', () => {
  ...
  for (const marker of [
    'summarizeDailySubmission',
    "'daily_submission' => $dailySubmission",
    "'attention_status' => $attentionStatus",
    "'closure_inferred' => false",
    "'independent_verification' => false",
    '状态只供人工查看，不自动提醒、建任务、处罚或外发',
    'ManagerCapabilityPanel',
    'managerCapabilityRequest: apiRequest',
  ]) {
    assert.ok((marker === 'ManagerCapabilityPanel' || marker === 'managerCapabilityRequest: apiRequest'
      ? appMain
      : service).includes(marker), marker);
  }
  ...
  const loaderVersion = componentsLoader.match(/app-main-components\.js\?v=(20\d{6}-manager-capability(?:-[a-z-]+)?-h[a-f0-9]{10})/)?.[1];
  assert.ok(loaderVersion, 'versioned manager capability component');
...
test('deferred component bridge resolves the manager score panel and loop authority', async () => {
  ...
  const authority = await facade.OperatingLoopAuthority.loader();
  assert.equal(panel?.name, 'ManagerCapabilityPanel');
  assert.equal(authority?.name, 'OperatingLoopAuthority');
});
...
test('authenticated page dependency receives Vue ref and pins the repaired asset', () => {
  ...
  const componentHash = createHash('sha256').update(operatingComponents).digest('hex').slice(0, 10);
  assert.match(
    operatingLoader,
    new RegExp(`operating-intelligence-components\\.js\\?v=[^'\"]*-h${componentHash}`),
  );
});
