...
test('P0 Profile next-step report does not block when target traffic hotel is covered with extra reference steps', () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'p0-profile-next-steps-extra-reference-'));
  const input = path.join(dir, 'verifier.json');
  try {
    writeFileSync(input, JSON.stringify({
            ...
            hotel_scoped_next_steps: [
              {
                ...
                profile_login_trigger: {
                  status: 'available',
                  entry: '/api/online-data/profile-login-trigger/ctrip',
                  after_login_sync: { entry: '/api/online-data/data-sources/19/sync' },
                },
              },
              {
                system_hotel_id: 60,
