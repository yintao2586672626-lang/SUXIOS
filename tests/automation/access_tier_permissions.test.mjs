...
assert.match(indexHtml, /const hotelPlatformBlockingIssueText = \(account = \{\}\) => \{[\s\S]*login_expired[\s\S]*mismatch[\s\S]*最近采集失败/, 'hotel management must derive one explicit blocking reason for login, identity, and capture failures');
...
assert.match(indexHtml, /'采集阻塞门店' : 'OTA采集接入状态'[\s\S]*filteredHotels\.length/, 'hotel account ledger should name the active blocking filter without losing the normal OTA collection title');
