...
const envExample = read('deploy/systemd/ota-cloud-profile.env.example');
...
test('cloud OTA service serializes Ctrip then Meituan and cannot send messages', () => {
  ...
  assert.match(timer, /OnCalendar=\*-\*-\* \*:40:00 Asia\/Shanghai/);
  assert.match(timer, /Persistent=false/);
  assert.doesNotMatch(envExample, /^SUXIOS_CLOUD_OTA_(?:OWNER_USER_ID|CTRIP_SOURCE_ID|MEITUAN_SOURCE_ID)=\d+$/mu);
  assert.match(envExample, /SUXIOS_CLOUD_OTA_OWNER_USER_ID=REPLACE_WITH_EXECUTION_OWNER_USER_ID/);
});
