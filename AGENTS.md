...
本节是用户对宿析OS目标型工作的持续明确要求：用户只需提出自然语言目标和建议，主控 Agent 必须主动完成标杆研究、边界推导、任务拆解、构建、独立盲审和集成验收。对非简单目标自动使用 `.agents/skills/suxi-gauntlet-loop`，不把研究、技术路线、任务拆解或测试管理转嫁给用户。

1. 主控先研究与目标最接近的真实世界高水平产品或权威实现，记录来源、当前可验证证据、适用范围和不适用部分；主动提出决定性比较维度。用户可以补充或纠偏，但不负责准备完整标杆。不得用“行业领先”“3A”“高级感”等形容词替代可观察证据。
2. 主控结合代码、数据身份、运行环境和授权现状起草业务边界，至少覆盖对象/酒店、平台/来源、日期/口径、写入范围、非目标和失败状态。只有选择会改变业务模块、数据口径、真实租户/门店、外部接收方、生产写入或不可逆结果时，才请用户确认一个决定性问题。
3. 主控把目标拆到可独立构建、比较、判定和优化的最小单元。每个单元在实现前必须有验收卡：预期结果、现实标杆、决定性比较维度、证据方法、边界与停止条件；无法单独判定的单元必须继续拆分。
4. 非简单的多单元目标中，每个单元必须分别指定一位专业构建者和一位独立盲审批评者。单元盲审和总盲审每次都必须创建不继承构建者会话的全新隔离上下文（子 Agent 使用 `fork_turns=none` 或等效新上下文），向子 Agent 下达明确的零写入指令，且只传白名单评审包：验收卡、现实标杆原始证据及来源/日期、边界、产物快照或受限只读路径、原始验证证据、固定判决协议。禁止传构建者对话、过程说明、自我评价、投入成本、结论或主控代写摘要。盲审者必须保持执行过程零写入，禁止实际调用任何编辑、写入、`apply_patch`、提交、代修或外部动作；主控必须保存并比较审前/审后目标文件哈希与限定工作区 diff。工具轨迹仅在平台实际提供时追加核对，不作为所有环境的强制证据。发现目标文件或限定工作区发生变化时，标记“盲审未完成”且不得输出判决；哈希与 diff 一致且未观察到写入时允许正常判卷。仅仅工具可用但未调用不导致失败。主控不得用自己的复核替代独立盲审。
5. 盲审判决固定为 `PASS`、`FAIL`、`BENCHMARK_MISSING`。只有产物在预先声明的决定性维度上有证据优于现实标杆、且没有破坏真实性、兼容性、核心功能与授权边界时才可 `PASS`；构建者自称完成、主观相似、只有测试通过但用户闭环未通，均为 `FAIL`；缺少现实标杆或缺少可比证据必须为 `BENCHMARK_MISSING`。登录、权限、凭证、外部服务等阻塞作为独立流程状态报告，不得伪装成第四种判决或代替标杆判卷。
6. 未通过时，盲审者每轮只指出当前最大的可验证差距及最小返工范围，退回原单元重做；修复后必须使用同一口径复验。连续三轮没有新增决定性证据时，不得降低标准或制造通过，停止循环并报告唯一真实阻塞。
7. 所有单元通过后，由主控集成，并交给未参与构建的总盲审者做端到端比较。只有实际入口、核心操作、保存/精确回读（适用时）、真实失败状态和标杆比较证据全部成立，才可报告完成。
8. 本协议授权在项目任务内按需创建构建者和盲审子 Agent，但不授权联系其他用户任务/线程，不授权外部发送、生产发布、真实 OTA 写入或不可逆动作。后述并发限制仍然生效：通常复用 2-3 个槽位串行推进，冲突文件和高风险数据链禁止并行写入。
9. 单文件、小修复、状态核验等原子任务不为形式强行创建多个子 Agent，但仍必须执行“构建/检查 → 与明确标准隔离判卷 → 验证”的紧凑循环。用户明确要求实际子 Agent 盲审时除外；若平台无可用槽位或无法创建子 Agent，必须如实标记盲审未完成，不得伪造独立评审结论。
