...
1. 默认把用户每条消息视为“可能含转录噪声的普通话语音文字”，除非用户明确说明是键入原文或要求逐字保留。
2. 每轮解释意图前自动使用 `.agents/skills/suxi-voice-intent`，并读取本机 `$transcript-fixer` 的 `suxios-hotel` 语境；用户不需要点名或手动调用 Skill。
