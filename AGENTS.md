...
4. 用户明确当前业务目标并说“继续”“执行”或“完善”后，即视为对该闭环内正常实现、可回退迁移、指定环境发布、生产验证和必要写入的一次授权；不得要求固定授权口令，也不得逐步骤重复确认。仅当无法从上下文安全确定真实外发目标或租户、需要用户输入凭证、涉及不可逆删除/覆盖，或工作范围发生实质扩张时，才提出一个决定性问题。
