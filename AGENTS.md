...
本节是用户对宿析OS目标型工作的持续明确要求：用户只需提出自然语言目标和建议，主控 Agent 负责标杆审计、边界草案、拆解、构建、隔离评审、返工和集成验收。对有可检查产物的非简单目标自动使用 `.agents/skills/suxi-gauntlet-loop`。该完整流程是基于公开证据组合出的工程协议，不得宣称它已经作为整体被科学验证，也不得把 Agent 数量或循环次数当作质量。
