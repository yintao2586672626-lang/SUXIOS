...
assert_true(str_contains($saveMeituanCommentConfig, 'commentCollectionDisabledResponse'), 'Meituan comment config save must remain disabled');
assert_true(str_contains($saveCtripCommentConfig, 'commentCollectionDisabledResponse'), 'Ctrip comment config save must remain disabled');
assert_true(!str_contains($saveMeituanCommentConfig, 'setConfigList'), 'disabled Meituan comment config save must not persist stale comment credentials');
assert_true(!str_contains($saveCtripCommentConfig, 'setConfigList'), 'disabled Ctrip comment config save must not persist stale comment credentials');
...
assert_true(str_contains($getMeituanCommentConfigList, 'return $this->success([]);'), 'Meituan comment config list must stay empty while comment collection is disabled');
assert_true(str_contains($getCtripCommentConfigList, 'return $this->success([]);'), 'Ctrip comment config list must stay empty while comment collection is disabled');
assert_true(!str_contains($getMeituanCommentConfigList, 'getConfigList'), 'disabled Meituan comment config list must not expose stored comment credentials');
assert_true(!str_contains($getCtripCommentConfigList, 'getConfigList'), 'disabled Ctrip comment config list must not expose stored comment credentials');
