...
export async function inspectFrontendTemplateBuild(repoRoot) {
  ...
  if (!template) failures.push('resources/frontend/app-template.html is missing or empty.');
  else {
    try {
  ...
  if (Buffer.byteLength(template) < 1_000_000) failures.push('The canonical root template is unexpectedly small.');
  if (!template.includes('v-if="!isLoggedIn"') || !template.includes('v-if="toast.show"')) {
    failures.push('The canonical root template is missing login or global toast boundaries.');
  }
