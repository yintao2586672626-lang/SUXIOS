...
export const AI_DAILY_REPORT_DELIVERY_RELATIVE_PATH = 'public/components/system/ai-daily-report-delivery.js';
...
async function inspectFrontendTemplateBuildUnlocked(repoRoot) {
  ...
  const aiDailyReportDelivery = readText(aiDailyReportDeliveryPath);
  const failures = [];
  ...
  if (!aiDailyReportDelivery) {
    failures.push(`${AI_DAILY_REPORT_DELIVERY_RELATIVE_PATH} is missing or empty.`);
  }
  ...
  if (!businessClosureLoader) {
    ...
    try {
      const deliveryVersion = readFrontendAssetVersion(
        businessClosureLoader,
        'ai-daily-report-delivery.js',
      );
      const deliveryHash = aiDailyReportDelivery
        ? crypto.createHash('sha256').update(aiDailyReportDelivery).digest('hex').slice(0, 10)
        : '';
      if (deliveryHash && deliveryVersion.hash !== deliveryHash) {
        failures.push('The business closure loader must reference the current AI daily report delivery content hash.');
      }
    } catch (error) {
      failures.push(error.message);
    }
