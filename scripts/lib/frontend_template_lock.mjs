...
export function writeFileAtomic(file, content) {
  ...
  try {
    ...
    try {
      ...
      if (!['EPERM', 'EEXIST'].includes(String(error?.code || '')) || !fs.existsSync(file)) {
        throw error;
      }
      // Windows can temporarily deny replacing a loaded frontend artifact even
      // though overwriting its contents is allowed. Keep the generated bytes
      // and fall back to a direct replacement instead of publishing stale code.
