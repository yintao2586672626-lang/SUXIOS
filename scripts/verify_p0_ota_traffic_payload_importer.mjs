...
const cases = [
  ...
  {
    name: 'ctrip_standard_rows_all_zero_required_metrics_blocked',
    platform: 'ctrip',
    payload: {
      standard_rows: [
        {
          ...
          _source_path: 'standard_rows.0',
          capture_evidence: {
            source_trace_id: 'ctrip:standard-row-zero-traffic',
            source_url_hash: hash('z'),
          },
          list_exposure: 0,
          detail_exposure: 0,
          flow_rate: 0,
    ...
    expect: {
      ...
      requiredMetricValueStatus: 'zero_value_unverified',
      issuesPresent: ['target_date_required_traffic_metrics_zero_unverified'],
      issuesAbsent: ['required_traffic_metric_keys_missing', 'traffic_field_fact_preview_rows_incomplete'],
    },
  },
  {
...
for (const item of cases) {
  ...
  if (Number.isFinite(Number(item.expect.nonzeroRequiredMetricRows))) {
    check(item.name, `${item.name} nonzero required metric rows`, Number(summary.target_date_nonzero_required_metric_rows || 0) === item.expect.nonzeroRequiredMetricRows, JSON.stringify(summary));
  }
...
if (evidenceCase) {
  ...
  check('external_evidence_contract', 'P0 traffic gate exposes external evidence source path diagnostics', Number(verifierTrafficGate.external_evidence_rows || 0) > 0 && Number(verifierTrafficGate.external_evidence_row_source_path_rows || 0) > 0 && Number(verifierTrafficGate.external_evidence_row_source_path_structured_rows || 0) > 0, JSON.stringify(verifierTrafficGate));
  check('external_evidence_contract', 'P0 traffic gate exposes external raw_data field fact diagnostics without raw data', Number(verifierTrafficGate.external_evidence_raw_data_field_facts_rows || 0) > 0 && Number(verifierTrafficGate.external_evidence_raw_data_exposed_rows || 0) === 0, JSON.stringify(verifierTrafficGate));
  ...
  check('external_evidence_contract', 'P0 verifier counts row-level source path evidence', Number(verifierResult.external_traffic_evidence?.platforms?.meituan?.row_source_path_rows || 0) > 0 && Number(verifierResult.external_traffic_evidence?.platforms?.meituan?.row_source_path_structured_rows || 0) > 0, JSON.stringify(verifierResult.external_traffic_evidence || {}));
  check('external_evidence_contract', 'P0 verifier counts raw_data field facts without raw_data exposure', Number(verifierResult.external_traffic_evidence?.platforms?.meituan?.raw_data_field_facts_rows || 0) > 0 && Number(verifierResult.external_traffic_evidence?.platforms?.meituan?.raw_data_exposed_rows || 0) === 0, JSON.stringify(verifierResult.external_traffic_evidence || {}));
...
check('traffic_evidence_contract', 'P0 verifier exposes pre-import evidence status without marking closure complete', p0VerifierSource.includes('pre_import_evidence_status') && p0VerifierSource.includes('valid_external_evidence_not_ingested') && p0VerifierSource.includes('pre_import_evidence_policy') && p0VerifierSource.includes('pre-import evidence policy') && p0VerifierSource.includes('pre-import evidence') && p0VerifierSource.includes('not P0 complete until target-date traffic rows are ingested') && p0VerifierSource.includes('external_evidence_row_source_path_rows') && p0VerifierSource.includes('external_evidence_raw_data_field_facts_rows'));
check('traffic_evidence_contract', 'P0 verifier exposes payload candidate dry-run evidence diagnostics without raw payload content', p0VerifierSource.includes('p0_hotel_scoped_payload_evidence_diagnostics') && p0VerifierSource.includes('payload_candidate_evidence_source_path_rows') && p0VerifierSource.includes('payload_candidate_evidence_structured_source_path_rows') && p0VerifierSource.includes('payload_candidate_evidence_raw_data_field_facts_rows') && p0VerifierSource.includes('payload_candidate_evidence_raw_data_exposed_rows') && p0VerifierSource.includes('payload_candidate_evidence_sensitive_value_rows') && p0VerifierSource.includes('payload_candidate_evidence_metric_keys') && p0VerifierSource.includes('payload_candidate_evidence_missing_metric_keys'));
...
check('traffic_evidence_contract', 'P0 verifier does not treat profile directory presence as verified login', p0VerifierSource.includes('traffic_current_session_verified_count') && p0VerifierSource.includes('current_session_verified') && p0VerifierSource.includes('historical login metadata is reference only'));

const noSourceRowsVerifierResult = runP0VerifierSnapshot('ctrip', '1900-01-02');
