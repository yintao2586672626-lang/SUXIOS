...
check('traffic_evidence_contract', 'importer emits traffic evidence UI status with desensitized evidence and structured source path counts', importerSource.includes("'ui_status' => p0_import_external_ui_status") && importerSource.includes('desensitized_capture_evidence_count') && importerSource.includes('structured_source_path_count'));
...
check('traffic_evidence_contract', 'P0 verifier labels non-traffic source rows as reference only', p0VerifierSource.includes('source_chain_reference_only') && p0VerifierSource.includes('reference_only_non_traffic_source_rows') && p0VerifierSource.includes('Source field facts are non-traffic reference evidence'));
...
function trafficRow({ hotelId = '', poiId = '', date: rowDate = '', dataDate = '', dateSource = '', trace, hash: sourceHash, sourcePath = 'data.flowData.0' }) {
  return {
    ...(hotelId ? { hotelId } : {}),
    ...(poiId ? { poiId } : {}),
