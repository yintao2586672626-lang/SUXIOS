...
if (!runtimeAssetPaths.includes('app-startup-helpers.min.js')
  ...
  if (!revenueAiStaticContent.includes('window.SUXI_REVENUE_AI_STATIC')
    ...
    || !content.includes('if (operationCanReconcileExecution(item)) {')
  ...
  if (!content.includes('const scheduleCtripHotelConfigApply = (event = null, options = {}) => {')
    ...
    || !content.includes('const selectPlatformHotelOption = (hotel) => {')
    || !content.includes('handlePlatformHotelContextChange({ target: { value: hotelId } });')
  ...
  if (!content.includes('const ctripTargetHotelOptions = computed(() => {')
    ...
    || !content.includes('data-testid="platform-hotel-context-search"')
    || !content.includes('role="combobox"')
    || !content.includes('aria-controls="platform-hotel-context-options"')
  ...
  if (!content.includes('const autoFetchStatusRequestPromises = new Map();')
    ...
    || !content.includes("const requestScopeKey = `${requestSession.epoch}:${requestHotelId}`;")
    || !content.includes("const requestKey = `${requestScopeKey}|${includeDetail ? 'full' : 'light'}`;")
    || !content.includes("if (!force && !includeDetail) {")
    || !content.includes('if (cached && cached.expiresAt > Date.now() && cached.data) {')
    || !content.includes('return cached.data;')
    ...
    || !content.includes('autoFetchStatusResultCache.set(requestKey, {')
    || !content.includes('expiresAt: Date.now() + AUTO_FETCH_STATUS_RESULT_CACHE_TTL_MS,')
    || !content.includes('data: nextStatus,')
    || !content.includes('if (!isCurrentHotel()) return nextStatus;')) {
