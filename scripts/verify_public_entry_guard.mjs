...
if (!fs.existsSync(indexPath)) {
  ...
  const operationWorkflowContent = `${appMainContent}\n${operationStaticContent}`;
...
if (!runtimeAssetPaths.includes('app-startup-helpers.min.js')
  ...
  if (!content.includes('// AI_DAILY_REPORT_TASK_HELPERS_START')
    ...
    || !operationWorkflowContent.includes('`/ai-daily-reports/tasks/${encodeURIComponent(taskId)}`')
    || !operationWorkflowContent.includes('const readAiDailyReportById = async (reportId, expectedHotelId) => {')
    || !operationWorkflowContent.includes('`/ai-daily-reports/${normalizedReportId}`')
    || !operationWorkflowContent.includes('pollResult.task.resultReportId')
    || !operationWorkflowContent.includes('AI经营日报回读酒店范围不一致')
    || !operationWorkflowContent.includes('if (!responseTaskId) {')
  ...
  if (!revenueAiStaticContent.includes('window.SUXI_REVENUE_AI_STATIC')
    ...
    || !content.includes('const revenueAiBusinessDate = computed(() => revenueAiResolveBusinessDate({')
    || !content.includes('selectedDate: priceSuggestionFilter.value.date,')
    ...
    || !operationWorkflowContent.includes('await recordOperationExecutionEvidence(taskItem);')
    ...
    || !operationWorkflowContent.includes('await recordOperationRoiEvidence(taskItem);')
    || !operationWorkflowContent.includes('await reviewOperationExecutionTask(taskItem);')
    || !operationWorkflowContent.includes('const parseOptionalOperationEvidenceNumber = (value, label) => {')
    || !operationWorkflowContent.includes('const normalizeOperationReviewStatus = (value) => {')
    || !operationWorkflowContent.includes('const recordOperationRoiEvidence = async (item) => {')
    || !operationWorkflowContent.includes("evidence_type: 'manual_roi_evidence'")
    || !operationWorkflowContent.includes("source: 'revenue_ai_effect_review_input'")
    || !operationWorkflowContent.includes("evidence_boundary: 'local_manual_roi_evidence_no_ota_write'")
    || !operationWorkflowContent.includes('result_status: resultStatus')
    || !operationWorkflowContent.includes('result_summary: resultSummary || \'继续观察，等待次日收益或ROI证据\'')
    || !operationWorkflowContent.includes('复盘结论为达成/接近达成/未达成时必须填写说明')
    || !operationWorkflowContent.includes("evidence_type: 'manual_price_execution'")
    || !operationWorkflowContent.includes("evidence_boundary: 'local_manual_evidence_no_ota_write'")
    || !operationWorkflowContent.includes('const operationEvidenceLocalTimestamp = () => {')
    || !operationWorkflowContent.includes('执行前后收入需同时填写或都留空')
    || !operationWorkflowContent.includes('current_value: operationEvidenceCleanObject({ ...currentValue, executed_before_price: beforePrice })')
    || !operationWorkflowContent.includes('target_value: operationEvidenceCleanObject({ ...targetValue, executed_after_price: afterPrice })')
  ...
  if (compassPageGuardCount < 2
    ...
    || !/loadCompetitorSummary\(\{\s*requireCompass:\s*true(?:,\s*force:\s*true)?\s*\}\)/.test(content)) {
  ...
  if (!content.includes('const scheduleDownloadCenterTabLoad = (tab, context = {}) => {')
    ...
    || !content.includes('switchToMeituanDownloadCenter, openMeituanStoredBusinessDate, openMeituanStoredDataTab, queryMeituanStoredData, meituanDownloadData,')
  ...
  if (!content.includes("requireSystemStatic('createHotelForm')")
    ...
    || !content.includes('...createHotelForm({ hotel, operatorName, parsedDescription }),')
  ...
  if (!content.includes('const DATA_HEALTH_SECONDARY_PANEL_DELAY_MS = 900;')
    ...
    || !/if \(newPage !== 'online-data'\) \{[\s\S]{0,420}?clearDataHealthSecondaryPanelsReadyTimer\(\);\s+dataHealthSecondaryPanelsReady\.value = false;/.test(content)
