...
function validateTrafficSourceReadiness(label, questionOrEvidence) {
  ...
  for (const sourceRow of rows) {
    ...
    const latestSyncCodeCounts = sourceRow?.traffic_latest_sync_task_message_code_counts && typeof sourceRow.traffic_latest_sync_task_message_code_counts === 'object' && !Array.isArray(sourceRow.traffic_latest_sync_task_message_code_counts)
      ...
      : {};
    const latestSyncAllowedCodes = ['task_status_missing', 'sync_running', 'stale_running', 'sync_task_target_date_mismatch', 'sync_reported_saved_rows_requires_target_date_verifier', 'sync_normalized_without_saved_rows', 'sync_completed_without_saved_rows', 'login_or_profile_not_ready', 'waiting_config', 'browser_dependency_missing', 'no_rows_parsed', 'capture_failed', 'unknown'];
