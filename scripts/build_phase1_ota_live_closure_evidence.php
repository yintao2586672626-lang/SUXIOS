...
function evidence_scope_details(array $data, array $options): array
{
    ...
    $collectCandidates = static function (array $paths, callable $normalize) use ($data): array {
        $values = [];
        foreach ($paths as $path) {
            $value = array_value($data, [$path], null);
            $normalized = $normalize($value);
            if ($normalized !== null && !in_array($normalized, $values, true)) {
                $values[] = $normalized;
            }
        }
        return $values;
    ...
    $dateCandidates = $collectCandidates([
        'scope.date', 'diagnosis.scope.date', 'operation.scope.date', 'scope_date', 'target_date', 'date',
        'requested_date_range.start_date', 'requested_date_range.end_date',
        'requested_date_range.start', 'requested_date_range.end',
        'date_range.start_date', 'date_range.end_date',
        'date_range.start', 'date_range.end',
        'diagnosis.requested_date_range.start_date', 'diagnosis.requested_date_range.end_date',
        'diagnosis.requested_date_range.start', 'diagnosis.requested_date_range.end',
        'diagnosis.date_range.start_date', 'diagnosis.date_range.end_date',
        'diagnosis.date_range.start', 'diagnosis.date_range.end',
        'operation.requested_date_range.start_date', 'operation.requested_date_range.end_date',
        'operation.requested_date_range.start', 'operation.requested_date_range.end',
        'operation.date_range.start_date', 'operation.date_range.end_date',
        'operation.date_range.start', 'operation.date_range.end',
    ...
    foreach ([
        'scope.system_hotel_id', 'diagnosis.scope.system_hotel_id', 'operation.scope.system_hotel_id',
        'scope_system_hotel_id', 'system_hotel_id', 'hotel.system_hotel_id', 'hotel.id',
        'diagnosis.system_hotel_id', 'operation.system_hotel_id',
    ] as $path) {
        $value = array_value($data, [$path], null);
        if ($value === null || $value === '') {
            continue;
    ...
    $platformCandidates = $collectCandidates([
        'scope.platform', 'diagnosis.scope.platform', 'operation.scope.platform',
        'scope_platform', 'platform', 'diagnosis.platform', 'operation.platform',
    ], static function ($value): ?string {
        $normalized = strtolower(trim((string)($value ?? '')));
        return $normalized !== '' ? $normalized : null;
    });
    $scopeDate = (string)($dateCandidates[0] ?? '');
    $scopeSystemHotelId = (int)($systemHotelCandidates[0] ?? 0);
    $scopePlatform = (string)($platformCandidates[0] ?? '');
    $scopeDateStatus = $dateCandidates === []
        ? 'missing'
        : (count($dateCandidates) > 1 ? 'conflict' : ($scopeDate === $expectedDate ? 'matched' : 'mismatch'));
    $scopeSystemHotelStatus = $systemHotelInvalid
        ? ($systemHotelCandidates === [] ? 'mismatch' : 'conflict')
        : ($systemHotelCandidates === [] || $expectedSystemHotelId <= 0
            ? 'missing'
            : (count($systemHotelCandidates) > 1 ? 'conflict' : ($scopeSystemHotelId === $expectedSystemHotelId ? 'matched' : 'mismatch')));
    $scopePlatformStatus = $platformCandidates === [] || $expectedPlatform === ''
        ? 'missing'
        : (count($platformCandidates) > 1 ? 'conflict' : ($scopePlatform === $expectedPlatform ? 'matched' : 'mismatch'));
    $scopeIdentityStatus = $scopeDateStatus === 'matched'
        && $scopeSystemHotelStatus === 'matched'
        && $scopePlatformStatus === 'matched'
        ? 'matched'
        : 'mismatch';

    $scope['date'] = $scopeDate !== '' ? $scopeDate : null;
    $scope['system_hotel_id'] = $scopeSystemHotelId > 0 ? $scopeSystemHotelId : null;
    $scope['platform'] = $scopePlatform !== '' ? $scopePlatform : null;
    ...
    return [
        ...
        'scope_system_hotel_id' => $scopeSystemHotelId > 0 ? $scopeSystemHotelId : null,
        'expected_scope_system_hotel_id' => $expectedSystemHotelId > 0 ? $expectedSystemHotelId : null,
        'scope_system_hotel_status' => $scopeSystemHotelStatus,
        'scope_system_hotel_candidates' => $systemHotelCandidates,
        'scope_system_hotel_invalid' => $systemHotelInvalid,
        'scope_platform' => $scopePlatform !== '' ? $scopePlatform : null,
        'expected_scope_platform' => $expectedPlatform !== '' ? $expectedPlatform : null,
        'scope_platform_status' => $scopePlatformStatus,
        'scope_platform_candidates' => $platformCandidates,
        'scope_identity_status' => $scopeIdentityStatus,
    ];
}
...
function evidence_scope_blocking_code(array $payload): string
{
    if ((string)($payload['scope_date_status'] ?? '') !== 'matched') {
        return 'evidence_scope_date_mismatch';
    }
    if ((string)($payload['scope_system_hotel_status'] ?? '') !== 'matched') {
        return 'evidence_scope_system_hotel_mismatch';
    }
    if ((string)($payload['scope_platform_status'] ?? '') !== 'matched') {
        return 'evidence_scope_platform_mismatch';
    }
    return '';
}
...
function normalize_diagnosis(array $payload, array $options): array
{
    ...
    $normalized = array_merge([
        ...
        'decision_status' => array_value($data, ['decision_status', 'diagnosis.decision_status'], null),
        'analysis_runtime' => array_value($data, ['analysis_runtime', 'diagnosis.analysis_runtime'], []),
        'saved_record' => array_value($data, ['saved_record', 'diagnosis.saved_record'], []),
    ], evidence_scope_details($data, $options));
    $normalized['persistence_proof'] = (new OtaDiagnosisPersistenceProofService())->verify($normalized, [
...
function inspect_platform(string $platform, array $columns, array $options): array
{
    ...
    $calculationStatus = trim((string)($metrics['status'] ?? ''));
    $revenueStatus = ($revenueAiReadiness['revenue_ready'] ?? false) === true
        ? 'ready'
        : ($calculationStatus === 'ready' ? 'blocked' : ($calculationStatus !== '' ? $calculationStatus : 'empty'));
    ...
    return [
        ...
        'revenue_metrics' => [
            ...
            'calculation_status' => $calculationStatus !== '' ? $calculationStatus : 'unknown',
            ...
            'credibility_gate' => $metrics['credibility_gate'] ?? [],
        ],
    ];
}
...
function platform_row_counts(array $platformEvidence): array
{
    return array_values(array_map(static function (array $item): array {
        $latestAvailable = is_array($item['source_rows']['latest_available'] ?? null)
            ...
            : [];
        $fieldFacts = is_array($item['field_facts'] ?? null) ? $item['field_facts'] : [];
        return [
...
function data_gap_codes(array $platformEvidence, array $diagnosis): array
{
    ...
    if (evidence_scope_aligned($diagnosis)
...
function platform_evidence_missing_codes(array $platformEvidence): array
{
    ...
    foreach ($platformEvidence as $item) {
        ...
        $downstreamReadiness = is_array($item['revenue_metrics']['downstream_readiness'] ?? null)
            ? $item['revenue_metrics']['downstream_readiness']
            : [];
        if (($downstreamReadiness['revenue_ready'] ?? false) === true
            && ($downstreamReadiness['ai_ready'] ?? false) !== true
        ) {
            $codes[] = $platform . '_ai_input_not_ready';
        }
        $fieldFacts = is_array($item['field_facts'] ?? null) ? $item['field_facts'] : [];
        if ((int)($item['source_rows']['count'] ?? 0) > 0 && (int)($fieldFacts['fact_count'] ?? 0) === 0) {
            $codes[] = $platform . '_field_facts_missing';
        }
        if ((int)($item['source_rows']['count'] ?? 0) > 0
            && (int)($fieldFacts['fact_count'] ?? 0) > 0
            && ((int)($fieldFacts['incomplete_captured_fact_count'] ?? 0) > 0 || (int)($fieldFacts['complete_fact_count'] ?? 0) === 0)
        ) {
            $codes[] = $platform . '_field_fact_closure_incomplete';
        }
...
function build_blocked_diagnosis_from_platform_evidence(array $platformEvidence, array $options): array
{
    ...
    foreach ($platformEvidence as $item) {
        ...
        $evidenceSources[] = [
            ...
            'ai_status' => (string)($item['revenue_metrics']['ai_status'] ?? 'unknown'),
            'traffic_rows' => (int)($item['revenue_metrics']['traffic']['rows'] ?? 0),
    ...
    return [
        ...
        'scope_system_hotel_status' => (int)($options['system_hotel_id'] ?? 0) > 0 ? 'matched' : 'missing',
        'scope_platform_status' => trim((string)($options['platform'] ?? '')) !== '' ? 'matched' : 'missing',
        'scope_identity_status' => (int)($options['system_hotel_id'] ?? 0) > 0
            && trim((string)($options['platform'] ?? '')) !== ''
            ? 'matched'
            : 'mismatch',
...
{
    return diagnosis_content_present($diagnosis) && evidence_scope_aligned($diagnosis);
}

function diagnosis_persistence_proved(array $diagnosis): bool
{
    return ($diagnosis['persistence_proof']['proved'] ?? false) === true;
}
...
{
    return has_diagnosis_evidence($diagnosis)
        && actionable_diagnosis_action_count($diagnosis) > 0
        && diagnosis_persistence_proved($diagnosis);
}
...
function diagnosis_blocking_code(array $diagnosis): string
{
    ...
    $actionableCount = actionable_diagnosis_action_count($diagnosis);
    if ($actionableCount <= 0) {
        return 'ai_diagnosis_action_items_blocked';
    }

    $scopeBlocker = evidence_scope_blocking_code($diagnosis);
    if ($scopeBlocker !== '') {
        return $scopeBlocker;
    }
    if (!diagnosis_persistence_proved($diagnosis)) {
        return 'ai_diagnosis_persistence_unverified';
    }
    return '';
}
...
function operation_evidence_status(array $operation): string
{
    if (operation_payload_signal_count($operation) > 0) {
        ...
        if (!evidence_scope_aligned($operation)) {
            return 'warning';
        }
...
function operation_blocking_code(array $operation): string
{
    ...
    $scopeBlocker = evidence_scope_blocking_code($operation);
    if ($scopeBlocker !== '') {
        return $scopeBlocker;
    }
...
function metric_domain_readiness(array $platformEvidence, string $targetDate = ''): array
{
    return array_values(array_map(static function (array $item) use ($targetDate): array {
        ...
        $aiReady = (string)($item['revenue_metrics']['ai_status'] ?? '') === 'ready';
        ...
        return [
            ...
            'ai_status' => $aiReady ? 'ready' : 'missing',
...
function traffic_source_readiness(array $domainReadiness): array
{
    $platforms = [];
    ...
    foreach ($platforms as $platform => $context) {
        $result[] = traffic_source_readiness_for_platform($platform, $context);
    }
    return $result;
}
 ...
 */
function traffic_source_run_process(array $args, string $cwd): array
{
...
function traffic_source_p0_payload_dry_run_defaults(): array
{
    return [
 ...
 */
function traffic_source_p0_payload_evidence_diagnostics(array $trafficEvidence, array $summary): array
{
    $result = traffic_source_p0_payload_dry_run_defaults();
    $metricKeys = [];
    foreach ($trafficEvidence as $row) {
        if (!is_array($row)) {
            continue;
        }
        if (trim((string)($row['source_path'] ?? '')) !== '') {
...
function traffic_source_p0_payload_importer_dry_run(string $platform, string $targetDate, int $systemHotelId, string $absolutePayloadPath): array
{
    global $root;
    ...
    $run = traffic_source_run_process([
        PHP_BINARY,
    ...
    $result = array_merge($result, traffic_source_p0_payload_evidence_diagnostics($trafficEvidence, $summary));
...
function traffic_source_p0_payload_candidate(string $platform, string $targetDate, int $systemHotelId): array
{
    ...
    $absolutePayloadPath = resolve_path($payloadPath);
    $dryRun = $present ? traffic_source_p0_payload_importer_dry_run($platform, $targetDate, $systemHotelId, $absolutePayloadPath) : traffic_source_p0_payload_dry_run_defaults();
...
function traffic_source_latest_sync_task_summary(int $dataSourceId, string $targetDate): array
{
    if ($dataSourceId <= 0) {
    ...
    $taskTargetDate = traffic_source_sync_task_target_date($stats);
    return [
        'status' => traffic_source_effective_sync_task_status($task),
        'target_date' => $targetDate,
        'task_target_date' => $taskTargetDate,
        'target_date_matches_task' => $taskTargetDate === '' ? null : $taskTargetDate === $targetDate,
        'message_code' => traffic_source_sync_task_message_code($task, $stats, $targetDate),
...
function traffic_source_sync_task_target_date(array $stats): string
{
    ...
    foreach ([
        ...
        $date = traffic_source_normalize_task_date($value);
        if ($date !== '') {
            return $date;
        }
    }
...
function traffic_source_normalize_task_date($value): string
{
...
function traffic_source_sync_task_message_code(array $task, array $stats, string $targetDate): string
{
    ...
    if (traffic_source_sync_task_is_stale_running($task)) {
    ...
    if ($status === 'waiting_config') {
        return traffic_source_sync_task_message_looks_like_login_blocker($message) ? 'login_or_profile_not_ready' : 'waiting_config';
    }
    if (in_array($status, ['failed', 'capture_failed'], true)) {
        ...
        if (traffic_source_sync_task_message_looks_like_login_blocker($message)) {
...
function traffic_source_effective_sync_task_status(array $task): string
{
    $status = strtolower(trim((string)($task['status'] ?? 'unknown')));
    return traffic_source_sync_task_is_stale_running($task) ? 'stale_running' : $status;
}

function traffic_source_sync_task_is_stale_running(array $task): bool
{
    $status = strtolower(trim((string)($task['status'] ?? '')));
    if (!in_array($status, ['pending', 'queued', 'running', 'browser_opened', 'syncing', 'syncing_after_login'], true)) {
        return false;
    ...
    $ageSeconds = traffic_source_sync_task_age_seconds($task);
    return $ageSeconds !== null && $ageSeconds > 3600;
}

function traffic_source_sync_task_age_seconds(array $task): ?int
{
...
function traffic_source_sync_task_message_looks_like_login_blocker(string $message): bool
{
...
function traffic_source_accumulate_latest_sync_task(array &$summary, array $task): void
{
...
function traffic_source_readiness_for_platform(string $platform, array $context): array
{
    ...
    foreach ($rows as $row) {
        ...
        traffic_source_accumulate_latest_sync_task(
            $base,
            traffic_source_latest_sync_task_summary((int)($row['id'] ?? 0), $targetDate)
        );

        $currentSessionVerified = traffic_source_profile_login_state_verified($row);
        $profileLoginTrigger = traffic_source_profile_login_trigger_action($platform, (int)($row['id'] ?? 0), $systemHotelId, $targetDate);
        ...
        if (($config['registered_by'] ?? '') === 'p0_ota_field_loop') {
            ...
            $candidate = traffic_source_p0_payload_candidate($platform, $targetDate, $systemHotelId);
    ...
    foreach (array_values(array_diff($expectedHotelIds, $candidateHotelIds)) as $systemHotelId) {
        OtaP0ScopeProjectionService::accumulatePayloadCandidate(
            $base,
            traffic_source_p0_payload_candidate($platform, $targetDate, $systemHotelId)
        );
    }
...
function traffic_source_p0_standard_fact_summary(array $requiredMetricKeys, array $requiredStorageFields, array $fieldLoopMatrix, int $targetDateTrafficRows): array
{
    $statusCounts = [];
    $completeMetricKeys = [];
    $missingMetricKeys = [];
...
function traffic_source_latest_sync_task_text(array $source): string
{
...
function traffic_source_readiness_text(array $source): string
{
    ...
    $latestSyncSuffix = traffic_source_latest_sync_task_text($source);
    $entryParts = [];
...
function traffic_input_contract(string $platform, string $mode): array
{
    $platform = strtolower(trim($platform));
    $mode = strtolower(trim($mode));
...
function traffic_acceptance_contract(): array
{
    return [
...
function traffic_entry_options_with_readiness(string $platform, array $options): array
{
    $platform = strtolower(trim($platform));
    ...
    foreach ($indexed as &$option) {
        ...
        $contract = traffic_input_contract($platform, (string)($option['mode'] ?? ''));
        if ($contract !== []) {
            $option['input_contract'] = $contract;
            $option['acceptance_contract'] = traffic_acceptance_contract();
        }
    }
    unset($option);
    return entry_options_with_readiness($platform, $indexed);
}
...
function next_action_entry_options(string $code): array
{
    if (str_contains($code, 'etl_not_ready')
        ...
        return entry_options_with_readiness($platform, [[
            'mode' => 'status_check',
            'label' => '收益/AI 可信度状态核对',
    ...
    if (str_contains($code, 'traffic_facts_missing')) {
        if (str_starts_with($code, 'ctrip_')) {
            return traffic_entry_options_with_readiness('ctrip', [
                [
        ...
        if (str_starts_with($code, 'meituan_')) {
            return traffic_entry_options_with_readiness('meituan', [
                [
                    'mode' => 'browser_profile',
                    'label' => '浏览器 Profile',
...
function next_action_success_criteria(string $code): string
{
    ...
    if ($code === 'align_evidence_scope_date') {
        return '证据 JSON 的分段 scope.system_hotel_id、scope.platform、scope.date 与本次巡检完全一致。';
    }
...
function next_action_resolves_missing_codes(string $code): array
{
    ...
    if ($code === 'align_evidence_scope_date') {
        return ['evidence_scope_date_mismatch', 'evidence_scope_system_hotel_mismatch', 'evidence_scope_platform_mismatch'];
    }
    return [];
}
...
function next_action_for_blocker_code(string $code): string
{
    ...
    if (in_array($code, ['evidence_scope_date_mismatch', 'evidence_scope_system_hotel_mismatch', 'evidence_scope_platform_mismatch'], true)) {
...
function build_next_actions(array $platformEvidence, array $diagnosis, array $operation, array $options): array
{
    ...
    foreach ($platformEvidence as $item) {
        ...
        if ((int)($item['source_rows']['count'] ?? 0) === 0) {
            $actions[] = next_action_row(
                ...
                'missing',
                '酒店运营人员',
                '默认使用现有' . $label . '浏览器 Profile 采集入口补齐 ' . (string)$options['date'] . ' 的 OTA 数据，然后重新运行真实闭环巡检；手动 Cookie/API 仅作临时补数或排障。',
        ...
        if (($downstreamReadiness['revenue_ready'] ?? false) === true
            ...
            $actions[] = next_action_row(
                $platform . '_ai_input_not_ready_review_credibility_gate',
                'high',
                'missing',
                'AI/收益运营人员',
                '收益指标可复核，但 ' . $label . ' 当前未通过 AI 决策输入门；先核对阻断原因，解除前不得生成可执行经营建议。',
                ['credibility_gate.status', 'decision_use.ai_decision_support.allowed', 'ai_reason_codes'],
                '只复核可信度与治理状态，不用收益可用性替代 AI 可用性，不自动创建或执行运营动作。'
            );
        }
        $fieldFacts = is_array($item['field_facts'] ?? null) ? $item['field_facts'] : [];
        if ((int)($item['source_rows']['count'] ?? 0) > 0 && (int)($fieldFacts['fact_count'] ?? 0) === 0) {
            $actions[] = next_action_row(
                $platform . '_field_facts_missing_verify_capture_evidence',
                'high',
                'missing',
                'Product/Engineering',
                'Verify existing ' . $label . ' target-date rows and write field_facts metadata without changing acquisition logic.',
                ['raw_data.field_facts or raw_data.facts', 'capture_evidence', 'metric_key', 'source_path', 'storage_field'],
                'Read existing online_daily_data only; do not expose raw_data values or invent fallback mappings.'
            );
        }
        if ((int)($item['source_rows']['count'] ?? 0) > 0
            && (int)($fieldFacts['fact_count'] ?? 0) > 0
            ...
            $actions[] = next_action_row(
                $platform . '_field_fact_closure_incomplete_complete_mapping',
                'high',
                'missing',
                'Product/Engineering',
    ...
    if (array_intersect([
        ...
        $actions[] = next_action_row(
            ...
            'missing',
            '产品/技术',
            '重新生成同系统酒店、同平台、同目标日的 AI 诊断/运营执行证据 JSON。',
            ['scope.system_hotel_id', 'scope.platform', 'scope.date=' . (string)$options['date'], 'metric_scope=ota_channel'],
            '不复用其他门店、其他平台或错日期证据证明当前 OTA 闭环。'
        );
    }
    ...
    } elseif (actionable_diagnosis_action_count($diagnosis) === 0) {
        $actions[] = next_action_row(
    ...
    } elseif (evidence_scope_aligned($diagnosis)
        && actionable_diagnosis_action_count($diagnosis) > 0
        && !diagnosis_persistence_proved($diagnosis)
    ) {
        $actions[] = next_action_row(
            ...
            'missing',
            'AI 运营人员',
            '诊断内容存在，但必须按 saved_record.id 在 agent_logs 中完成同门店、同平台、同目标日实时回读后才可进入闭环。',
            ['saved_record.id', 'agent_logs.hotel_id/action/context_data', 'platform', 'requested_date_range', 'database readback proof'],
            '外部 JSON 的 saved/readback 布尔值不能单独作为持久化证明。',
    ...
    if ($operationStatus !== 'proved') {
        ...
        $actions[] = next_action_row(
            ...
            match ($operationBlockingCode) {
                ...
                'operation_execution_evidence_incomplete' => '已有执行意图/执行流程样例，但还不能证明动作已落地；必须同时有执行证据和已执行、复盘或 ROI 状态。',
                default => '创建或附上一个真实执行意图/执行流程样例，并关联到 OTA 诊断动作项。',
            },
            $operationBlockingCode === 'operation_execution_ai_action_link_missing'
                ? ['source_module=ota_diagnosis 或 source=ota_diagnosis#action_item', 'evidence_refs 或 action_item_id', 'evidence.count>0，且 execution.status=executed、review.status 或 ROI 状态']
                : ['execution_intents 或 execution_flow', 'evidence.count>0', 'execution.status=executed、review.status 或 ROI 复盘状态'],
            $operationBlockingCode === 'operation_execution_ai_action_link_missing'
                ? '只补齐执行证据关联，不改携程/美团采集字段和采集逻辑。'
                : '审批通过只代表允许执行；仅声明已执行也不够，必须有执行证据和落地状态。',
            $blockedBy
        );
    }
...
function evidence_metric_domain_summary(array $metricDomainReadiness, array $trafficSourceReadiness = []): array
{
    ...
    foreach ($metricDomainReadiness as $row) {
        ...
        $summary[] = [
            ...
            'traffic_source_text' => $trafficSource !== [] ? traffic_source_readiness_text($trafficSource) : '',
            'traffic_source_next_action' => $trafficSource !== [] ? traffic_source_next_action_text($trafficSource) : '',
...
function build_employee_questions(array $platformEvidence, array $diagnosis, array $operation, string $targetDate = ''): array
{
    ...
    $effectiveDiagnosisProved = $diagnosisStatus === 'proved' && $aiUpstreamBlockers === [];
    $effectiveDiagnosisStatus = $effectiveDiagnosisProved
        ? 'proved'
        : ($diagnosisStatus === 'proved' ? 'warning' : $diagnosisStatus);
    ...
    $aiQuestionBlockers = $aiUpstreamBlockers !== []
        ? $aiUpstreamBlockers
        : ($diagnosisStatus === 'proved'
            ? []
            : ($aiBlockers !== [] ? $aiBlockers : array_values(array_filter([$aiDiagnosisBlocker ?: 'ai_diagnosis_evidence_sample_missing']))));
    $aiPersistenceUnverified = in_array('ai_diagnosis_persistence_unverified', $aiQuestionBlockers, true);
    ...
    $fieldFactGapCodes = array_values(array_filter(
        evidence_missing_codes($platformEvidence, $diagnosis, $operation),
    ...
    $trustedFieldsStatus = $fieldFactGapCodes !== []
        ? 'warning'
        : ($coverageStatus === 'complete' && count($metricTrustKeys) > 0 ? 'proved' : ($sourceRows > 0 && count($metricTrustKeys) > 0 ? 'warning' : 'not_proved_no_source_rows'));
    ...
    return [
        [
            ...
            'next_action' => $coverageStatus === 'complete' ? '' : '默认使用携程/美团浏览器 Profile 采集入口补齐缺失平台同日数据后重新巡检；手动 Cookie/API 仅作临时补数或排障：' . ($missingPlatformText !== '' ? $missingPlatformText : '携程/美团'),
        ],
        [
            'key' => 'trusted_fields',
            'question' => '哪些字段可信',
        ...
        [
            ...
            'evidence' => [
                ...
                'metric_domain_summary' => evidence_metric_domain_summary($domainReadiness, $trafficSourceReadiness),
        ...
        [
            'key' => 'ai_evidence',
            'question' => 'AI 建议依据是什么',
            'status' => $effectiveDiagnosisStatus,
            'evidence' => [
                'diagnosis_status' => $effectiveDiagnosisProved
                    ? 'proved'
                ...
                'action_item_status' => $effectiveDiagnosisProved && $actionableDiagnosisCount > 0
                    ? 'actionable'
                    : ($aiPersistenceUnverified && $actionableDiagnosisCount > 0
                        ? 'actionable_persistence_unverified'
                        : ($aiHasExistingBlockerEvidence ? 'blocked_by_verified_ota_gaps' : 'missing')),
                'source_policy' => $effectiveDiagnosisProved
                ...
                'scope_system_hotel_status' => (string)($diagnosis['scope_system_hotel_status'] ?? 'missing'),
                'scope_platform_status' => (string)($diagnosis['scope_platform_status'] ?? 'missing'),
                'persistence_proof' => is_array($diagnosis['persistence_proof'] ?? null)
                    ? $diagnosis['persistence_proof']
                    : ['proved' => false, 'status' => 'missing'],
                'blocking_missing_codes' => $aiQuestionBlockers,
            ],
            'next_action' => $effectiveDiagnosisProved
                ? ''
        ...
        [
            ...
            'evidence' => [
                ...
                'scope_system_hotel_status' => (string)($operation['scope_system_hotel_status'] ?? 'missing'),
                'scope_platform_status' => (string)($operation['scope_platform_status'] ?? 'missing'),
                'blocking_missing_codes' => $operationBlockingCodes,
            ],
            'next_action' => $operationStatus === 'proved' ? '' : ($effectiveDiagnosisProved
 ...
 */
function closure_top_action_from_next_action(array $action, array $questions = [], array $fallback = []): array
{
    $code = (string)($action['action_code'] ?? '');
    if ($code === '') {
        return $fallback;
...
function build_closure_summary(array $platformEvidence, array $diagnosis, array $operation, ?array $questions = null, string $targetDate = '', ?array $nextActions = null): array
{
    ...
    if (is_array($nextActions[0] ?? null)) {
        $topAction = closure_top_action_from_next_action($nextActions[0], $questions, $topAction);
    }
    return array_merge([
    ...
    $evidence = [
        ...
        'collection_reliability' => [
            ...
            'platforms' => array_map(static fn(array $item): array => [
                ...
                'field_facts' => [
                    'status' => $item['field_facts']['status'] ?? 'not_loaded',
                    'fact_count' => $item['field_facts']['fact_count'] ?? 0,
                    'complete_fact_count' => $item['field_facts']['complete_fact_count'] ?? 0,
                    'incomplete_captured_fact_count' => $item['field_facts']['incomplete_captured_fact_count'] ?? 0,
                    'raw_data_exposed' => $item['field_facts']['raw_data_exposed'] ?? false,
                ],
            ], $platformEvidence),
        ],
        ...
        'closure_summary' => build_closure_summary($platformEvidence, $diagnosis, $operation, $employeeQuestions, (string)$options['date'], $nextActions),
