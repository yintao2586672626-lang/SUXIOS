...
const assessmentCatalog = {
  ...
  perf_auto: {
    status: '满足', basis: '自动化',
    evidence: 'verify:performance-budget 通过：index 6,345B、startup gzip 747,168B（预算 850,000B）、inline 4,172B、blocking scripts=0。',
...
const categories = [
  {
    ...
    cases: [
      ...
      ['停用账号登录', '将账号置为停用后尝试登录。', '返回禁止/待审核状态，不签发 Token，并记录失败原因。', 'P0'],
      ...
      ['注册确认密码一致性', '注册时提交不同 password 与 confirm_password。', '注册被拒绝，数据库不产生用户或酒店授权残留。', 'P0'],
      ['修改密码校验旧密码', '登录后使用错误 old_password 修改密码。', '修改失败，原密码仍有效，新密码无效。', 'P0'],
      ['修改密码执行新策略', '提交不符合长度/复杂度的新密码。', '修改失败，不绕过注册/建用户使用的密码策略。', 'P0'],
  ...
  },
  {
    ...
    cases: [
      ['全路由默认鉴权', '枚举 route/app.php 的业务路由并无 Token 访问。', '除明确公开的登录、注册、健康和受控采集接口外均要求认证。', 'P0'],
