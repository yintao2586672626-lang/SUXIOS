...
requireText('public/index.html', "review_collection_policy: 'policy_disabled'", 'Ctrip review order automation keeps review collection policy disabled');
requireText('public/index.html', '模板只用于字段结构；存在 `replace-with-*` 占位符时，系统会拒绝导入', 'Ctrip review order payload template warns that placeholders are rejected');
...
requireText('public/index.html', "copyCtripReviewMatchCliCommand('preflight')", 'Ctrip review order UI exposes the pure preflight CLI command');
...
requireText('public/index.html', "ctripReviewMatchResult.data.next_commands", 'Ctrip review order UI renders closure next commands');
requireText('public/index.html', "@click=\"copyToClipboard(command)\"", 'Ctrip review order UI lets the operator copy closure next commands');
...
requireText('public/index.html', '@click="runCtripReviewMatchPreflight"', 'Ctrip review order UI exposes page preflight-only action');
...
requireText('public/index.html', '@click="runCtripReviewMatchDryRun"', 'Ctrip review order UI exposes page dry-run matching action');
requireText('public/index.html', 'payload.dry_run = true;', 'Ctrip review order page dry-run action requests rollback mode');
requireText('public/index.html', "写入 {{ ctripReviewMatchResult.data.source_status.storage_write === false ? '否' : '是' }}", 'Ctrip review order UI exposes whether the action wrote storage');
requireText('public/index.html', '授权 payload 预检：{{ ctripReviewMatchResult.data.payload_preflight.status', 'Ctrip review order UI renders authorized payload preflight status');
...
requireText('public/index.html', "scheduleDelayedPageTask(() => {\n                            if (currentPage.value !== 'compass') return null;\n                            loadWeatherForCity();\n                            return null;\n                        }, COMPASS_WEATHER_REFRESH_DELAY_MS);", 'compass response delays weather and skips after leaving the compass page');
