...
ALTER TABLE `temporal_forecast_trials`
  ADD COLUMN IF NOT EXISTS `active_slot` TINYINT GENERATED ALWAYS AS (
    CASE WHEN `status` IN ('draft', 'pending_approval', 'running') THEN 1 ELSE NULL END
  ) STORED AFTER `status`,
  ADD UNIQUE INDEX IF NOT EXISTS `uniq_temporal_forecast_trial_active` (`tenant_id`, `system_hotel_id`, `active_slot`);
