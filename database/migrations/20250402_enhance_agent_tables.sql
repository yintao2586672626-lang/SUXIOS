...
ADD COLUMN IF NOT EXISTS `last_maintenance_date` DATE DEFAULT NULL COMMENT '上次维护日期' AFTER `last_maintenance`,
...
-- 插入默认能耗基准配置示例；按稳定业务键逐项幂等，迁移重复执行不得累加种子。
INSERT INTO `energy_benchmarks` (`hotel_id`, `energy_type`, `benchmark_type`, `benchmark_value`, `alert_threshold_high`, `alert_threshold_low`, `season_factor`, `remark`)
SELECT 0, 1, 1, 500.00, 20.00, 10.00, '{"spring":1.0,"summer":1.3,"autumn":1.0,"winter":1.2}', '默认日电耗基准'
WHERE NOT EXISTS (
    SELECT 1 FROM `energy_benchmarks` WHERE `hotel_id` = 0 AND `energy_type` = 1 AND `benchmark_type` = 1
);

INSERT INTO `energy_benchmarks` (`hotel_id`, `energy_type`, `benchmark_type`, `benchmark_value`, `alert_threshold_high`, `alert_threshold_low`, `season_factor`, `remark`)
SELECT 0, 2, 1, 50.00, 20.00, 10.00, '{"spring":1.0,"summer":1.1,"autumn":1.0,"winter":0.9}', '默认日水耗基准'
WHERE NOT EXISTS (
    SELECT 1 FROM `energy_benchmarks` WHERE `hotel_id` = 0 AND `energy_type` = 2 AND `benchmark_type` = 1
);

INSERT INTO `energy_benchmarks` (`hotel_id`, `energy_type`, `benchmark_type`, `benchmark_value`, `alert_threshold_high`, `alert_threshold_low`, `season_factor`, `remark`)
SELECT 0, 3, 1, 30.00, 20.00, 10.00, '{"spring":1.0,"summer":0.8,"autumn":1.0,"winter":1.4}', '默认日燃气基准'
WHERE NOT EXISTS (
    SELECT 1 FROM `energy_benchmarks` WHERE `hotel_id` = 0 AND `energy_type` = 3 AND `benchmark_type` = 1
);
