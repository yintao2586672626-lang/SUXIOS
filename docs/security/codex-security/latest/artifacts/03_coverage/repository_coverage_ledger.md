...
| R13 | Production release evidence | Release readiness | `review:release-readiness` fails on env, LLM attestation, design handoff, OTA rotation attestation, and security scan before this artifact. | needs_follow_up | This scan closes only the security-scan artifact blocker. |
