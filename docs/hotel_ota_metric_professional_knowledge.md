...
| 美团酒店商学院历史 HOS 资料 | 仅保留 HOS 的历史解释和可观察运营项 | 当前分制、周期、维度、权重与排名影响均须以当前后台为准，不再作为现行公式依据 |
| 美团酒店商家诚信经营制度及细则 | 诚信、服务和经营违规用于界定信息真实性、履约、评价、引流和价格风险 | 2024 诚信细则可作为较新规则基线；2023 服务/经营细则只作带版本证据，不把旧阈值默认成当前值 |
...
| 美团酒店服务规则拒单率（2023 版） | 订单提交后、美团确认预订成功前，商家拒绝确认、诱导取消或超时不确认的比例 | `拒单订单量 / 支付订单量`；仅对应 2023-04-10 生效的美团酒店服务细则 | 作为带版本的履约风险口径；当前公式、窗口、阈值和处罚执行前必须复核，不能扩展为全平台通用拒单率 |
...
| 美团 HOS | 平台展示的酒店经营综合信号；当前公开资料未给出现行完整公式 | 历史资料提到酒店信息、服务质量、经营产能、违规违约、预留房、点评和订单，但当前周期、分制、维度与权重未核验 | 字段可保留 `meituan_hos_score` 原始展示值、来源和日期；不得写死“满分 5 分”“近 28 天”或据此反推排名 |
