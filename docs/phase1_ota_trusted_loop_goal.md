...
真实闭环验收使用 `npm.cmd run inspect:phase1-live-closure` 读取当前数据库和脱敏接口证据；运行时动作队列守卫使用 `npm.cmd run verify:phase1-live-action-queue` 检查员工六问、闭环摘要和下一步动作队列可被消费；严格验收使用 `npm.cmd run verify:phase1-live-closure`，缺真实采集、AI 证据或运营执行样例时必须失败。
