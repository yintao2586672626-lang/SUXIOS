...
| `design-handoff-missing` | `@figma` / `@canva` | `npm run review:release-design` | `DESIGN_HANDOFF_MANIFEST_FILE` pointing to a controlled manifest outside the repo, or `docs/design_handoff_manifest.json` for local default review. | Fails until real design source handoff exists. | Manifest includes accessible Figma, Canva, Brand Kit, design token path, required flow coverage, review date, owner, and empty `open_issues`. |
