...
|---|---|---|---|---|---|
| `local-git-state-open` | `@github` | `npm run review:release-external-state` | Clean local Git state, absent `.git/index.lock`, PR metadata, PR checks, and `git ls-files database/backups` evidence. | Fails until local worktree and index state are closed. | Command passes, or `RELEASE_EXTERNAL_STATE_FILE` points to equivalent reviewed evidence. |
