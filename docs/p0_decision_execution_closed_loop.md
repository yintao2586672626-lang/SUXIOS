...
```text
授权 OTA 可见数据 -> 采集证据 -> 字段目录 -> 标准事实 -> 收益/流量/转化/竞争圈诊断 -> 待确认 AI 建议 -> 动作追踪 -> 效果复盘
```

| 模块 | 当前定位 |
|---|---|
| OTA 数据 | 主闭环数据入口，优先保证携程、美团、携程商旅授权可见数据可采集、可校验、可追溯。 |
| 收益诊断 | 主闭环分析层，按 OTA 渠道口径输出收入、订单、间夜、转化率等指标；ADR、RevPAR 只有分母完整时才进入全酒店口径。 |
| AI 能力 | 嵌入式能力，只用于解释、诊断和待确认建议生成，不作为独立产品线扩张。 |
| 运营管理 | 主闭环执行层，承接预警、策略模拟、动作追踪和复盘。 |
| 筹建/开业/扩张/转让/投资测算 | 二期辅助模块，保留入口和记录能力，暂不作为第一阶段深闭环。 |

## 0.1 系统思维约束

本闭环按“输入 -> 存量 -> 流量 -> 反馈 -> 延迟 -> 目标”设计：

| 维度 | 在本方案中的约束 |
|---|---|
| 输入 | OTA、日报、竞争圈/同圈、服务质量和执行证据必须保留来源、时间、口径和状态。 |
| 存量 | 评分、历史收入、策略沉淀、执行记录和客户信任不能被单日流量波动替代。 |
| 流量 | 曝光、访客、订单、间夜、收入、调价、促销和投诉需要按周期记录。 |
| 反馈 | AI建议必须进入审批、执行、证据和复盘链路，不能停留在文本建议。 |
| 延迟 | 调价、投放、服务优化和点评变化必须有观察期，避免即时误判。 |
| 目标 | 不只追求订单量，还要综合收入质量、利润、评分、转化和长期投资判断。 |

详细项目级构思见 `docs/system_design_logic.md`。
