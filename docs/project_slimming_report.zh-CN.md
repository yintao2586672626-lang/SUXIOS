...
| 项目 | 体积 | 处理策略 |
|---|---:|---|
| 完整 `HOTEL/` 目录 | 约 284 MB | 包含 `.git`、依赖、本地报告和项目代码；会随本地 Git 对象轻微波动。 |
| 不含 `.git` | 约 92.42 MB | 更接近工作副本体积。 |
| 不含 `.git`、`node_modules/`、`vendor/` | 约 63.23 MB | 更接近业务与资料体积。 |
| Git 跟踪文件 | 约 18.30 MB / 613 个文件 | 这是代码提交体积口径。 |
...
|---|---:|---|---|
| `public/index.html` | 1,396 个函数级块；44 个 `currentPage` 引用 | `triggerAutoFetch` 82 行、`fetchCtripFlowOverviewData` 72 行、`fetchCtripAdsData` 72 行、`generateOtaDiagnosis` 71 行、`fetchCtripOverviewData` 71 行、`importKnowledgeUnits` 68 行 | `general` 7,165 行、`ctrip` 3,032 行、`hotel_admin` 1,289 行、`ai` 1,159 行、`meituan` 1,148 行、`ota` 437 行 |
