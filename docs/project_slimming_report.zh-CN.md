...
| Git 跟踪文件 | 约 17.79 MB / 600 个文件 | 这是代码提交体积口径。 |
...
| 文件 | 行数 | 体积 | 当前本地改动 | 判断 |
|---|---:|---:|---|---|
| `public/index.html` | 42,521 | 3.04 MB | 本轮拆分中 | 当前前端 SPA 主入口；已先抽出扩张/市场测算静态选项数据到 `public/expansion-static-options.js`，抽出酒店图片优化/AI 工具箱静态选项到 `public/hotel-image-optimizer-static.js`，抽出收益研究中心静态产品清单到 `public/revenue-research-static.js`，抽出自动采集静态配置到 `public/auto-fetch-static.js`，抽出门店罗盘静态配置到 `public/compass-static.js`，并抽出模拟测算/转让字段静态配置到 `public/simulation-static.js`；后续继续按页面或面板拆分，同时保持 Vue CDN 运行契约。 |
...
|---|---:|---|---|
| `public/index.html` | 1,168 个函数级块；44 个 `currentPage` 引用 | `printFeasibilityReport` 342 行、`setOnlineDataTabFromPage` 304 行、`runOtaDiagnosisHotelFetch` 265 行 | `general` 10,842 行、`ctrip` 3,909 行、`ai` 1,516 行、`hotel_admin` 1,515 行、`meituan` 1,426 行 |
