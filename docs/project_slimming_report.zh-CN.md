...
| 项目 | 体积 | 处理策略 |
|---|---:|---|
| 完整 `HOTEL/` 目录 | 约 228.42 MB | 包含 `.git`、依赖、本地报告和项目代码。 |
| 不含 `.git` | 约 91.87 MB | 更接近工作副本体积。 |
| 不含 `.git`、`node_modules/`、`vendor/` | 约 62.68 MB | 更接近业务与资料体积。 |
...
`npm run self:audit` 已新增跟踪代码热点和拆分候选输出。当前拆分候选不是自动删除清单，只用于后续架构瘦身排序。

| 文件 | 行数 | 体积 | 判断 |
|---|---:|---:|---|
| `public/index.html` | 43,322 | 3.11 MB | 当前前端 SPA 主入口，后续应按页面或面板拆分，同时保持 Vue CDN 运行契约。 |
| `app/controller/OnlineData.php` | 31,140 | 1.43 MB | OTA 采集、字段配置、展示和诊断职责过重，后续应迁移到聚焦 service，但不改变现有路由。 |
| `public/tailwind.min.css` | 1 | 2.80 MB | 疑似压缩生成/供应商 CSS；当前只标记观察，不在普通瘦身中删除。 |

`npm run self:check` 保持日常可通过，只把默认可清理产物超过阈值视为失败；`npm run self:check:strict` 用于核心大文件拆分阶段，当前预计会因为以上 3 个拆分候选失败。
