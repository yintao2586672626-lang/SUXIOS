...
| 项目 | 体积 | 处理策略 |
|---|---:|---|
| 完整 `HOTEL/` 目录 | 约 286.55 MB | 包含 `.git`、依赖、本地报告和项目代码；会随本地 Git 对象轻微波动。 |
| 不含 `.git` | 约 92.47 MB | 更接近工作副本体积。 |
| 不含 `.git`、`node_modules/`、`vendor/` | 约 63.28 MB | 更接近业务与资料体积。 |
| Git 跟踪文件 | 约 18.36 MB / 613 个文件 | 这是代码提交体积口径。 |
...
|---|---:|---|---|
| `public/index.html` | 1,396 个函数级块；44 个 `currentPage` 引用 | `importKnowledgeUnits` 68 行、`fetchMeituanOrdersData` 68 行、`openSystemConfigModal` 68 行、`testDataConfig` 66 行、`fetchMeituanAdsData` 66 行、`transferDecisionLayerRows` 63 行 | `general` 7,111 行、`ctrip` 2,906 行、`hotel_admin` 1,289 行、`ai` 1,159 行、`meituan` 1,148 行、`ota` 384 行 |
