...
| 项目 | 体积 | 处理策略 |
|---|---:|---|
| 完整 `HOTEL/` 目录 | 约 279.65 MB | 包含 `.git`、依赖、本地报告和项目代码；会随本地 Git 对象轻微波动。 |
| 不含 `.git` | 约 92.34 MB | 更接近工作副本体积。 |
| 不含 `.git`、`node_modules/`、`vendor/` | 约 63.15 MB | 更接近业务与资料体积。 |
...
|---|---:|---|---|
| `public/index.html` | 1,396 个函数级块；44 个 `currentPage` 引用 | `recheckCtripProfileMismatchedFields` 102 行、`runOtaDiagnosisHotelFetch` 100 行、`runCtripBrowserCapture` 100 行、`fetchCtripData` 96 行、`fetchMeituanData` 88 行、`startAiAnalysis` 86 行 | `general` 7,165 行、`ctrip` 3,261 行、`hotel_admin` 1,289 行、`meituan` 1,205 行、`ai` 1,187 行、`ota` 506 行 |
