...
| `local-git-state-open` | open | `@github` | P0 release handoff blocker | PR #2 is open, mergeable, green on head `6192b1bed2ac7efe0e023b7f7793372b88fcaa13`, local worktree is clean, `.git/index.lock` is absent, and `database/backups` has no tracked files; PR #2 is still draft. | `npm run review:release-external-state` | PR #2 is marked ready for review after release-readiness passes, and release external-state evidence passes on the final head. |
