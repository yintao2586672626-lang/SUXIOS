...
| `@codex-security` | Dependency audits and lightweight security checks pass; backups are ignored, not tracked by Git, and the current `review:release-ota-credentials` scan reports no credential-shaped backup text matches. | Formal scan and OTA credential rotation attestation are missing. |
