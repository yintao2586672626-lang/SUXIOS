...
| `local-git-state-open` | open | `@github` | P0 release handoff blocker | Current `review:release-external-state` evidence shows PR #2 is already `MERGED`, mergeability is `UNKNOWN`, and the local worktree is dirty; `.git/index.lock` is absent and `database/backups` has no tracked files. The old draft PR #2 handoff target is stale. | `npm run review:release-external-state` | After release-readiness passes, point `RELEASE_PR_NUMBER` at the actual open non-draft final release PR, keep local Git clean, and pass release external-state evidence on that final head. |

## Controlled But Not Sufficient

| Scope | Controlled evidence | Remaining risk |
|---|---|---|
| `@github` | `.git/index.lock` is absent; `database/backups` has no tracked files; the configured PR #2 still has green historical checks. | Current handoff target is stale because PR #2 is merged, mergeability is unknown, and the local worktree is dirty. A final open release PR must be selected through `RELEASE_PR_NUMBER` before `npm run review:release-external-state` can pass. |
...
5. Run `npm run review:release-external-state` with the actual final release PR number as the final local/PR handoff gate.
