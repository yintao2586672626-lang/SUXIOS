...
6. 最后保持 `RELEASE_PR_NUMBER=6` 指向实际 open final release PR，先用 `npm run review:release-staged-scope` 排除 runtime/local staged 文件，再从本地 HEAD 匹配最终 PR head 的干净 checkout 复验本地 Git 状态和 PR 交接证据，通过 `npm run review:release-external-state`。
