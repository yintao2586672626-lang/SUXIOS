...
- OTA 数据和运营管理是 P0 主线。
- AI 只作为诊断、解释和待确认建议生成能力嵌入主线，不单独扩展为独立产品线。
- 筹建、开业、扩张、转让、投资测算保留现有入口和记录能力，按二期辅助模块管理。
- 未绑定真实 OTA 执行字段、授权和回写结果前，不宣称自动调价、自动房态或自动投放已闭环。

## 0.1 系统构思原则

五大板块不是并列堆功能，而是围绕经营反馈系统分层：

```text
授权OTA可见数据 -> 字段目录 -> 标准事实 -> 收益/流量/竞争圈诊断 -> 待确认AI建议 -> 运营动作 -> 效果复盘 -> 投资判断
```

| 系统视角 | 五大板块中的落点 |
|---|---|
| 存量 | 酒店评分、历史经营数据、资产条件、投资项目、策略沉淀和执行记录。 |
| 流量 | OTA曝光、访客、订单、间夜、收入、调价、促销、投诉和点评新增。 |
| 反馈 | 诊断建议进入运营动作，动作结果回写到复盘，再影响下一轮诊断和投资判断。 |
| 延迟 | 调价、投放、服务优化、点评和投资回报都需要观察期，不用即时结果替代复盘。 |
| 目标 | 从短期订单优化升级为收入质量、运营效率、评分稳定、现金流和投资回报的综合判断。 |
