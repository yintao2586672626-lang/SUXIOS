...
| 开业管理（已启用） | 开业准备总览 | `opening-overview` | 复用现有项目、评分与执行意图链，按绑定酒店权限协作。 |
| 开业管理（已启用） | 开业检查清单 | `opening-checklist` | 复用现有任务、期限、进度与验收记录；不自动证明开业效果。 |
...
员工工单、资产能耗、开业、扩张、转让、投决、策略仿真和可行性报告不再进入当前核心闭环总览。原数据库表、历史记录及酒店数据合并/删除兼容映射继续保留，不作为当前可用能力展示。
