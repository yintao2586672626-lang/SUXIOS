...
| 2 | `production-env-missing` | `@openai-developers` | Prepare controlled production env outside the repository. | `RELEASE_ENV_FILE` points to a real non-template production config, `DB_USER` is not `root`, and `npm run review:release-readiness` no longer reports missing production env. |
