...
Current status: `production-env-missing`, `llm-connectivity-attestation-missing`, and `codex-security-scan-missing` are closed by external/repo evidence and their isolated review commands. `design-handoff-missing`, `ota-credential-rotation-attestation-missing`, and `local-git-state-open` remain open.
