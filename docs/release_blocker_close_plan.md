...
| 2 | `production-env-missing` | `@openai-developers` | Prepare controlled production env outside the repository. | `RELEASE_ENV_FILE` points to a real non-template production config, `APP_DEBUG=false`, `APP_TRACE=false`, `DB_HOST` is not localhost or loopback, `DB_USER` is not `root`, and `npm run review:release-readiness` no longer reports missing production env. |
| 3 | `llm-connectivity-attestation-missing` | `@openai-developers` | Test production `ai_model_configs` through the real `LlmClient` path. | `LLM_CONNECTIVITY_ATTESTATION_FILE` or `docs/llm_connectivity_attestation.json` passes review, contains no secret values, and confirms `redaction_checked=true`. |
...
- For GitHub/local-state review, run `npm run review:release-external-state` with `RELEASE_EXTERNAL_STATE_RESULT_FILE=<controlled-path>`; if Node child_process is blocked, run `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/collect_release_external_state.ps1 -OutputPath docs/release_external_state_evidence.local.json`, then rerun with `RELEASE_EXTERNAL_STATE_FILE=docs/release_external_state_evidence.local.json`.
