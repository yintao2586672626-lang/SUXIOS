...
| `ctrip_source_rows_missing` | 携程目标日没有同日 OTA 源数据行，不能证明今天携程数据已采到。 | 携程收入、流量、转化、字段可信度和 AI 诊断 | 携程最近可用历史数据只能作参考，不能替代目标日 | 默认使用携程浏览器 Profile 采集入口补齐目标日源数据；手动 Cookie/API 仅作临时补数或排障 |
...
| `meituan_source_rows_missing` | 美团目标日没有同日 OTA 源数据行，不能证明今天美团数据已采到。 | 美团收入、流量、转化、字段可信度和 AI 诊断 | 美团最近可用历史数据只能作参考，不能替代目标日 | 默认使用美团浏览器 Profile 采集入口补齐目标日源数据；手动 Cookie/API 仅作临时补数或排障 |
