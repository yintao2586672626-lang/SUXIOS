-- NON-EXECUTING PROPOSAL ONLY.
-- Preserved from a concurrent 2026-07-23 rewrite after the applied migration
-- checksum guard correctly rejected mutation of migration history.
-- Review and redesign as a new forward-only migration before any execution.
-- Backfill remaining tenant-aware history tables after the tenant foundation
-- migration. Each table is repaired in bounded primary-key batches. Every
-- successful batch is committed, so a failed run can safely resume by rerunning
-- this idempotent migration without repeating already-correct rows.
