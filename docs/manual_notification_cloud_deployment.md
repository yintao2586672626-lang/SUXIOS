...
- 触发类型为每日固定时分、整点循环，或 `5–1440` 分钟间隔循环；
- 执行日命中计划的生效星期和可选生效/失效日期；
- 整点/分钟循环只在计划设置的起止时分内执行（默认 `09:00–22:00`）；
- 内容业务日期按计划选择的今日累计 `T0` 或昨日 `T-1` 解析；
- 经营日报计划明确绑定 `携程`、`美团`、`订单来了 PMS` 或兼容的三源汇总范围；
- 计划只校验并发送所勾选的内容分项，未选择来源的缺口不会阻断当前计划；
- 来源证据、同店同日回读状态和 OTA/全酒店范围边界固定附带，不能关闭；
- 酒店、租户、机器人和共享范围校验通过；
...
`outcome_unknown`，不会自动重复发送；超过五分钟调度窗口的计划不补发。
...
先在“运营自动化中心 → 企业微信推送”中分别配置三源计划的发送时分、间隔、内容、
正式群绑定，完成真实测试并启用至少一个计划，
再由有权操作员执行：
