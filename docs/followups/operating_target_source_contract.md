...
- `source_type=pms`：当前仓库未发现可证明的 PMS 日结采集、持久化和回读适配器。PMS 字段可人工录入并保留来源说明，但在真实授权采集→保存→回读完成前不得标为 `verified`。
...
1. 用户提供的经营目标源码包已完成只读复核；值得吸收的 `gap + delta`、时间归一、取消修正、差距变化、量价收矩阵、异常优先和满房质量已沉淀到 `.agents/skills/suxi-ota-revenue-semantic-layer/references/operating-target-delta-detection.md`。当前 `OperatingTargetService` 仍只有单点计算，尚未实现运行时差值引擎，不得宣称功能与朋友项目完全等价。
2. PMS 实时/日结采集尚未完成真实授权、保存和回读验证。
