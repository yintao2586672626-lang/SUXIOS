...
| 问题 | 系统承载面 | 当前证据 | 完成口径 |
|---|---|---|---|
| 今天有没有采到 | 数据健康 / 采集可靠性 | `/api/online-data/collection-reliability`, `collectionHealthSummaryCards` | 能看到平台、门店、采集状态、登录/Cookie 状态、最近采集日志 |
...
数据健康页的“失败原因”列表和排行榜必须把 `platform`、`type`、`reason`、`next_action` 映射成员工可读的平台、失败类型、失败原因和下一步动作；原始失败类型、机器码、接口路径或平台返回原文只能保留在标题追溯或结构化响应中，不能作为员工主文案。登录/Cookie、目标日源数据缺失、字段结构异常、流量/转化缺失、标准事实或收益指标未就绪必须有稳定可读文案，且不能改变携程/美团手动或自动获取逻辑。
数据健康页的“登录/Cookie 记录 / 携程登录/Cookie 记录”必须把 `authorization.list[].platform`、`status`、`message`、`action_hint` 映射成员工可读的平台、登录/Cookie 状态、状态说明和下一步动作；原始平台码、状态码、接口消息或动作提示只能保留在标题追溯或结构化响应中，不能作为员工主文案，也不能改变携程/美团手动或自动获取逻辑。
...
`traffic_source_readiness` 必须同步暴露 P0 酒店级流量 gate 元数据：`p0_traffic_gate_status`、`p0_next_action_mode`、`p0_next_action_entry`、`p0_next_step_count`、`next_command_policy=metadata_only_no_sensitive_commands`、`p0_external_evidence_status`、`p0_pre_import_evidence_status`、`p0_pre_import_evidence_policy`、`p0_traffic_field_fact_status`、`p0_payload_candidate_policy`、`p0_payload_candidate_status_counts`、`p0_payload_candidate_missing_count`、`p0_payload_candidate_unverified_count`、`p0_payload_candidate_paths`、`p0_required_metric_keys`、`p0_required_storage_fields`、`p0_required_field_fact_keys`、`p0_missing_metric_keys`、`p0_field_loop_matrix`、`p0_platform_hotel_identifier_source`、`p0_platform_hotel_identifier_status`、`p0_platform_hotel_identifier_policy`、`p0_target_traffic_data_types`、`p0_source_chain_reference_only`、`p0_source_chain_scope`、`p0_source_chain_policy` 和 `manual_login_state_verified` 缺口。`p0_next_action_mode=browser_profile` 只表示建议用授权浏览器 Profile 补齐美团目标日流量证据，`p0_pre_import_evidence_status=not_provided` 只表示当前未提供外部预导入证据，`p0_payload_candidate_policy=ui_metadata_only_no_import` 只表示员工端展示预期 payload 路径元数据；`missing_expected_payload` 表示预期文件缺失，`expected_payload_present_unverified` 表示文件存在但仍需 importer dry-run 和 P0 verifier 证明，`path_metadata_only_no_payload_content` 表示只展示路径元数据、不展示 payload 内容；这些状态都不能表示采集已成功、目标日入库完成或 P0 闭环完成。`p0_required_metric_keys` 与 `p0_required_storage_fields` 只是闭环要求清单，三者都不能表示采集已成功、目标日入库完成或 P0 闭环完成；`p0_field_loop_matrix` 必须逐项列出 metric key、期望入库字段和当前状态，员工端至少展示“字段矩阵 N 项/未加载 N 项”，不能只展示汇总缺口；`p0_platform_hotel_identifier_status` 必须把平台酒店身份证明标为 `no_target_date_traffic_rows` 或 `requires_p0_verifier`，员工端只能展示状态和来源族，不能展示原始 OTA 酒店 ID/POI ID；没有目标日流量行时，`p0_traffic_field_fact_status` 必须是 `no_target_date_traffic_rows`，员工端展示为“目标日流量字段未加载”，不能写成字段复验中或 ready；即使 `target_date_traffic_rows > 0`，员工端 `p0_traffic_gate_status` 也只能先标记 `requires_p0_verifier`，不能仅凭行数显示 P0 ready，必须由 P0 field-loop verifier 证明字段事实、入库值、平台酒店身份和 UI 状态均 ready 后才可视为闭环。员工端只能展示建议模式、酒店级步骤数、证据状态和所需指标/入库字段数量，不展示 Cookie、token、Profile 原值或可直接复制的敏感命令。P0 field-loop verifier 还必须暴露 `source_chain_reference_only` 和 `source_chain_scope`，员工端 `traffic_source_readiness` 必须暴露对应的 `p0_source_chain_reference_only` 和 `p0_source_chain_scope`：当目标日 source rows 为 0 时，必须标记为 `no_target_date_source_rows`；当目标日 source rows 只有 `business` 等非 `traffic/flow/conversion` 类型时，必须标记为 `reference_only_non_traffic_source_rows`，不能把缺失源数据或 source path/metric/storage 的参考闭环当成 P0 流量闭环。

`traffic_source_readiness` 的候选 payload 诊断还必须暴露 dry-run 聚合字段：`p0_payload_candidate_target_date_rows`、`p0_payload_candidate_traffic_evidence_rows`、`p0_payload_candidate_evidence_source_path_rows`、`p0_payload_candidate_evidence_structured_source_path_rows`、`p0_payload_candidate_evidence_raw_data_field_facts_rows`、`p0_payload_candidate_evidence_raw_data_exposed_rows`、`p0_payload_candidate_evidence_sensitive_value_rows`、`p0_payload_candidate_evidence_metric_keys`、`p0_payload_candidate_evidence_missing_metric_keys`。这些字段只用于说明候选文件能否进入 importer/P0 verifier，不展示 raw payload、Cookie、token、Profile 路径或平台原始酒店标识。

`traffic_source_readiness` 还必须暴露采集入口聚合状态：`p0_profile_login_trigger_policy`、`p0_profile_login_trigger_available_count`、`p0_profile_login_trigger_unavailable_count`、`p0_after_login_sync_available_count`、`p0_manual_login_state_verified_count`。这些字段只表示系统有多少账号使用者本机授权入口、登录后同步入口和人工确认登录态；不能表示 OTA 已登录、已采到目标日数据或已完成入库。服务端/管理员电脑不得打开 OTA 登录窗口。
当目标日 traffic 行存在时，`p0_field_loop_matrix` 不能停留在占位矩阵：员工端必须按指标展示 `complete`、`incomplete`、`missing` 或 `requires_p0_verifier` 状态计数（完成/待补/缺事实/待复验），且 `complete` 必须同时证明采集证据、结构化 source path、期望入库字段、入库值和 UI ready。
员工端还必须暴露 `p0_traffic_closure_chain` 与 `p0_traffic_closure_chain_policy`：链路必须逐项覆盖 `capture_evidence`、`source_path`、`metric_key`、`storage_field`、`stored_value`、`ui_status`、`platform_hotel_identifier`、`verifier`；没有目标日 traffic 行时，除 `verifier` 外的链路状态必须是 `no_target_date_traffic_rows`，`verifier` 必须保持 `incomplete`，不能显示 ready；有目标日 traffic 行时，`verifier` 仍必须是 `requires_p0_verifier`，直到 P0 field-loop verifier 返回 ready。该链路只代表 OTA-channel evidence，不能上升为 whole-hotel operating truth。
P0 traffic payload 导入前必须证明平台酒店身份：携程行必须携带 `hotelId`/`hotel_id` 等平台酒店标识，美团行必须携带 `poiId`/`poi_id` 等平台 POI 标识；`system_hotel_id` 只能作为宿析本地酒店范围过滤，不能替代 OTA 源身份，也不能用于伪造平台酒店标识。外部 `traffic_evidence` 只能暴露 `platform_hotel_identifier_present=true` 与来源族（如 `hotel_id_family`/`poi_id_family`），不能输出原始 OTA 酒店 ID 或 POI ID。
...
## 2026-06-27 P0 最近同步任务诊断

- `traffic_source_readiness` 必须同步暴露最近同步任务诊断聚合：`traffic_latest_sync_task_count`、`traffic_latest_sync_task_status_counts`、`traffic_latest_sync_task_message_code_counts`、`traffic_latest_sync_task_saved_count`、`traffic_latest_sync_task_normalized_count`、`traffic_latest_sync_task_sensitive_values_exposed`。这些字段只能作为今日巡检诊断线索，不能作为采集成功、目标日入库完成或 P0 闭环完成证据。
- 员工端必须把 `login_or_profile_not_ready`、`sync_completed_without_saved_rows`、`sync_normalized_without_saved_rows`、`browser_dependency_missing`、`no_rows_parsed` 映射成原因分类，例如“登录/Profile未就绪”“同步完成但未入库”“已标准化但未入库”“浏览器依赖缺失”“未解析到业务行”。员工端不得展示原始同步任务 `message`、Cookie、token、Profile 路径或平台敏感标识；`traffic_latest_sync_task_sensitive_values_exposed` 必须保持 `false`。

## 2026-06-27 P0 标准事实层摘要

- `traffic_source_readiness` 必须同步暴露标准事实层派生摘要：`p0_standard_fact_policy`、`p0_standard_fact_status`、`p0_standard_fact_raw_data_policy`、`p0_standard_fact_required_metric_count`、`p0_standard_fact_complete_metric_count`、`p0_standard_fact_missing_metric_count`、`p0_standard_fact_incomplete_metric_count`、`p0_standard_fact_storage_field_count`、`p0_standard_fact_status_counts`、`p0_standard_fact_complete_metric_keys`、`p0_standard_fact_missing_metric_keys`、`p0_standard_fact_incomplete_metric_keys`。`p0_standard_fact_policy=derived_from_p0_field_loop_matrix_ota_channel_only` 表示该摘要只能从 P0 字段矩阵派生，不能替代整店经营事实；`p0_standard_fact_raw_data_policy=raw_data_field_facts_only_raw_payload_not_returned` 表示员工端只展示 raw_data 中的字段事实状态，不返回 raw payload。
- 当目标日 traffic 行为 0 时，`p0_standard_fact_status` 必须是 `missing_target_date_traffic_rows`，`p0_standard_fact_missing_metric_count` 必须反映缺失指标数，不能写成 `ready`。只有字段矩阵内每个指标都完成采集证据、source path、metric key、入库字段、已入库值和 UI ready 后，标准事实层摘要才允许显示 `ready`；即便如此，P0 闭环仍以 P0 field-loop verifier 的 `p0_traffic_gate.status=ready` 为准。

## 2026-06-27/2026-06-28 P0 Profile 下一步报告

- 项目必须提供只读命令 `npm.cmd run report:p0-profile-next-steps -- --date=<YYYY-MM-DD>`，用于把 P0 field-loop verifier 的脱敏元数据整理成运营可执行清单；当运营明确要求某个平台暂不采集时，命令必须支持 `--skip-platform=<platform>`，例如 `--skip-platform=meituan`。
- 该命令只能读取 `verify_p0_ota_field_loop_closure.php` 输出中的 `profile_login_trigger`、`after_login_sync`、`p0_verifier_command`、`manual_login_state_verified`、`target_date_rows` 和 `traffic_rows` 等元数据；不得触发 OTA 登录、采集、入库或状态写入。
- 报告必须标注脱敏策略 `metadata_only_no_cookie_token_profile_path_or_raw_payload`，并且不得输出 Cookie、token、Profile 原始路径、raw payload、手机号、账号密码或平台敏感标识。
- 当报告列出本机授权入口时，必须同时保留 `manual_login_state_verified=false` 和复验命令，提醒员工 Profile 目录存在不等于登录态已验证；只有账号使用者在自己电脑完成平台登录/验证码/短信/权限确认并导入授权证据后，才能继续触发同步和 P0 复验。
- 报告必须暴露 `collection_policy`：`mainline_mode=browser_profile`，`temporary_mode=manual_cookie_api`，`temporary_mode_policy=temporary_only`。`mainline_required_gates` 必须包含 `manual_login_state_verified`、`target_date_ota_rows`、`target_date_traffic_rows` 和 `p0_field_loop_verifier_ready`；`forbidden_claims_before_ready` 必须禁止把手动 Cookie/API 当成默认主线、把 Profile 目录存在当成已登录、把同步任务成功当成 P0 闭环、把历史行当成目标日闭环。
- 报告必须暴露 `operator_sequence`，但不能机械地把所有数据源都列为 `manual_login` → `after_login_sync` → `single_scope_verifier`。当平台已由 P0 verifier 证明 target-day traffic gate ready 时，必须输出 `already_ready` + `single_scope_verifier`，并把酒店级步骤显示为 `already_ready_no_login` / `already_ready_no_sync`，不再展示登录或同步入口。当平台被运营显式跳过时，必须输出 `operator_skip` + `single_scope_verifier`，并显示 `p0_skipped_by_operator_reference_only_no_collection`、`login_verified_reference_only`、`skipped_by_operator_no_capture`、`skipped_by_operator_no_sync` 和 `operator_skip_platforms`，不得展示该平台的采集入口、本机授权入口或 after-login sync 入口。只有未 ready 且未 skip 的数据源才允许展示 `manual_login` 和 `after_login_sync`；`manual_login` 只能指向账号使用者本机授权，不得指向服务端登录触发；`after_login_sync` 必须声明前置 `manual_login_state_verified=true`，不能让员工把 Profile 目录存在或同步任务成功误认为闭环。
- 报告必须暴露 `completion_gate.command`，默认是 `npm.cmd run verify:p0-ota-field-loop -- --date=<YYYY-MM-DD>`，并声明 `required_status=ready`。只有全局 P0 verifier 证明目标日 OTA 行、字段事实、source path、metric key、入库字段、已入库值和 UI 状态均 ready 后，才能继续向收益分析、AI 决策、运营闭环和投资判断推进。
- 报告必须暴露 `downstream_gate`：当 `completion_gate.current_status` 不是 `ready` 时，`downstream_gate.status` 必须是 `blocked_by_p0_ota_gate`，`blocked_stage_keys` 必须包含 `revenue_analysis`、`ai_decision_advice`、`operation_closure`、`investment_judgment`，`allowed_claims` 只能表达结构已准备、历史行仅参考、不能声明整店或下游闭环完成；这些字段用于机器判断下游推进边界，不能替代 P0 verifier。
