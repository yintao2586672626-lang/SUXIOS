...
## 2026-06-27 P0 Profile 下一步报告

- 项目必须提供只读命令 `npm.cmd run report:p0-profile-next-steps -- --date=<YYYY-MM-DD>`，用于把 P0 field-loop verifier 的脱敏元数据整理成运营可执行清单。
...
- 报告必须暴露 `operator_sequence`，按 `manual_login` → `after_login_sync` → `single_scope_verifier` 顺序列出每个数据源的人工登录、登录后同步和单店复验步骤；`after_login_sync` 必须声明前置 `manual_login_state_verified=true`，不能让员工把 Profile 目录存在或同步任务成功误认为闭环。
