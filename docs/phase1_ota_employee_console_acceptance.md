...
- 当报告列出登录触发入口时，必须同时保留 `manual_login_state_verified=false` 和复验命令，提醒员工 Profile 目录存在不等于登录态已验证；只有人工完成平台登录/验证码/短信/权限确认后，才能继续触发同步和 P0 复验。
