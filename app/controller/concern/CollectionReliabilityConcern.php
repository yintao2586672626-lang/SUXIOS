    ...
    private function buildCollectionPendingActions(array $authorizationRows, array $alerts, array $collectionLogs, array $qualityRows): array
    {
        ...
        foreach ($collectionLogs as $log) {
            ...
            $actions[] = [
                ...
                'action' => '检查授权、字段结构和平台响应后，默认使用现有浏览器 Profile 采集入口重试；手动 Cookie/API 仅作临时补数或排障',
                'next_action' => '检查授权、字段结构和平台响应后，默认使用现有浏览器 Profile 采集入口重试；手动 Cookie/API 仅作临时补数或排障',
                'entry' => '',
                'owner' => '产品/技术 + 酒店运营人员',
        ...
        if ($qualityRows === []) {
            $actions[] = [
                ...
                'action' => '默认使用携程/美团浏览器 Profile 采集入口补齐同日数据，再查看字段可信度、收益指标、AI 诊断和执行动作；手动 Cookie/API 仅作临时补数或排障',
                'next_action' => '默认使用携程/美团浏览器 Profile 采集入口补齐同日数据，再查看字段可信度、收益指标、AI 诊断和执行动作；手动 Cookie/API 仅作临时补数或排障',
