...
class InitDatabase extends Command
{
    ...
    protected function initMySQL(Output $output)
    {
        ...
        Db::execute("
            CREATE TABLE IF NOT EXISTS user_hotel_permissions (
                ...
                can_fill TINYINT DEFAULT 0,
                can_edit TINYINT DEFAULT 0,
                can_fetch_ota TINYINT DEFAULT 0,
                can_delete_ota TINYINT DEFAULT 0,
                can_export TINYINT DEFAULT 0,
                can_ai TINYINT DEFAULT 0,
                can_operation TINYINT DEFAULT 0,
                can_investment TINYINT DEFAULT 0,
    ...
    protected function insertDefaultData(Output $output)
    {
        ...
        $roles = [
            ...
            ['id' => 2, 'name' => 'beta_user', 'display_name' => 'VIP 用户', 'description' => '酒店经营者；可新增酒店，只能访问和管理自己名下酒店', 'level' => 2, 'permissions' => json_encode(['dashboard.view', 'hotel.create', 'hotel.view', 'hotel.update', 'ota.view', 'ota.collect', 'report.view', 'report.export', 'ai.view', 'ai.execute', 'operation.view', 'operation.execute', 'investment.view', 'investment.simulate'])],
            ['id' => 3, 'name' => 'normal_user', 'display_name' => '普通用户', 'description' => '数据查看者；只能查看授权酒店，不可新增酒店或发起 OTA 采集', 'level' => 3, 'permissions' => json_encode(['dashboard.view', 'hotel.view', 'ota.view', 'report.view'])],
        ];
        ...
        // 插入示例酒店
        $hotels = [
            ['name' => '东方大酒店', 'code' => 'DF001', 'address' => '北京市朝阳区', 'contact_person' => '张经理', 'contact_phone' => '010-12345678', 'owner_user_id' => 2, 'created_by' => 2],
