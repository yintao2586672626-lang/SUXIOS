...
final class ManagerCapabilityScoringService
{
    ...
    public function profile(
        ...
        $rows = Db::name(self::CASE_TABLE)->alias('c')
            ->leftJoin(self::SCORE_TABLE . ' s', 's.case_id = c.id AND s.formula_version = \'' . self::FORMULA_VERSION . '\'')
            ->where('c.tenant_id', $tenantId)
            ->where('c.hotel_id', $hotelId)
            ->where('c.manager_user_id', $managerUserId)
            ->field('c.*,s.formula_version,s.source_reference_key,s.source_fingerprint,s.dimensions_json,s.case_score,s.scored_dimension_count,s.score_status,s.evidence_digest,s.created_at AS scored_at')
            ->order('c.business_date', 'desc')
            ->order('c.id', 'desc')
            ->limit(1000)
            ->select()
            ->toArray();
        ...
        $adjustments = $this->adjustmentsForCases($tenantId, $hotelId, $managerUserId, $caseIds);
        $reviews = $this->reviewsForCases($tenantId, $hotelId, $managerUserId, $caseIds);
        $publicCases = array_map(
            fn(array $row): array => $this->publicCase(
                $row,
                $followups[(int)$row['id']] ?? [],
                $adjustments[(int)$row['id']] ?? [],
                $reviews[(int)$row['id']] ?? []
            ),
            $rows
        );
        usort($publicCases, static function (array $left, array $right): int {
        ...
        $publicCases = array_values(array_filter(
            $publicCases,
        ...
        return [
            ...
            'profile_status' => $aggregate['status'],
            'profile_label' => $aggregate['label'],
    ...
    public function followupQueue(
        ...
        $byManager = [];
        foreach ($rows as $row) {
            $byManager[(int)$row['manager_user_id']][] = $row;
        }

        $queueRows = [];
        foreach ($byManager as $rowManagerId => $managerRows) {
            $caseIds = array_map(static fn(array $row): int => (int)$row['id'], $managerRows);
            $followups = $this->followupsForCases($tenantId, $hotelId, $rowManagerId, $caseIds);
            $adjustments = $this->adjustmentsForCases($tenantId, $hotelId, $rowManagerId, $caseIds);
            $reviews = $this->reviewsForCases($tenantId, $hotelId, $rowManagerId, $caseIds);
            foreach ($managerRows as $row) {
                $caseId = (int)$row['id'];
                $case = $this->publicCase(
                    $row,
                    $followups[$caseId] ?? [],
                    $adjustments[$caseId] ?? [],
                    $reviews[$caseId] ?? []
                ...
                $due = new \DateTimeImmutable((string)$case['current_followup_due_date'], new \DateTimeZone('Asia/Shanghai'));
                $daysOffset = (int)$today->diff($due)->format('%r%a');
                if ($daysOffset > 7) {
                    continue;
                }
                $bucket = $daysOffset < 0 ? 'overdue' : ($daysOffset === 0 ? 'today' : 'upcoming');
                $case['due_bucket'] = $bucket;
        ...
        return [
            ...
            'data_status' => $queueRows === [] ? 'empty' : 'ready',
            'counts' => $counts,
            'rows' => $queueRows,
            'source' => [
    ...
    private function followupsForCases(
        ...
        $rows = Db::name(self::FOLLOWUP_TABLE)
            ->where('tenant_id', $tenantId)
            ->where('hotel_id', $hotelId)
            ->where('manager_user_id', $managerUserId)
    ...
    private function adjustmentsForCases(
        ...
        $rows = Db::name(self::ADJUSTMENT_TABLE)
            ->where('tenant_id', $tenantId)
            ->where('hotel_id', $hotelId)
            ->where('manager_user_id', $managerUserId)
            ->whereIn('case_id', $caseIds)
    ...
    private function publicCase(
        ...
        $projectedCaseStatus = (string)($projection['case_status'] ?? $row['case_status']);
