...
class OperationManagementService
{
    ...
    private function buildSourceVerifiedMetricReadbackPayload(array $task, array $intent): ?array
    {
        ...
        $isRevenueCockpitAction = $sourceModule === RevenueCockpitApprovalService::SOURCE_MODULE;
        if (!$isPatrolGapTask
        ...
        if ($sourceModule === 'ota_diagnosis_saved' || $isOperatingQuestion || $isRevenueCockpitAction) {
    ...
    private function hasVerifiedExecutionSourceProvenance(array $intent, array $task): bool
    {
        ...
        return match (strtolower(trim((string)($intent['source_module'] ?? '')))) {
            ...
            RevenueCockpitApprovalService::SOURCE_MODULE =>
                (new RevenueCockpitApprovalService())->isIntentCurrent($intent),
            default => false,
        };
    }
    ...
    private function normalizeTrustedExecutionIntentIdempotencyKey(?string $value): ?string
    {
        ...
        $value = trim($value);
        if (preg_match('/^(?:expansion:v1:[1-9][0-9]*|ota_diagnosis_action_[a-f0-9]{32}:attempt:[1-9][0-9]*|operation_alert_[a-f0-9]{32}|operating_target_[a-f0-9]{32}|operation_optimizer_[a-f0-9]{32}|operating_network_replication_[a-f0-9]{32}|operating_question_action_[a-f0-9]{32}|operation_action_[a-f0-9]{32}|source_intent_[a-f0-9]{32})$/D', $value) !== 1) {
