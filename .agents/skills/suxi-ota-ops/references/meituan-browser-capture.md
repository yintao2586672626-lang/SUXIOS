...
> 公共采集流程见 [平台采集器适配合同](platform-collector-adapter-contract.md)；本文件只定义美团的身份、页面/端点、字段、单位、日期和失败差异。
...
| 平台字段 | 业务含义 | 宿析OS映射 |
| --- | --- | --- |
| `data.myHotel.intentionPerExposure` | 曝光→浏览转化率 | `raw_data.exposure_to_browse_rate`，并写入对应 `field_facts` |
| `data.myHotel.payOrderPerIntention` | 浏览→支付转化率 | `flow_rate` |

- 两个转化率属于不同漏斗阶段，禁止互相覆盖或都写入 `flow_rate`。
- 平台明确返回的 `0` 是真实零值；字段未返回则保持 `missing`，不得由曝光人数、浏览人数或支付订单数补算平台原值。
- 首页“曝光量”和流量转化漏斗“曝光人数”不是同一字段；今日实时播报以流量转化漏斗同一快照中的曝光人数、浏览人数、支付订单数和两级转化率为准。
- 昨日数据或未来 30 天在平台更新窗口内未展示时，状态应为 `pending_source_update` 或 `missing`，不得写成 0。

### 实时发送门禁

实时播报必须依次满足：

`同门店新采集 → 目标业务日匹配 → 保存 → 数据库精确回读 → 生成候选消息 → 企业微信发送 → 业务成功回执`

- 历史成功和过期快照不能证明当前采集成功；发送前必须检查采集新鲜度，过期则先重新采集并回读。
- 门店身份以当前环境真实绑定为准，不做数值 ID 猜测、跨环境映射或数据复制。本地 `system_hotel_id=80` 的事实不能替代生产环境其他 ID 的事实。
- 企业微信 HTTP 请求完成不等于送达；只有业务回执 `wecom_business_success` 才标记测试发送成功。
- 手动测试发送成功不自动启用正式群、发送计划或 timer；三者必须分别配置和验收。
- 用户可见消息只保留标题、门店、业务日、采集时间和核心指标。模式、任务号、内部状态、数据范围等证据保留在结构化记录中，不在每条运营消息里重复堆叠。
