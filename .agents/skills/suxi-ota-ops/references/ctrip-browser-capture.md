...
>
> 公共采集流程见 [平台采集器适配合同](platform-collector-adapter-contract.md)；本文件只定义携程的身份、页面/端点、字段、单位、日期和失败差异。

## 定位

携程数据获取分一条日常主线和一条临时补充路径：

1. 自动获取（默认主线）：宿析OS使用每个门店独立 Profile 打开携程商家后台，在授权范围内监听页面业务 JSON，并按模块入库。
2. 手动获取（临时路径）：用户提供携程后台导出文件、Trip Connect 接口结果，或已取得的 Cookie、`spidertoken`、`node_id`、Payload 等上下文；宿析OS只负责校验、抓取/导入、清洗和入库。

自动路径服务日常采集、日报、巡检和预警；手动 Cookie/API 只用于临时补数、首次接入、平台改版排障或自动采集失效后的补录，不作为默认运营主线。

## 当前优先业务

| 优先级 | 数据模块 | 主要用途 | 推荐路径 |
| --- | --- | --- | --- |
| P0 | 经营概况、订单、间夜、收入 | OTA 经营日报、收益分析 | 自动 Profile 主线；手动导入/接口上下文仅临时补充 |
| P0 | 流量、转化、排名 | 周月看板、漏斗诊断 | 自动 Profile 主线；手动报表或 Payload 仅临时补充 |
| 暂缓 | 点评、评分、回复 | 平台管控较严，短期投入产出低 | 仅保留显式手动入口，不进入默认自动采集 |
| P1 | 房态、房价、ARI、产品 | 房态价量日报、价格巡检 | 自动 Profile 主线；Trip Connect/API/导出仅作授权补充 |
...
手动路径只适合临时补数、首次接入、平台改版排障、自动采集失效时补录。
