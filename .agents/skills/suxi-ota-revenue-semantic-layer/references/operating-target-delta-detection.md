...
- 已进入运行时：
  - 订单来了与美团云 PMS 作为两个平级独立真实数据源，各自完成门店、日期、内部对账、保存和数据库回读门禁。
  - 每个 PMS 可与自身上一条同店、同日、同范围、已验证快照计算房费、净已售、ADR、OCC、RevPAR 和可售房量变化，并按真实间隔计算每小时净拾取与房费速度。
  - 第一条合格快照只建立基线；累计房费或已售回落优先输出 `PMS_DELTA_REVERSAL_UNKNOWN`；房量基数变化输出重新建基线；取消累计缺失时不计算毛预订。
  - 两个 PMS 只有在各自已验证且采集时点接近时才做同口径横向差值；总房费与预计房费只并列展示，不直接相减，也不自动选择真值。
  - 结果从 `PMS经营数据` 页面回显，差值只做诊断，不改变任一来源自己的真实性状态。
- 尚未实现：带版本的当日节奏曲线、目标差距收窄/扩大、同星期历史分布校准、取消累计采集、动作因果绑定和差值提醒状态机。
- 当前可声明为“PMS 核心相邻快照差值已集成”；完整经营目标节奏引擎仍为后续范围。
